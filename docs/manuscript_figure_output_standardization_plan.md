...
| Figure 7 | Six in-vivo TGI, CellCycle pseudotime, and state-pathway source panels | Day-17 TGI calculation and associations, selected ECDF comparisons, and pathway activity across pseudotime 0.30-0.49 | `Code/in-vivo/figure7/` | Opt-in `standard` | `--modules in_vivo_figure7`; `--figure7-panels-ae-only` while F is unavailable; explicit full-analysis RDS/gene-set flags | A reviewed five-panel A-E contract can run now. The complete six-panel/default inclusion remains blocked until the canonical saved-state tables and pinned gene-set artifact are exported. Final A-F composition remains manual. |

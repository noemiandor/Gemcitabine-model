...
| Figure 7 | Publication-scale `figures/Figure7/Figure7_reviewed_GRCh.{pdf,png}` A-K composite plus matched scientific source PDF/PNG assets from `panel_7A_day17_tgi_calculation.*` through `panel_7F_pseudotime_state_pathway_activity.*` | `Results/in-vivo/figure7/runs/20260801_figure7_publication_scale_v4_figure7/` |
...
`Figure7_reviewed_GRCh.{pdf,png}` at 7.1 x 9.7 inches. Its six fixed rows are
A/B; C/D/E; F/G; H; I; and J/K. The A-K display order is frozen to first citation:
A=source 7A, B=source 7C, C-E=SI4A-C, F=SI4E, G=SI7B, H=source 7B,
I=source 7F, and J-K=source 7D-E. Supplementary copies remain published.
...
panel letters refer to the assembled A-K mapping described above, whose
...
- Add a small script or Makefile target for each remaining composite figure that is still assembled manually. Figure 7 now has a scripted, publication-scale A-K compositor; several earlier figures remain manual/PowerPoint-based.

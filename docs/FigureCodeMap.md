...
| Figure 7 | Publication-scale `figures/Figure7/Figure7_reviewed_GRCh.{pdf,png}` A-L composite plus matched scientific source PDF/PNG assets from `panel_7A_day17_tgi_calculation.*` through `panel_7F_pseudotime_state_pathway_activity.*` | `Results/in-vivo/figure7/runs/20260802_figure7_copy_number_relayout_v7_figure7/` |
| Supplementary | Existing Supplementary 1-2 source panels plus final `panel_SuppFig4_composite.*` through `panel_SuppFig7_composite.*` PDF/PNG pairs | Supplementary 1-2 use their existing module runs; Supplementary 4-7 are regenerated by the frozen-input `si_figures` module. |
...
I=source 7F, J=the exact 9,832-cell QC-filtered NUMBAT chromosome heatmap,
and K-L=source 7D-E. The density-localization component of source 7B is
published as SI4I; Supplementary copies of the shared context panels remain published.
`--figure7-panels-ae-only` retains the explicit five-source-panel mode and does
not create the composite. The cache-first `full-refit` path can reconstruct the
shared final Seurat object from 18 external Cell Ranger H5 matrices or use the
checksum-pinned deposited final RDS. Manager schedules `si_figures` first,
passes its run-scoped human-only 11-table cache plus complete lineage into
Figure 7, and then runs only missing scVelo, TGI-table, and human-only
pathway-support stages. Full-refit writes
`Figure7_generated_GRCh_candidate.{pdf,png}`; it cannot consume the reviewed SI cache
...
canonical output is the publication-scale
`figures/Figure7/Figure7_reviewed_GRCh.{pdf,png}` pair. Presentation polishing
does not alter the scientific source-panel contract.
...
|---|---|---|
| Supplementary Figure 4 | Tumor and CellLine UMAPs, S-phase score, equal-sample context/ploidy composition with sample-level enrichment tests, the explicit 2,881-cell CellCycle subset count, and the mouse-balanced pseudotime localization analysis in panel I | `figures/Supplementary/panel_SuppFig4_composite.pdf` and `.png` |
...
| Supplementary Figure 6 | Endpoint tumor ploidy overall/by origin/by mouse and the descriptive injected-reference-proxy-to-endpoint comparison; the QC-filtered chromosome-level copy-number heatmap is main Figure 7J | `figures/Supplementary/panel_SuppFig6_composite.pdf` and `.png` |
...
- Add a small script or Makefile target for each remaining composite figure that is still assembled manually. Figure 7 now has a scripted, publication-scale A-L compositor; several earlier figures remain manual/PowerPoint-based.

...
\textbf{(H)} Equal-mouse mean empirical cumulative distribution functions (ECDFs) of cell-cycle-associated pseudotime for 2,881 tumor cells in clusters 4c, 6, and 10 across the 16 tumors (1,328 vehicle-treated and 1,553 gemcitabine-treated cells). Gray and blue curves denote vehicle and gemcitabine treatment, respectively. The combined comparison contains eight mice per group, and each injected-origin comparison contains four mice per group; solid curves show the combined and 2N-origin comparisons, whereas dashed curves show the 4N-origin comparison. The combined comparison gave RMSE \(=0.0378\) and exact mouse-label permutation \(P=0.00513\); the 4N- and 2N-origin comparisons gave RMSE \(=0.0317\), \(P=0.143\), and RMSE \(=0.0456\), \(P=0.0571\), respectively. The lower strip shows the equal-mouse Gaussian-kernel density contrast for all tumors (gemcitabine minus vehicle) and its two-sided 95\% simultaneous permutation null envelope in gray. The pale-blue band marks the connected positive region with pointwise two-sided permutation \(P\leq0.05\) (0.296--0.486); the dark-blue band and curve segment mark the region that also passed studentized max-\(|T|\) family-wise control (0.414--0.426). The circle marks the largest raw density difference at 0.452, the diamond marks the strongest standardized evidence at 0.420, and the dashed lines mark the frozen 0.30--0.49 state interval used in I. The global exact injected-origin-stratified max-\(|T|\) permutation test gave \(P=0.0473\) across 4,900 assignments.

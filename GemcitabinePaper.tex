...
\textbf{(C)} Joint uniform manifold approximation and projection (UMAP) of 35,513 cells---9,832 final-QC tumor cells from 16 tumors and 25,681 cells from two cultured SUM-159 reference preparations---colored by the nine graph-based clusters. Color-matched cluster-number labels mark cluster locations and provide the cluster-color key.
\textbf{(D)} The same UMAP colored by injected tumor origin or cultured-reference ploidy, with blue indicating 2N and vermilion indicating 4N, as shown in the in-panel key.
...
\captionsetup{list=off,singlelinecheck=false}
\caption{\footnotesize
\textbf{Tumor growth, tumor growth inhibition, cellular states, and endpoint ploidy in gemcitabine-treated SUM-159 xenografts (continued; panels G--L).}
...
\textbf{(J)} Cell-by-chromosome heatmap for the exact 9,832-cell final-QC tumor universe (4,880 2N-origin and 4,952 4N-origin cells; 5,335 treated) selected from the validated 14,125-cell CBS inventory; clusters 3, 4, 9, and 9c were excluded. Each entry is the length-weighted mean copy-number state across segments available for that chromosome within its origin-specific CBS schema. Columns are chromosomes 1--22; rows are ordered by injected origin, dose, mouse, ploidy, and cell identifier, with no clustering. The matrix color scale runs from blue for lower copy-number states through gold for copy number 3 to red and purple for higher states; gray denotes unavailable chromosome-level values. From the matrix outward, annotation bars denote injected origin (2N, blue; 4N, vermilion), dose (vehicle, gray; 30~mg/kg, orange; 120~mg/kg, green), and mouse, with every annotation color defined in the accompanying key. The two origin-specific schemas are displayed together but were not locus-aligned across runs.
...
\textbf{(L)} Unadjusted association across the same eight treated tumors between Day-17 TGI and mean endpoint tumor-cell ploidy. NUMBAT-derived cell estimates were restricted to the 5,335 treated cells within the 9,832-cell final-QC tumor universe before calculating one mean per mouse. Text labels identify mice; point colors and shapes are as in K. The black line is the linear fit and the gray band its 95\% confidence interval. The annotation reports Pearson's \(r\) and an exact unrestricted permutation \(P\) value. Because the 2N- and 4N-origin cohorts were processed in separate NUMBAT runs with different segment schemas and calibration, this unadjusted association is descriptive and does not establish an origin- or dose-independent endpoint-ploidy effect. TGI, tumor growth inhibition; UMAP, uniform manifold approximation and projection; ECDF, empirical cumulative distribution function; RMSE, root-mean-square error; NES, normalized enrichment score.
}
\end{figure}
\endgroup
\clearpage
...
\textbf{(I)} Mouse-balanced localization of the gemcitabine-associated cell excess along cell-cycle pseudotime. The curve is the equal-mouse treated-minus-vehicle Gaussian-kernel density contrast, and the gray band is its two-sided 95\% simultaneous permutation null envelope. The pale-blue band marks the connected positive region with pointwise two-sided permutation \(P\leq0.05\) (0.296--0.486); the dark-blue band and curve segment mark the region that also passed studentized max-\(|T|\) family-wise control (0.414--0.426). The circle marks the largest raw density difference at 0.452, the diamond marks the strongest standardized evidence at 0.420, and dashed lines mark the frozen 0.30--0.49 state interval used in Fig.~\ref{fig:in_vivo_pseudotime_tgi}I. The global exact injected-origin-stratified max-\(|T|\) permutation test gave \(P=0.0473\) across 4,900 assignments.
}

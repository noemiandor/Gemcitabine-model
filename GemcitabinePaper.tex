...
Equal-mouse mean empirical cumulative distribution functions (ECDFs) of cell-cycle-associated pseudotime differed between the vehicle and gemcitabine groups (eight tumors each; RMSE \(=0.0378\), exact permutation \(P=0.00513\); Fig.~\ref{fig:in_vivo_pseudotime_tgi}H). The shift had the same direction within both injected-origin strata, although the smaller comparisons were not independently significant (2N origin: RMSE \(=0.0456\), \(P=0.0571\); 4N origin: RMSE \(=0.0317\), \(P=0.143\)). In a mouse-balanced density analysis, the raw treated-minus-vehicle density difference was largest at pseudotime 0.452, whereas the standardized evidence was strongest at 0.420. The positive region with simultaneous family-wise support spanned 0.414--0.426 (exact injected-origin-stratified max-\(|T|\) permutation \(P=0.0473\); Supplementary Fig.~\ref{fig:supp_cellular_landscape}I). The maximal connected region with a positive treated-minus-vehicle density contrast and nominal two-sided exact injected-origin-stratified pointwise permutation support spanned pseudotime 0.296--0.486 (96 grid points; pointwise \(P=0.00245\)--0.0494 across 4,900 assignments). No single region-level \(P\) value was calculated for this boundary-selected region, and only the narrower 0.414--0.426 region had simultaneous support. We defined the computed 0.296--0.486 region as the gemcitabine-associated cell-excess interval and, without rounding its bounds, compared model-estimated expression within it with that in the two equally wide adjacent intervals, [0.106,0.296) and (0.486,0.676], using mouse-aware pseudobulk models adjusted for dose and injected origin (Fig.~\ref{fig:in_vivo_pseudotime_tgi}I). The profiled state was negatively enriched for MYC- and E2F-target programs, oxidative phosphorylation, DNA replication and repair, translation, rRNA processing, and ribosome biogenesis; the genes contributing most strongly to these enrichment signals also had lower model-estimated expression in the interval than in its flanks. Representative signals included MYC targets (\(\mathrm{NES}=-4.52\), FDR \(=7.13\times10^{-73}\)), DNA replication (\(\mathrm{NES}=-4.20\), FDR \(=6.86\times10^{-71}\)), and oxidative phosphorylation (\(\mathrm{NES}=-4.18\), FDR \(=9.69\times10^{-58}\)). Conversely, receptor-associated programs were positively enriched, and the genes contributing most strongly to these signals had higher model-estimated expression in the interval. Representative signals included class A/1 rhodopsin-like receptors (\(\mathrm{NES}=1.96\), FDR \(=8.48\times10^{-9}\)), GPCR ligand binding (\(\mathrm{NES}=1.78\), FDR \(=1.93\times10^{-7}\)), and peptide ligand-binding receptors (\(\mathrm{NES}=1.85\), FDR \(=3.82\times10^{-5}\)). Together, the gemcitabine-associated excess of cells in this region and its molecular profile identify a replication- and biosynthesis-attenuated cell-cycle-associated state with increased receptor-associated expression. The molecular inferences are conditional on the data-selected interval boundaries; the pathway model compared the selected state with its flanks and therefore did not estimate a treatment effect at fixed pseudotime.

The extent of this transcriptional redistribution was related to tumor response. Across the eight treated tumors, pseudotime-distribution displacement from the equal-mouse vehicle reference matched by injected origin was positively associated with Day-17 TGI after both quantities were centered within dose (\(r=0.740\), exact within-dose permutation \(P=0.0174\); Fig.~\ref{fig:in_vivo_pseudotime_tgi}K). Among these same tumors, Day-17 TGI showed a sizeable inverse association with mean endpoint tumor-cell ploidy calculated exclusively from the 5,335 final-QC treated cells (Pearson \(r=-0.698\), exact unrestricted permutation \(P=0.0591\); Fig.~\ref{fig:in_vivo_pseudotime_tgi}L). This association is consistent with stronger inhibition at lower realized ploidy, but the eight-mouse sample and limited endpoint-ploidy variation constrain precision. Because the analysis was unadjusted for injected origin and dose, it remains descriptive and does not establish an independent endpoint-ploidy effect.
...
\caption{\scriptsize
\textbf{Tumor growth and growth inhibition, transcriptional states, and endpoint copy-number architecture in gemcitabine-treated SUM-159 xenografts.}
}
...
\caption{\footnotesize
\textbf{Cell-cycle state definition and gemcitabine-associated pseudotime localization.}
...
\textbf{(I)} Mouse-balanced localization of the gemcitabine-associated cell excess along cell-cycle pseudotime. The curve is the equal-mouse treated-minus-vehicle Gaussian-kernel density contrast, and the gray band is its two-sided 95\% simultaneous permutation null envelope. The pale-blue band marks the maximal connected positive region with nominal two-sided exact pointwise permutation support (0.296--0.486; 96 grid points; pointwise \(P=0.00245\)--0.0494 across 4,900 assignments); dashed lines mark these exact boundaries, which directly define the interval modeled in Fig.~\ref{fig:in_vivo_pseudotime_tgi}I. The broad region has no single region-level \(P\) value. The dark-blue band and curve segment mark the narrower region that passed studentized max-\(|T|\) family-wise control (0.414--0.426; global exact injected-origin-stratified permutation \(P=0.0473\)). The circle marks the largest raw density difference at 0.452, and the diamond marks the strongest standardized evidence at 0.420.
}
...
\caption{\footnotesize
\textbf{Mouse-resolved landscape and treatment-group composition of xenograft-derived tumor cells.}
...
\caption{\footnotesize
\textbf{Cluster-level Hallmark over-representation analysis in xenograft-derived tumor cells.}
\textbf{(A)} Cluster-versus-rest Hallmark over-representation analysis (ORA) annotation scores across the nine clusters.
The reviewed matrix retains exact GRCh38-prefixed RNA features before normalization and differential-expression analysis.
}
...
\captionsetup{name=Supplementary Figure}
\centering
\includegraphics[width=\textwidth,height=0.84\textheight,keepaspectratio]{figures/Supplementary/panel_SuppFig9_endpoint_flow_cytometry.pdf}
\caption{\footnotesize
\textbf{Endpoint tumor DNA-content flow cytometry.}
\textbf{(A)} Representative raw-event gating hierarchy for one prespecified 4N-origin tumor, showing the sequential Human Cell Enrichment and HumanCells gates and the workspace DNA gates. Frozen FlowJo and full-precision raw-event replay counts are reported; scatter displays are deterministically subsampled for legibility, whereas gate counts use all events.
\textbf{(B)} Within-mouse-normalized DNA-content distributions for all 16 tumors using identical 1,000-arbitrary-unit bins and a common raw fluorescence window. Each facet reports injected origin, dose, replayed HumanCells count, and the reviewed FlowJo peak annotation. The eight 4N-origin peak annotations span 1.88N--2.20N. The 2N-A1-0 sample was retained and marked despite its low HumanCells count (174 by frozen FlowJo; 173 by replay). Peak-gate names are analyst-supplied workspace annotations rather than newly calibrated or NUMBAT-equivalent ploidy estimates.
}
\label{fig:supp_endpoint_flow}
\end{figure}
\endgroup
...
For each mouse, an empirical cumulative distribution function (ECDF) was evaluated on a common 501-point pseudotime grid. Group curves were arithmetic means of the mouse-specific ECDFs, giving every mouse equal weight. Untreated-versus-treated differences were summarized by the root-mean-square difference between group ECDFs, and exact $P$ values were obtained by enumerating all treatment-label assignments that preserved the observed group sizes, both across all 16 tumors and separately within each injected-origin stratum. For Fig.~\ref{fig:in_vivo_pseudotime_tgi}K, each treated tumor's displacement was the ECDF RMSE from the equal-mouse untreated reference matched by injected ploidy origin. Displacement and Day-17 TGI were each centered within dose, their Pearson correlation was reported, and its exact two-sided $P$ value was obtained from all 576 TGI-label permutations within dose. Day-17 TGI was calculated from change since treatment baseline relative to the mean change of the four untreated tumors matched by injected origin. The dose-adjusted injected-origin contrast was estimated with dose as a categorical covariate and tested by exact injected-origin-label enumeration within dose; the same procedure was used for the Day-24 and Day-31 sensitivity endpoints.

To localize treated-cell excess along pseudotime in Supplementary Fig.~\ref{fig:supp_cellular_landscape}I, a Gaussian kernel-density estimate was calculated separately for each mouse on the 501-point grid from 0 to 1 using the common, treatment-label-invariant bandwidth obtained by applying R \texttt{stats::bw.nrd0} to all 2,881 pseudotime values (bandwidth 0.050647). Vehicle and treated curves were equal-mouse means, and their difference was treated minus vehicle. All \({8\choose4}^{2}=4{,}900\) treatment-label assignments preserving four vehicle and four treated mice within each injected-origin stratum were enumerated. At each grid point, the two-sided exact pointwise permutation $P$ value was the proportion of assignments whose absolute permuted contrast was at least as large as the absolute observed contrast. For simultaneous inference, observed and permuted contrasts were divided by the pointwise permutation standard deviation, the maximum absolute statistic across all 501 positions was recorded for every assignment, and the 95th percentile of that exact max-\(|T|\) distribution defined the family-wise null envelope. The global $P$ value was the proportion of permuted maxima at least as large as the observed maximum. The raw density difference was largest at 0.452, and the maximum absolute studentized statistic occurred at 0.420. The maximal contiguous run for which the density contrast was positive and the nominal pointwise $P$ value was at most 0.05 spanned 0.296--0.486 (96 grid points; pointwise $P=0.00245$--0.0494); no single region-level $P$ value was assigned to this boundary-selected run. The adjacent grid points at 0.294 and 0.488 each had $P=0.0527$. The separately calculated max-\(|T|\) test gave a global exact $P=0.0473$ and simultaneous support only over 0.414--0.426. The unique pointwise-supported run directly defined the state-expression interval; no rounded or static interval was substituted.

\subsubsection{Mouse-aware pseudotime-state expression and pathway analysis}
For Fig.~\ref{fig:in_vivo_pseudotime_tgi}I, exact \texttt{GRCh38-}-prefixed RNA counts from the CellCycle subset were retained before expression filtering, symbol resolution, model fitting, and Homo sapiens pathway analysis; \texttt{GRCm39-}-prefixed and unclassified features were excluded. The expression matrix was required to match all 2,881 cells used in density localization exactly. Pseudotime was divided into 20 equal-width bins, raw counts were summed within each mouse-by-bin combination, and bins containing fewer than five cells were excluded. Genes were retained when CPM exceeded 1 in at least 14 retained sample-bin observations. Pseudobulk libraries were TMM-normalized and analyzed with limma--voom using a five-degree-of-freedom natural spline for pseudotime, categorical gemcitabine dose and injected ploidy origin as nuisance terms, and mouse as the blocking factor in \texttt{duplicateCorrelation}; robust empirical-Bayes moderation was used. No treatment-by-pseudotime interaction was fitted. The contrast was constructed directly from the density-localization result and compared mean fitted expression in [0.296,0.486] with the average of the equally wide left [0.106,0.296) and right (0.486,0.676] neighboring intervals.

Genes were ranked by the moderated $t$ statistic for this contrast and analyzed with \texttt{fgseaMultilevel} against Homo sapiens MSigDB 2026.1.Hs Hallmark, Reactome, and Gene Ontology Biological Process collections (gene-set size 15--500). Unresolved pathways were retried at prespecified \texttt{nPermSimple} budgets of 10,000, 100,000, and 1,000,000; Benjamini--Hochberg adjustment was recomputed across each complete collection. The figure displays up to four positive and four negative pathways per collection among pathways with collection-wide FDR $\leq 0.05$, without backfilling nonsignificant pathways. For visualization, fitted expression was standardized across pseudotime within each leading-edge gene and then averaged across the pathway's leading-edge genes. Enrichment direction describes the selected interval relative to its neighboring states and is not a treatment effect at fixed pseudotime. The gene- and pathway-level inferential statistics are conditional on the data-selected interval boundaries and do not propagate uncertainty from the localization step.

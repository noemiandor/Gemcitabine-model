...
if (length(refresh_drugs) > 0) {
  coxIn <- data.frame(drug = refresh_drugs, drugName = refresh_drugs, stringsAsFactors = FALSE)
  annotated <- tryCatch(
    annotate_from_pubchem(coxIn, cmap_file),
    error = function(e) {
      log_rows$status[log_rows$drug %in% refresh_drugs] <- "failed"
      log_rows$message[log_rows$drug %in% refresh_drugs] <- conditionMessage(e)
      NULL
    }
  )
  ...
  if (!is.null(annotated)) {
    refreshed <- data.frame(
      ...
      annotation_source = "pubchem_refresh_legacy_parser",
      annotation_status = ifelse(is.na(annotated$drugCategory_Pubchem) | annotated$drugCategory_Pubchem == "", "unclassified", "refreshed"),
      retrieved_at = format(Sys.time(), "%Y-%m-%dT%H:%M:%S%z"),
      pubchem_url = NA_character_,
      notes = "Generated by refresh_pubchem_annotations.R using legacy parser",
      stringsAsFactors = FALSE
    )
    log_rows$status[log_rows$drug %in% refreshed$drug] <- refreshed$annotation_status
    log_rows$message[log_rows$drug %in% refreshed$drug] <- "legacy parser completed"
  }
}

...
# Standalone 04h pseudotime/TGI workflow.
# Internal method engines are embedded below so runtime execution depends only
# on this file, Utils.R, the declared input data, and installed R packages.

.ptgi_internal_arg <- function(args, name) {
  exact <- grep(paste0('^--', name, '='), args, value = TRUE)
  if (length(exact) > 0L) return(sub(paste0('^--', name, '='), '', exact[[1L]]))
  flag <- which(args == paste0('--', name))
  if (length(flag) > 0L && flag[[1L]] < length(args)) return(args[[flag[[1L]] + 1L]])
  NULL
}

.ptgi_internal_engine <- .ptgi_internal_arg(commandArgs(trailingOnly = TRUE), 'internal_engine')

if (identical(.ptgi_internal_engine, 'ETP')) {
  local({
...
if (identical(.ptgi_internal_engine, 'initial_ploidy')) {
  local({

    # V5 retains the complete V4 analysis workflow while restoring the V2 grouping
    # design: every grouped, stratified, reference-matched, or interaction analysis
    # uses the original sample-level initial_ploidy label (2N or 4N). Continuous
    # endpoint cell-ploidy summaries remain available as descriptive measurements,
    # but they are never thresholded to define analysis groups.
    ...
    results_base <- arg_or(
      ...
      Sys.getenv(
        "RESULTS_ROOT",
        "/Volumes/Protable Disk/Project/BreastCancerOrthotopicModels/Results/04h_pseudotime_TGI_analysis_v5"
      )
    )
    results_base <- normalizePath(results_base, mustWork = FALSE)
    analysis_version <- arg_or("method_name", "initial_ploidy")
    seed <- as.integer(arg_or("seed", "1"))
    n_perm <- as.integer(arg_or("n_perm", "10000"))
    n_boot <- as.integer(arg_or("n_boot", "1000"))
    n_downsample <- as.integer(arg_or("n_downsample", "200"))
    ...
    dirs <- list(
      CellCycle = file.path(analysis_roots$A, "A1_TreatmentDistributionShift", "CellCycle"),
      NonCellCycle = file.path(analysis_roots$A, "A1_TreatmentDistributionShift", "NonCellCycle"),
      Pseudotime = file.path(analysis_roots$A, "A2_PloidyEffectModification"),
      AUCTGIAssociation = file.path(analysis_roots$B, "B1_AUC_TGI_Association"),
      DoseSpecific = file.path(analysis_roots$B, "B2_DoseSpecific_TGI"),
      PloidyResponse = file.path(analysis_roots$B, "B3_Ploidy_TGI_Response"),
      GrowthCurveMixedModel = file.path(analysis_roots$B, "B4_LongitudinalGrowth_MixedModel"),
      GrowthModel = file.path(analysis_roots$B, "B5_GrowthModel_Sensitivity"),
      Robustness = file.path(analysis_roots$C, "C1_Robustness"),
      Comparison = file.path(analysis_roots$C, "C2_CellCycle_vs_NonCellCycle_Comparison"),
      ...
      Pseudobulk = file.path(analysis_roots$C, "C5_Pseudobulk"),
      NeighborhoodDA = file.path(analysis_roots$C, "C6_NeighborhoodDA"),
      EndpointExploration = file.path(analysis_roots$C, "C7_Ploidy_Exploration"),
      Manuscript = file.path(results_root, "Manuscript"),
      logs = file.path(results_root, "logs")
    )
    invisible(lapply(dirs, ensure_dir))
    ...
    write_result_structure <- function() {
      write_text(c(
        ...
        "- `A1_TreatmentDistributionShift/CellCycle`: CellCycle pseudotime distribution shift, sample-level ECDF tests, shift metrics, and primary CellCycle shift-TGI scatter.",
        "- `A1_TreatmentDistributionShift/NonCellCycle`: the same pseudotime distribution-shift outputs for NonCellCycle cells.",
        "- `A2_PloidyEffectModification`: formal 2N-vs-4N pseudotime treatment-effect modification using Delta ECDF difference-in-differences curves.",
        "",
        "## B_TGI_GrowthResponse",
        "",
        "- `B1_AUC_TGI_Association`: AUC-TGI models asking whether overall tumor-growth inhibition is associated with pseudotime shift and initial ploidy.",
        "- `B2_DoseSpecific_TGI`: dose-specific AUC/endpoint TGI comparisons and within-dose shift-TGI association checks.",
        "- `B3_Ploidy_TGI_Response`: treated-sample AUC/endpoint TGI by initial ploidy plus supporting response-shift summaries.",
        "- `B4_LongitudinalGrowth_MixedModel`: repeated-measures spline/linear mixed-model growth-response analysis.",
        "- `B5_GrowthModel_Sensitivity`: supportive growth-curve summaries and model-based sensitivity outputs.",
        "",
        "## C_SupportingAnalyses",
        ...
        "- `C5_Pseudobulk`: targeted pseudobulk differential-expression support analyses.",
        "- `C6_NeighborhoodDA`: pseudotime-bin fallback neighborhood differential-abundance analyses.",
        "- `C7_Ploidy_Exploration`: dose-colored continuous endpoint-ploidy/TGI and ploidy/pseudotime scatter plots, displayed by initial-ploidy group.",
        "",
        "## Manuscript and Logs",
        "",
        "- `Manuscript`: final report, TeX conclusion, and slide deck.",
    ...
    run_parameters <- list(
      ...
      results_root = results_root,
      analysis_version = analysis_version,
      grouping_variable = "initial_ploidy",
      grouping_levels = paste(initial_ploidy_levels, collapse = ","),
      grouping_source = "original sample metadata (V2 design)",
      seed = seed,
      n_perm = n_perm,
      n_boot = n_boot,
      n_downsample = n_downsample,
    ...
    derive_sample_initial_ploidy_assignments <- function(compartment_cfg) {
      raw_list <- lapply(names(compartment_cfg), function(name) {
        path <- compartment_cfg[[name]]$input
        if (!file.exists(path)) stop("Missing initial-ploidy assignment input: ", path, call. = FALSE)
        raw <- read.csv(path, check.names = FALSE, stringsAsFactors = FALSE)
        needed <- c(
          "cell_id", "sample_id", "initial_ploidy", "gemcitabine_dose",
          "gemcitabine_dose_mg_per_kg", "cell_ploidy"
        ...
        missing <- setdiff(needed, names(raw))
        if (length(missing) > 0) {
          stop("Initial-ploidy assignment input is missing: ", paste(missing, collapse = ", "), call. = FALSE)
        }
        raw <- raw[, needed, drop = FALSE]
        raw$source_compartment <- name
        raw$cell_ploidy <- safe_num(raw$cell_ploidy)
      ...
      union_cells <- do.call(rbind, raw_list)
      union_cells <- union_cells[is.finite(union_cells$cell_ploidy), , drop = FALSE]
      if (nrow(union_cells) == 0) stop("No finite cell_ploidy values are available", call. = FALSE)
      ...
      if (length(duplicated_ids) > 0) {
        for (cell_id in duplicated_ids) {
          local <- union_cells[union_cells$cell_id == cell_id, , drop = FALSE]
          if (length(unique(local$sample_id)) != 1L || length(unique(local$cell_ploidy)) != 1L) {
            stop("Conflicting duplicated cell_id in compartment inputs: ", cell_id, call. = FALSE)
          }
        }
        union_cells <- union_cells[!duplicated(union_cells$cell_id), , drop = FALSE]
      }
      ...
      sample_rows <- lapply(split(union_cells, union_cells$sample_id), function(local) {
        ...
        data.frame(
          sample_id = local$sample_id[[1]],
          initial_ploidy = original_ploidy,
          dose = dose,
          dose_mg = dose_mg,
          n_endpoint_ploidy_cells = nrow(local),
          sample_mean_ploidy = mean(local$cell_ploidy),
      ...
      assignments <- do.call(rbind, sample_rows)
      unexpected_groups <- setdiff(unique(assignments$initial_ploidy), initial_ploidy_levels)
      if (length(unexpected_groups) > 0) {
        stop("Unexpected initial_ploidy labels: ", paste(unexpected_groups, collapse = ", "), call. = FALSE)
      }
      assignments$initial_ploidy <- factor(assignments$initial_ploidy, levels = initial_ploidy_levels)
      assignments$version <- analysis_version
      assignments <- assignments[
        order(assignments$dose_mg, assignments$initial_ploidy, assignments$sample_id),
        ,
        drop = FALSE
      ]
      rownames(assignments) <- NULL
      write_csv(assignments, file.path(results_root, "sample_initial_ploidy_assignments.csv"))

      observed <- aggregate(
        sample_id ~ dose + dose_mg + initial_ploidy,
        assignments,
        function(x) length(unique(x))
      )
      names(observed)[names(observed) == "sample_id"] <- "n_samples"
      full <- expand.grid(
        dose = unique(assignments$dose[order(assignments$dose_mg)]),
        initial_ploidy = initial_ploidy_levels,
        stringsAsFactors = FALSE
      )
      dose_map <- unique(assignments[, c("dose", "dose_mg")])
      full$dose_mg <- dose_map$dose_mg[match(full$dose, dose_map$dose)]
      coverage <- merge(
        full,
        observed,
        by = c("dose", "dose_mg", "initial_ploidy"),
        all.x = TRUE
      )
      coverage$n_samples[is.na(coverage$n_samples)] <- 0L
      coverage$empty_group <- coverage$n_samples == 0L
      coverage$version <- analysis_version
      coverage <- coverage[order(coverage$dose_mg, match(coverage$initial_ploidy, initial_ploidy_levels)), ]
      write_csv(coverage, file.path(results_root, "initial_ploidy_dose_coverage_audit.csv"))
      assignments
    }

    initial_ploidy_assignments <- derive_sample_initial_ploidy_assignments(compartments)
    ...
    ploidy_numeric <- function(x) {
      safe_num(gsub("[^0-9.]+", "", as.character(x)))
    }
    ...
    read_compartment_data <- function(name, cfg) {
      ...
      for (col in numeric_cols) df[[col]] <- safe_num(df[[col]])
      df <- df[is.finite(df$pseudotime), , drop = FALSE]
      assignment_index <- match(df$sample_id, initial_ploidy_assignments$sample_id)
      if (anyNA(assignment_index)) {
        stop("Missing initial-ploidy assignment for samples: ", paste(unique(df$sample_id[is.na(assignment_index)]), collapse = ", "), call. = FALSE)
      }
      expected_initial <- as.character(initial_ploidy_assignments$initial_ploidy[assignment_index])
      if (any(as.character(df$initial_ploidy) != expected_initial)) {
        stop("Initial-ploidy metadata disagree between compartment inputs", call. = FALSE)
      }
      df$initial_ploidy <- factor(expected_initial, levels = initial_ploidy_levels)
      df$sample_mean_endpoint_ploidy <- initial_ploidy_assignments$sample_mean_ploidy[assignment_index]
      df$sample_median_endpoint_ploidy <- initial_ploidy_assignments$sample_median_ploidy[assignment_index]
      df$sample_max_endpoint_ploidy <- initial_ploidy_assignments$sample_max_ploidy[assignment_index]
      df$compartment <- name
      df$dose <- df$gemcitabine_dose
      df$dose_mg <- df$gemcitabine_dose_mg_per_kg
      df
    ...
    build_sample_meta <- function(df) {
      ...
      meta[["dose"]] <- meta[["gemcitabine_dose"]]
      meta[["dose_mg"]] <- safe_num(meta[["gemcitabine_dose_mg_per_kg"]])
      meta[["initial_ploidy_numeric"]] <- ploidy_numeric(meta[["initial_ploidy"]])
      meta <- meta[order(meta[["dose_mg"]], meta[["initial_ploidy_numeric"]], meta[["sample_id"]]), , drop = FALSE]
      rownames(meta) <- meta$sample_id
      meta
    }
    ...
    calculate_shift_metrics <- function(df, sample_meta, reference_type = c("primary_equal_sample_reference", "sensitivity_pooled_cell_reference")) {
      ...
      rows <- lapply(sample_ids, function(sample_id) {
        ...
        sample_dose <- sample_meta[sample_id, "dose_mg"]
        controls <- sample_meta$sample_id[sample_meta$initial_ploidy == sample_ploidy & sample_meta$dose_mg == 0]
        reference_samples <- if (sample_dose == 0) setdiff(controls, sample_id) else controls
        if (length(reference_samples) == 0) {
          delta <- rep(NA_real_, length(grid))
          reference_mean <- NA_real_
      ...
      out <- out[order(out$dose_mg, out$initial_ploidy_numeric, out$sample_id), , drop = FALSE]
      rownames(out) <- NULL
      out
    }
    ...
    ecdf_group_test <- function(df, group_col, group_a, group_b, stratify_by_initial_ploidy = FALSE, n_perm = 10000L) {
      local <- df[df[[group_col]] %in% c(group_a, group_b), , drop = FALSE]
      sample_ids <- sort(unique(local$sample_id))
      if (length(sample_ids) < 3) {
        small_meta <- local[!duplicated(local$sample_id), c("sample_id", group_col), drop = FALSE]
        small_groups <- as.character(small_meta[[group_col]])
        return(data.frame(
          group_a = group_a, group_b = group_b, stratified_by_initial_ploidy = stratify_by_initial_ploidy,
          n_samples = length(sample_ids), n_group_a = sum(small_groups == group_a), n_group_b = sum(small_groups == group_b),
          observed_ecdf_rmse = NA_real_, observed_ecdf_ks = NA_real_, observed_ecdf_mean_abs = NA_real_,
          p_ecdf_rmse = NA_real_, p_ecdf_ks = NA_real_, p_ecdf_mean_abs = NA_real_,
          n_permutations = NA_integer_, permutation_mode = "not_estimable_insufficient_samples", stringsAsFactors = FALSE
      ...
      meta <- local[!duplicated(local$sample_id), c("sample_id", "initial_ploidy", group_col), drop = FALSE]
      meta <- meta[match(sample_ids, meta$sample_id), , drop = FALSE]
      groups <- as.character(meta[[group_col]])
      strata <- if (stratify_by_initial_ploidy) as.character(meta$initial_ploidy) else rep("all", length(groups))
      if (!all(c(group_a, group_b) %in% groups)) {
        return(data.frame(
          group_a = group_a, group_b = group_b, stratified_by_initial_ploidy = stratify_by_initial_ploidy,
          n_samples = length(sample_ids), n_group_a = sum(groups == group_a), n_group_b = sum(groups == group_b),
      ...
      data.frame(
        group_a = group_a,
        group_b = group_b,
        stratified_by_initial_ploidy = stratify_by_initial_ploidy,
        n_samples = length(sample_ids),
        n_group_a = sum(groups == group_a),
        n_group_b = sum(groups == group_b),
        observed_ecdf_rmse = observed[["ecdf_rmse"]],
    ...
    plot_basic_compartment <- function(df, sample_meta, equal_shift, cfg) {
      ...
      dose_levels <- unique(sample_meta$dose[order(sample_meta$dose_mg)])
      df$dose_factor <- factor(df$dose, levels = dose_levels)
      df$initial_ploidy_factor <- factor(df$initial_ploidy, levels = initial_ploidy_levels)
      p_density <- ggplot(df, aes(x = pseudotime, color = dose_factor, fill = dose_factor)) +
        geom_density(alpha = 0.12, linewidth = 0.8) +
        facet_wrap(~ initial_ploidy_factor, ncol = 1) +
        scale_color_manual(values = dose_cols_all[names(dose_cols_all) %in% dose_levels], name = "dose") +
        scale_fill_manual(values = dose_cols_all[names(dose_cols_all) %in% dose_levels], name = "dose") +
        labs(title = paste0(cfg$label, ": cell-level pseudotime density"), x = "pseudotime", y = "density") +
        plot_theme
      ...
      shift_plot <- ggplot(equal_shift, aes(x = dose, y = ecdf_rmse, color = dose)) +
        geom_boxplot(aes(group = dose), outlier.shape = NA, color = "grey55", linewidth = 0.45) +
        geom_point(aes(shape = initial_ploidy), position = position_jitter(width = 0.08, height = 0), size = 2.6) +
        scale_color_manual(values = dose_cols_all[names(dose_cols_all) %in% dose_levels], name = "dose") +
        labs(title = paste0(cfg$label, ": sample-equal ECDF shift by dose"), x = "dose", y = "ECDF RMSE") +
        plot_theme
      save_square_plot(shift_plot, fig_dir, paste0(cfg$prefix, "_ecdf_rmse_by_dose_equal_sample_ref"), 6.4, 6.4)
    ...
    run_compartment <- function(name, cfg, n_perm) {
      ...
      write_csv(equal_shift, file.path(cfg$dir, "tables", paste0(cfg$prefix, "_shift_metrics_equal_sample_ref.csv")))
      write_csv(pooled_shift, file.path(cfg$dir, "tables", paste0(cfg$prefix, "_shift_metrics_pooled_ref_sensitivity.csv")))
      write_csv(dose_tests, file.path(cfg$dir, "stats", "dose_group_ecdf_tests_v5.csv"))
      write_csv(tgi_stats, file.path(cfg$dir, "stats", paste0(cfg$prefix, "_TGI_associations_v5.csv")))
      plot_sample_ecdf(df, sample_meta, cfg)
      plot_tgi_association(equal_shift, cfg)
      plot_basic_compartment(df, sample_meta, equal_shift, cfg)
      list(df = df, sample_meta = sample_meta, equal_shift = equal_shift, pooled_shift = pooled_shift, dose_tests = dose_tests, tgi_stats = tgi_stats)
    ...
    plot_endpoint_continuous_exploration <- function(name, result) {
      ...
      figure_dir <- ensure_dir(file.path(comp_dir, "figures"))
      meta <- result$sample_meta
      meta$initial_ploidy <- factor(meta$initial_ploidy, levels = initial_ploidy_levels)
      meta$dose <- factor(meta$dose, levels = unique(meta$dose[order(meta$dose_mg)]))
      ...
      if (length(tgi_cols) > 0) {
        ...
        p_tgi <- ggplot(
          tgi_long,
          aes(x = endpoint_ploidy_value, y = tgi_value, color = dose, shape = initial_ploidy)
        ) +
          geom_hline(yintercept = 0, color = "grey80", linewidth = 0.3) +
          geom_smooth(
            data = tgi_long,
          ...
          coord_cartesian(clip = "off") +
          labs(
            title = paste0(name, ": sample endpoint-ploidy summaries vs TGI"),
            subtitle = paste0(analysis_version, "; shapes indicate original initial ploidy"),
            x = "endpoint ploidy",
            y = "TGI (%)",
            shape = "initial ploidy"
          ) +
          plot_theme +
          theme(plot.margin = margin(8, 24, 8, 24))
        save_plot(p_tgi, figure_dir, paste0(name, "_endpoint_ploidy_mean_median_max_vs_TGI"), 12, 7.2)
      ...
      p_pt <- ggplot(
        pseudotime_long,
        aes(x = endpoint_ploidy_value, y = pseudotime_value, color = dose, shape = initial_ploidy)
      ) +
        geom_smooth(
          data = pseudotime_long,
          aes(x = endpoint_ploidy_value, y = pseudotime_value, group = 1),
        ...
        coord_cartesian(clip = "off") +
        labs(
          title = paste0(name, ": sample endpoint-ploidy summaries vs pseudotime"),
          subtitle = paste0(analysis_version, "; shapes indicate original initial ploidy"),
          x = "endpoint ploidy",
          y = "sample pseudotime",
          shape = "initial ploidy"
      ...
      cell_df <- result$df
      cell_df$dose <- factor(cell_df$dose, levels = unique(meta$dose))
      cell_df$initial_ploidy <- factor(cell_df$initial_ploidy, levels = initial_ploidy_levels)
      cell_plot_data <- cell_df[, c(
        "cell_id", "sample_id", "initial_ploidy", "dose", "dose_mg",
        "cell_ploidy", "pseudotime"
      )]
      write_csv(cell_plot_data, file.path(table_dir, paste0(name, "_single_cell_ploidy_vs_single_cell_pseudotime_plot_data.csv")))
      p_cell <- ggplot(cell_plot_data, aes(x = cell_ploidy, y = pseudotime, color = dose)) +
        geom_point(size = 0.45, alpha = 0.18) +
        facet_wrap(~ initial_ploidy, ncol = 1, drop = FALSE) +
        scale_color_manual(values = dose_cols_all[names(dose_cols_all) %in% levels(meta$dose)], drop = FALSE, name = "dose") +
        labs(
          title = paste0(name, ": single-cell ploidy vs single-cell pseudotime"),
          subtitle = "Facets indicate original sample initial ploidy",
          x = "single-cell ploidy",
          y = "single-cell pseudotime"
        ) +
        plot_theme
    ...
    plot_sample_level_initial_ploidy_violin <- function(name, result, cfg, n_perm) {
      df <- result$df
      assignment_index <- match(as.character(df$sample_id), as.character(initial_ploidy_assignments$sample_id))
      if (anyNA(assignment_index)) {
        stop("Plot data contain samples without a global initial-ploidy assignment", call. = FALSE)
      }
      expected_group <- as.character(initial_ploidy_assignments$initial_ploidy[assignment_index])
      if (any(as.character(df$initial_ploidy) != expected_group)) {
        stop("Plot data initial-ploidy labels disagree with the global assignment table", call. = FALSE)
      }
      ...
      dose_levels <- as.character(dose_lookup$dose)
      df$dose_plot <- factor(as.character(df$dose), levels = dose_levels)
      df$initial_ploidy <- factor(as.character(df$initial_ploidy), levels = initial_ploidy_levels)

      sample_summary <- do.call(rbind, lapply(split(df, df$sample_id), function(local) {
        data.frame(
          sample_id = as.character(local$sample_id[[1]]),
          initial_ploidy = as.character(local$initial_ploidy[[1]]),
          dose = as.character(local$dose[[1]]),
          dose_mg = safe_num(local$dose_mg[[1]]),
          sample_mean_cell_ploidy = mean(local$cell_ploidy, na.rm = TRUE),
          sample_median_cell_ploidy = median(local$cell_ploidy, na.rm = TRUE),
          sample_mean_endpoint_ploidy = safe_num(local$sample_mean_endpoint_ploidy[[1]]),
          n_cells = nrow(local),
          stringsAsFactors = FALSE
        )
      }))
      sample_summary$initial_ploidy <- factor(sample_summary$initial_ploidy, levels = initial_ploidy_levels)
      sample_summary$dose_plot <- factor(sample_summary$dose, levels = dose_levels)
      sample_summary$version <- analysis_version
      sample_summary$analysis_unit <- "biological_sample"

      table_dir <- ensure_dir(file.path(cfg$dir, "tables"))
      stats_dir <- ensure_dir(file.path(cfg$dir, "stats"))
      figure_dir <- ensure_dir(file.path(cfg$dir, "figures"))
      write_csv(
        sample_summary,
        file.path(table_dir, paste0(cfg$prefix, "_ploidy_violin_dose_initial_ploidy_sample_means.csv"))
      )

      dose_test_rows <- list()
      for (ploidy_group in initial_ploidy_levels) {
        local <- sample_summary[as.character(sample_summary$initial_ploidy) == ploidy_group, , drop = FALSE]
        if (length(unique(local$dose_mg)) < 2L) next
        dose_pairs <- combn(sort(unique(local$dose_mg)), 2L, simplify = FALSE)
        for (pair in dose_pairs) {
          dose_a <- dose_lookup$dose[match(pair[[1]], dose_lookup$dose_mg)]
          dose_b <- dose_lookup$dose[match(pair[[2]], dose_lookup$dose_mg)]
          test <- sample_level_mean_difference_test(
            local,
            "sample_mean_cell_ploidy",
            "dose",
          ...
          dose_test_rows[[length(dose_test_rows) + 1L]] <- data.frame(
            version = analysis_version,
            compartment = name,
            comparison_family = "pairwise_dose_within_initial_ploidy",
            initial_ploidy = ploidy_group,
            dose_a_mg = pair[[1]],
            dose_b_mg = pair[[2]],
            test,
            stringsAsFactors = FALSE
      ...
      write_csv(
        dose_tests,
        file.path(stats_dir, paste0(cfg$prefix, "_ploidy_violin_pairwise_dose_within_initial_ploidy_tests.csv"))
      )

      ploidy_test_rows <- lapply(seq_len(nrow(dose_lookup)), function(i) {
        local <- sample_summary[sample_summary$dose_mg == dose_lookup$dose_mg[[i]], , drop = FALSE]
        test <- sample_level_mean_difference_test(
          local,
          "sample_mean_cell_ploidy",
          "initial_ploidy",
        ...
        data.frame(
          version = analysis_version,
          compartment = name,
          comparison_family = "initial_ploidy_within_dose",
          dose = as.character(dose_lookup$dose[[i]]),
          dose_mg = dose_lookup$dose_mg[[i]],
          test,
          stringsAsFactors = FALSE
        )
      })
      ploidy_tests <- do.call(rbind, ploidy_test_rows)
      write_csv(
        ploidy_tests,
        file.path(stats_dir, paste0(cfg$prefix, "_ploidy_violin_2N_vs_4N_within_dose_tests.csv"))
      )

      group_cols <- c("2N" = "#4C78A8", "4N" = "#E45756")
      p_faceted <- ggplot(df, aes(x = dose_plot, y = cell_ploidy, fill = dose_plot)) +
        geom_violin(scale = "width", trim = TRUE, alpha = 0.55, color = "grey35", linewidth = 0.35) +
        geom_point(
          data = sample_summary,
          aes(x = dose_plot, y = sample_mean_cell_ploidy, shape = initial_ploidy),
          inherit.aes = FALSE,
          position = position_jitter(width = 0.05, height = 0),
          size = 2.5,
          color = "black"
        ) +
        facet_wrap(~ initial_ploidy, nrow = 1, drop = FALSE) +
        scale_fill_manual(values = dose_cols_all[names(dose_cols_all) %in% dose_levels], drop = FALSE, name = "dose") +
        labs(
          title = paste0(cfg$label, ": cell ploidy by dose within initial-ploidy groups"),
          subtitle = "Violins represent cells; points represent biological-sample means",
          x = "gemcitabine dose",
          y = "cell ploidy",
          shape = "initial ploidy"
        ) +
      ...
      save_plot(
        p_faceted,
        figure_dir,
        paste0(cfg$prefix, "_ploidy_violin_dose_by_initial_ploidy"),
        10,
        6.8
      )
      ...
      p_grouped <- ggplot(
        df,
        aes(x = dose_plot, y = cell_ploidy, fill = initial_ploidy, group = interaction(dose_plot, initial_ploidy))
      ) +
        geom_violin(
          scale = "width",
          trim = TRUE,
        ...
        geom_point(
          data = sample_summary,
          aes(x = dose_plot, y = sample_mean_cell_ploidy, color = initial_ploidy),
          inherit.aes = FALSE,
          position = position_jitterdodge(jitter.width = 0.04, dodge.width = 0.82),
          size = 2.5
        ) +
        scale_fill_manual(values = group_cols, limits = initial_ploidy_levels, drop = FALSE, name = "initial ploidy") +
        scale_color_manual(values = group_cols, limits = initial_ploidy_levels, drop = FALSE, guide = "none") +
        labs(
          title = paste0(cfg$label, ": 2N versus 4N cell ploidy within dose"),
          subtitle = "Groups are defined by original sample initial ploidy; points are biological-sample means",
          x = "gemcitabine dose",
          y = "cell ploidy"
        ) +
        plot_theme
      save_plot(
        p_grouped,
        figure_dir,
        paste0(cfg$prefix, "_ploidy_violin_2N_vs_4N_within_dose"),
        8.8,
        6.8
      )

      invisible(list(sample_summary = sample_summary, dose_tests = dose_tests, ploidy_tests = ploidy_tests))
    }
    ...
    plot_direct_group_ecdf_comparisons <- function(name, result, cfg, n_perm) {
      ...
      add_curves(
        df,
        "2. 0 vs treated, initial-ploidy-stratified",
        paste(df$treatment_group, df$initial_ploidy, sep = " | "),
        df$treatment_group,
        as.character(df$initial_ploidy)
      )
      dose_specs <- list(
        list(panel = "3. 0 vs 30 mg/kg", doses = c(0, 30)),
        list(panel = "4. 0 vs 120 mg/kg", doses = c(0, 120)),
      ...
      add_curves(
        treated,
        "6. Treated: 2N vs 4N",
        as.character(treated$initial_ploidy),
        "treated",
        as.character(treated$initial_ploidy)
      )
      control <- df[df$dose_mg == 0, , drop = FALSE]
      add_curves(
        control,
        "7. Control: 2N vs 4N",
        as.character(control$initial_ploidy),
        "0mg/kg",
        as.character(control$initial_ploidy)
      )
      high <- df[df$initial_ploidy == "4N", , drop = FALSE]
      add_curves(high, "8. 4N: 0 vs treated", high$treatment_group, high$treatment_group, "4N")
      low <- df[df$initial_ploidy == "2N", , drop = FALSE]
      add_curves(low, "9. 2N: 0 vs treated", low$treatment_group, low$treatment_group, "2N")
      curves <- do.call(rbind, curve_rows)
      ...
      get_overall_test <- function(comparison, stratified_flag = FALSE) {
        hit <- overall_tests[
          overall_tests$comparison == comparison &
            overall_tests$stratified_by_initial_ploidy == stratified_flag,
          ,
          drop = FALSE
        ]
        if (nrow(hit) == 0) return(data.frame())
      ...
      ploidy_between <- function(local) {
        ecdf_group_test(local, "initial_ploidy", "2N", "4N", FALSE, n_perm)
      }
      treatment_within_ploidy <- function(group) {
        local <- df[df$initial_ploidy == group, , drop = FALSE]
        ecdf_group_test(local, "treatment_group", "0mg/kg", "treated", FALSE, n_perm)
      }
      test_rows <- list(
        cbind(panel = "1. 0 vs treated", test_design = "sample_label_permutation", get_overall_test("0_vs_30plus120", FALSE)),
        cbind(panel = "2. 0 vs treated, initial-ploidy-stratified", test_design = "treatment_label_permutation_within_initial_ploidy", get_overall_test("0_vs_30plus120", TRUE)),
        cbind(panel = "3. 0 vs 30 mg/kg", test_design = "sample_label_permutation", get_overall_test("0_vs_30", FALSE)),
        cbind(panel = "4. 0 vs 120 mg/kg", test_design = "sample_label_permutation", get_overall_test("0_vs_120", FALSE)),
        cbind(panel = "5. 30 vs 120 mg/kg", test_design = "sample_label_permutation", get_overall_test("30_vs_120", FALSE)),
        cbind(panel = "6. Treated: 2N vs 4N", test_design = "unpaired_sample_initial_ploidy_label_permutation", ploidy_between(treated)),
        cbind(panel = "7. Control: 2N vs 4N", test_design = "unpaired_sample_initial_ploidy_label_permutation", ploidy_between(control)),
        cbind(panel = "8. 4N: 0 vs treated", test_design = "sample_treatment_label_permutation_within_initial_ploidy", treatment_within_ploidy("4N")),
        cbind(panel = "9. 2N: 0 vs treated", test_design = "sample_treatment_label_permutation_within_initial_ploidy", treatment_within_ploidy("2N"))
      )
      all_test_columns <- unique(unlist(lapply(test_rows, names), use.names = FALSE))
      test_rows <- lapply(test_rows, function(x) {
        missing <- setdiff(all_test_columns, names(x))
      ...
      panel_levels <- paste0(seq_len(9), ". ", c(
        "0 vs treated", "0 vs treated, initial-ploidy-stratified", "0 vs 30 mg/kg", "0 vs 120 mg/kg", "30 vs 120 mg/kg",
        "Treated: 2N vs 4N", "Control: 2N vs 4N",
        "4N: 0 vs treated", "2N: 0 vs treated"
      ))
      curves$panel <- factor(curves$panel, levels = panel_levels)
      tests$panel <- factor(tests$panel, levels = panel_levels)
      direct_colors <- c("0mg/kg" = "#666666", "treated" = "#1b9e77", "30mg/kg" = "#d95f02", "120mg/kg" = "#7570b3")
      p <- ggplot(curves, aes(x = pseudotime, y = mean_ecdf, color = color_group, linetype = line_group, group = curve_label)) +
        ...
        facet_wrap(~ panel, ncol = 3, drop = FALSE) +
        scale_color_manual(values = direct_colors, name = "group") +
        scale_linetype_manual(values = c("All" = "solid", "2N" = "solid", "4N" = "22"), name = "Initial ploidy") +
        coord_cartesian(xlim = c(0, 1), ylim = c(0, 1.05), clip = "off") +
        labs(
          title = paste0(name, ": direct group mean ECDF comparisons (", analysis_version, ")"),
          subtitle = "Equal-sample mean ECDFs; groups use original sample initial ploidy",
          x = "pseudotime",
          y = "mean ECDF"
        ) +
        plot_theme +
    ...
    downsample_sensitivity <- function(name, df, sample_meta, n_iter = 200L, n_perm_iter = 1000L) {
      ...
      iter_rows <- lapply(seq_len(n_iter), function(i) {
        ...
        pearson <- permutation_cor_p(treated$ecdf_rmse, treated$TGI_percent_auc, "pearson", n_perm_iter, exact = FALSE)
        spearman <- permutation_cor_p(treated$ecdf_rmse, treated$TGI_percent_auc, "spearman", n_perm_iter, exact = FALSE)
        combined <- dt[dt$comparison == "0_vs_30plus120" & !dt$stratified_by_initial_ploidy, , drop = FALSE]
        data.frame(
          iteration = i,
          target_cells_per_sample = target_n,
          n_samples_retained = length(retained),
    ...
    run_confounding <- function(results) {
      ...
      for (name in names(results)) {
        ...
        for (sample_set in c("all", "treated")) {
          local <- if (sample_set == "treated") sh[sh$dose_mg > 0, , drop = FALSE] else sh
          for (metric in c("ecdf_rmse", "signed_mean_shift")) {
            for (ploidy_col in c("mean_cell_ploidy", "median_cell_ploidy", "p90_cell_ploidy", "initial_ploidy_numeric")) {
              for (method in c("pearson", "spearman")) {
                cr <- safe_cor_result(local[[metric]], local[[ploidy_col]], method)
                ploidy_rows[[length(ploidy_rows) + 1L]] <- data.frame(
                  compartment = name, sample_set = sample_set, shift_metric = metric, ploidy_measure = ploidy_col,
        ...
        if (nrow(treated) >= 6) {
          local <- treated[is.finite(treated$ecdf_rmse) & is.finite(treated$TGI_percent_auc), , drop = FALSE]
          if (nrow(local) >= 6 && length(unique(local$dose_mg)) > 1) {
            shift_resid <- resid(lm(ecdf_rmse ~ factor(dose_mg) + initial_ploidy_numeric, data = local))
            tgi_resid <- resid(lm(TGI_percent_auc ~ factor(dose_mg) + initial_ploidy_numeric, data = local))
            cr <- safe_cor_result(shift_resid, tgi_resid, "pearson")
            resid_rows[[length(resid_rows) + 1L]] <- data.frame(compartment = name, analysis = "residualized_against_dose_and_initial_ploidy", method = "pearson", n = cr$n, estimate = cr$estimate, p_value = cr$p_value, stringsAsFactors = FALSE)
          }
          centered <- local
          centered$shift_centered <- centered$ecdf_rmse - ave(centered$ecdf_rmse, centered$dose_mg, FUN = function(x) mean(x, na.rm = TRUE))
          centered$tgi_centered <- centered$TGI_percent_auc - ave(centered$TGI_percent_auc, centered$dose_mg, FUN = function(x) mean(x, na.rm = TRUE))
    ...
    make_endpoint_tgi_long <- function(treated, endpoint_cols) {
      ...
      do.call(rbind, lapply(endpoint_cols, function(col) {
        data.frame(
          sample_id = treated$sample_id,
          initial_ploidy = treated$initial_ploidy,
          dose = treated$dose,
          dose_mg = treated$dose_mg,
          day = endpoint_day_num(col),
          tgi_measure = col,
    ...
    plot_dose_specific_tgi <- function(name, treated, comp_dir, tgi_comparisons) {
      ...
      p_auc <- ggplot(treated, aes(x = dose, y = TGI_percent_auc, color = dose)) +
        geom_hline(yintercept = 0, color = "grey75", linewidth = 0.35) +
        geom_boxplot(aes(group = dose), outlier.shape = NA, color = "grey55", linewidth = 0.45) +
        geom_point(aes(shape = initial_ploidy), position = position_jitter(width = 0.08, height = 0), size = 2.8) +
        geom_text(aes(label = sample_id), size = 2.35, nudge_y = 2.5, check_overlap = TRUE, show.legend = FALSE) +
        scale_color_manual(values = dose_cols_all[names(dose_cols_all) %in% dose_levels], name = "dose") +
        labs(title = paste0(comp_prefix, ": AUC TGI by gemcitabine dose"), x = "dose", y = "AUC-based TGI (%)") +
        plot_theme
      ...
      if (nrow(endpoint_long) > 0) {
        p_endpoint <- ggplot(endpoint_long, aes(x = day, y = TGI_percent_endpoint, color = dose, group = sample_id)) +
          geom_hline(yintercept = 0, color = "grey75", linewidth = 0.35) +
          geom_line(alpha = 0.6, linewidth = 0.55) +
          geom_point(aes(shape = initial_ploidy), size = 2.1) +
          scale_color_manual(values = dose_cols_all[names(dose_cols_all) %in% dose_levels], name = "dose") +
          labs(title = paste0(comp_prefix, ": endpoint TGI across all measured days"), x = "day", y = "endpoint TGI (%)") +
          plot_theme
        save_plot(p_endpoint, fig_dir, paste0(comp_prefix, "_endpoint_TGI_by_treated_dose_over_days"), 7.4, 4.8)
    ...
    run_auc_tgi_association_analysis <- function(results) {
      ...
      for (name in names(results)) {
        ...
        treated <- results[[name]]$equal_shift[results[[name]]$equal_shift$dose_mg > 0, , drop = FALSE]
        keep <- c(
          "sample_id", "initial_ploidy", "initial_ploidy_numeric", "dose", "dose_mg",
          "sample_mean_endpoint_ploidy",
          "ecdf_rmse", "ecdf_ks", "ecdf_mean_abs", "signed_mean_shift",
          "TGI_percent_auc", endpoint_tgi_cols(names(treated))
        )
        ...
        treated <- treated[, keep, drop = FALSE]
        treated$compartment <- name
        treated$initial_ploidy <- factor(treated$initial_ploidy, levels = unique(treated$initial_ploidy[order(treated$initial_ploidy_numeric)]))
        treated$dose <- factor(treated$dose, levels = unique(treated$dose[order(treated$dose_mg)]))
        treated$dose_mg_factor <- factor(treated$dose_mg, levels = sort(unique(treated$dose_mg)))
        write_csv(treated, file.path(comp_dir, "tables", paste0(name, "_treated_AUC_TGI_shift_ploidy_sample_table.csv")))
        all_samples[[length(all_samples) + 1L]] <- treated
        ...
        fits <- list(
          shift_only = try(lm(TGI_percent_auc ~ scale(ecdf_rmse), data = local), silent = TRUE),
          shift_ploidy_dose_adjusted = try(lm(TGI_percent_auc ~ scale(ecdf_rmse) + initial_ploidy + dose_mg_factor, data = local), silent = TRUE),
          shift_by_ploidy_dose_adjusted = try(lm(TGI_percent_auc ~ scale(ecdf_rmse) * initial_ploidy + dose_mg_factor, data = local), silent = TRUE)
        )
        labels <- c(
          shift_only = "TGI_percent_auc ~ scale(ecdf_rmse)",
          shift_ploidy_dose_adjusted = "TGI_percent_auc ~ scale(ecdf_rmse) + initial_ploidy + factor(dose_mg)",
          shift_by_ploidy_dose_adjusted = "TGI_percent_auc ~ scale(ecdf_rmse) * initial_ploidy + factor(dose_mg)"
        )
        for (analysis in names(fits)) {
          fit <- fits[[analysis]]
          if (!inherits(fit, "try-error") && !is.null(fit)) {
        ...
        mean_etp_fits <- list(
          mean_etp_only = try(lm(TGI_percent_auc ~ scale(sample_mean_endpoint_ploidy), data = local_mean_etp), silent = TRUE),
          mean_etp_ploidy_dose_adjusted = try(lm(TGI_percent_auc ~ scale(sample_mean_endpoint_ploidy) + initial_ploidy + dose_mg_factor, data = local_mean_etp), silent = TRUE),
          mean_etp_by_ploidy_dose_adjusted = try(lm(TGI_percent_auc ~ scale(sample_mean_endpoint_ploidy) * initial_ploidy + dose_mg_factor, data = local_mean_etp), silent = TRUE)
        )
        mean_etp_labels <- c(
          mean_etp_only = "TGI_percent_auc ~ scale(sample_mean_endpoint_ploidy)",
          mean_etp_ploidy_dose_adjusted = "TGI_percent_auc ~ scale(sample_mean_endpoint_ploidy) + initial_ploidy + factor(dose_mg)",
          mean_etp_by_ploidy_dose_adjusted = "TGI_percent_auc ~ scale(sample_mean_endpoint_ploidy) * initial_ploidy + factor(dose_mg)"
        )
        for (analysis in names(mean_etp_fits)) {
          fit <- mean_etp_fits[[analysis]]
          if (!inherits(fit, "try-error") && !is.null(fit)) {
        ...
        if (nrow(local) > 0) {
          p <- ggplot(local, aes(x = ecdf_rmse, y = TGI_percent_auc, color = initial_ploidy, shape = dose)) +
            geom_hline(yintercept = 0, color = "grey75", linewidth = 0.35) +
            geom_smooth(aes(group = 1), method = "lm", se = TRUE, color = "black", linewidth = 0.55) +
            geom_point(size = 3) +
            geom_text(aes(label = sample_id), size = 2.4, nudge_y = 2.5, check_overlap = TRUE, show.legend = FALSE) +
        ...
        summary_lines <- c(
          "# AUC-TGI Association Summary",
          "",
          "This module addresses Question 1: whether overall tumor-growth inhibition is associated with pseudotime shift and Initial ploidy.",
          "",
          "The inference unit is the biological sample. The primary outcome is `TGI_percent_auc`, and the primary pseudotime-shift metric is sample-equal ploidy-matched `ecdf_rmse`.",
          "",
          "Primary models:",
          "",
          "- `TGI_percent_auc ~ scale(ecdf_rmse) + initial_ploidy + factor(dose_mg)`",
          "- `TGI_percent_auc ~ scale(ecdf_rmse) * initial_ploidy + factor(dose_mg)`",
          "",
          "The first model asks whether pseudotime shift and ploidy explain overall AUC-TGI after accounting for treated dose. The second model asks whether the shift-TGI relationship differs by Initial ploidy.",
          "",
          "Parallel exploratory mean-ETP models:",
          "",
          "- `TGI_percent_auc ~ scale(sample_mean_endpoint_ploidy)`",
          "- `TGI_percent_auc ~ scale(sample_mean_endpoint_ploidy) + initial_ploidy + factor(dose_mg)`",
          "- `TGI_percent_auc ~ scale(sample_mean_endpoint_ploidy) * initial_ploidy + factor(dose_mg)`",
          "",
          paste0("Mean-ETP model terms: `stats/", name, "_AUC_TGI_mean_ETP_ploidy_dose_models.csv`."),
          paste0("Mean-ETP model summaries: `stats/", name, "_AUC_TGI_mean_ETP_ploidy_dose_model_summaries.csv`.")
    ...
    ploidy_label_permutations_within_treatment <- function(sample_meta, group_a, group_b, n_perm = 10000L) {
      treatment_group <- ifelse(sample_meta$dose_mg == 0, "control", "treated")
      choices_by_group <- lapply(split(seq_len(nrow(sample_meta)), treatment_group), function(idx) {
        n_a <- sum(as.character(sample_meta$initial_ploidy[idx]) == group_a)
        if (n_a == 0) return(list(integer(0)))
        if (n_a == length(idx)) return(list(idx))
        combn(idx, n_a, simplify = FALSE)
      })
      ...
      if (is.finite(n_exact) && n_exact <= n_perm) {
        ...
        split_idx <- split(seq_len(nrow(sample_meta)), treatment_group)
        observed_n_a <- vapply(split_idx, function(idx) sum(as.character(sample_meta$initial_ploidy[idx]) == group_a), integer(1))
        out <- replicate(n_perm, {
          labels <- rep(group_b, nrow(sample_meta))
          for (nm in names(split_idx)) {
            idx <- split_idx[[nm]]
    ...
    bootstrap_pseudotime_did <- function(ecdf_mat, sample_meta, group_a, group_b, n_boot = 1000L) {
      ...
      treatment_group <- ifelse(sample_meta$dose_mg == 0, "control", "treated")
      strata <- list(
        control_a = sample_ids[treatment_group == "control" & as.character(sample_meta$initial_ploidy) == group_a],
        treated_a = sample_ids[treatment_group == "treated" & as.character(sample_meta$initial_ploidy) == group_a],
        control_b = sample_ids[treatment_group == "control" & as.character(sample_meta$initial_ploidy) == group_b],
        treated_b = sample_ids[treatment_group == "treated" & as.character(sample_meta$initial_ploidy) == group_b]
      )
      if (any(vapply(strata, length, integer(1)) < 2)) return(data.frame())
      boot <- replicate(n_boot, {
        ids <- lapply(strata, function(x) sample(x, length(x), replace = TRUE))
    ...
    run_pseudotime_ploidy_effect_modification <- function(results, n_perm = 10000L, n_boot = 1000L) {
      ...
      for (name in names(results)) {
        df <- results[[name]]$df
        sample_meta <- results[[name]]$sample_meta
        ploidies <- initial_ploidy_levels[initial_ploidy_levels %in% as.character(sample_meta$initial_ploidy)]
        if (length(ploidies) < 2) next
        group_a <- ploidies[1]
        group_b <- ploidies[2]
        keep_samples <- rownames(sample_meta)[as.character(sample_meta$initial_ploidy) %in% c(group_a, group_b)]
        local_meta <- sample_meta[keep_samples, , drop = FALSE]
        local_df <- df[df$sample_id %in% keep_samples, , drop = FALSE]
        grid <- make_grid(local_df)
        ecdf_mat <- make_sample_ecdf_matrix(local_df, rownames(local_meta), grid)
        treatment_group <- ifelse(local_meta$dose_mg == 0, "control", "treated")
        original_ids <- list(
          control_a = rownames(local_meta)[treatment_group == "control" & as.character(local_meta$initial_ploidy) == group_a],
          treated_a = rownames(local_meta)[treatment_group == "treated" & as.character(local_meta$initial_ploidy) == group_a],
          control_b = rownames(local_meta)[treatment_group == "control" & as.character(local_meta$initial_ploidy) == group_b],
          treated_b = rownames(local_meta)[treatment_group == "treated" & as.character(local_meta$initial_ploidy) == group_b]
        )
        delta_a <- pseudotime_delta_from_ids(ecdf_mat, local_meta, original_ids$treated_a, original_ids$control_a)
        delta_b <- pseudotime_delta_from_ids(ecdf_mat, local_meta, original_ids$treated_b, original_ids$control_b)
        did_curve <- delta_b$delta - delta_a$delta
        ...
        p <- ggplot(curve_long, aes(x = pseudotime, y = value, color = group)) +
          ...
          facet_wrap(~ curve_type, ncol = 1, scales = "free_y") +
          labs(
            title = paste0(name, ": pseudotime treatment effect by Initial ploidy"),
            x = "pseudotime",
            y = "ECDF difference",
            color = "curve"
          ) +
          plot_theme
        save_square_plot(p, file.path(out_dir, "figures"), paste0(name, "_delta_ECDF_2N_vs_4N_effect_modification"), 7.2, 9.4)
      }
      ...
      if (nrow(curves) > 0) write_csv(curves, file.path(out_dir, "tables", "pseudotime_delta_ecdf_curves_by_ploidy.csv"))
      summary_lines <- c(
        "# Pseudotime Initial-Ploidy Effect-Modification Summary",
        "",
        "This module addresses pseudotime question A2: whether the gemcitabine-induced pseudotime distribution shift differs between 2N and 4N sample groups.",
        "",
        "The estimand is a difference-in-differences ECDF curve:",
        "",
        "- `Delta ECDF_2N(t) = mean ECDF(treated 2N) - mean ECDF(control 2N)`",
        "- `Delta ECDF_4N(t) = mean ECDF(treated 4N) - mean ECDF(control 4N)`",
        "- `DID(t) = Delta ECDF_4N(t) - Delta ECDF_2N(t)`",
        "",
        "Permutation p values shuffle 2N/4N labels within treatment groups, preserving the observed number of both initial-ploidy groups among control and treated samples. Bootstrap CIs resample samples within each treatment-by-initial-ploidy stratum.",
        "",
        "Primary outputs:",
        "",
        "- `stats/pseudotime_ploidy_effect_modification_tests.csv`: DID ECDF RMSE/KS/mean-absolute/signed-mean statistics, permutation p values, and bootstrap CIs.",
        "- `tables/pseudotime_delta_ecdf_curves_by_ploidy.csv`: Delta ECDF curves by initial-ploidy group plus the 4N-minus-2N DID curve.",
        "- `figures/*_delta_ECDF_2N_vs_4N_effect_modification.*`: treatment-effect and DID ECDF curves."
      )
      write_text(summary_lines, file.path(out_dir, "pseudotime_ploidy_effect_modification_summary.md"))
      list(tests = tests, curves = curves)
    }
    ...
    run_ploidy_response_analysis <- function(results, n_perm = 10000L, n_boot = 1000L) {
      ...
      for (name in names(results)) {
        ...
        sh <- results[[name]]$equal_shift
        df$treatment_group <- ifelse(df$dose_mg == 0, "0mg/kg", "treated")
        ploidies <- initial_ploidy_levels[initial_ploidy_levels %in% as.character(sh$initial_ploidy)]

        ecdf_rows <- list()
        for (ploidy in ploidies) {
          local <- df[as.character(df$initial_ploidy) == ploidy, , drop = FALSE]
          tst <- ecdf_group_test(local, "treatment_group", "0mg/kg", "treated", FALSE, n_perm)
          ecdf_rows[[length(ecdf_rows) + 1L]] <- cbind(compartment = name, initial_ploidy = ploidy, comparison = "0_vs_treated_within_ploidy", tst)
        }
        ecdf_tests <- if (length(ecdf_rows) > 0) do.call(rbind, ecdf_rows) else data.frame()
        if (nrow(ecdf_tests) > 0) write_csv(ecdf_tests, file.path(comp_dir, "stats", paste0(name, "_ploidy_stratified_treatment_ecdf_tests.csv")))
        all_ecdf[[length(all_ecdf) + 1L]] <- ecdf_tests
        ...
        if (length(ploidies) >= 2 && nrow(treated) > 0) {
          ...
          group_b <- ploidies[2]
          for (metric in c("ecdf_rmse", "ecdf_ks", "ecdf_mean_abs", "signed_mean_shift")) {
            a_sum <- dose_value_summary(treated[[metric]], treated$initial_ploidy, group_a)
            b_sum <- dose_value_summary(treated[[metric]], treated$initial_ploidy, group_b)
            wt <- safe_wilcox(treated[[metric]], treated$initial_ploidy)
            tt <- safe_ttest(treated[[metric]], treated$initial_ploidy)
            perm <- permutation_mean_diff_p(treated[[metric]], treated$initial_ploidy, group_a, group_b, n_perm, exact = TRUE)
            margin <- if (metric == "ecdf_rmse") 0.01 else NA_real_
            eq <- bootstrap_mean_diff_ci(treated[[metric]], treated$initial_ploidy, group_a, group_b, n_boot, margin)
            shift_rows[[length(shift_rows) + 1L]] <- data.frame(
              compartment = name,
              sample_set = "treated",
              comparison = paste0(group_a, "_vs_", group_b),
        ...
        plot_data <- treated
        if (nrow(plot_data) > 0) {
          p_shift <- ggplot(plot_data, aes(x = initial_ploidy, y = ecdf_rmse, color = initial_ploidy)) +
            geom_boxplot(aes(group = initial_ploidy), outlier.shape = NA, color = "grey55", linewidth = 0.45) +
            geom_point(aes(shape = dose), position = position_jitter(width = 0.08, height = 0), size = 2.8) +
            labs(title = paste0(name, ": treated ECDF RMSE by Initial ploidy"), x = "Initial ploidy", y = "ECDF RMSE") +
            plot_theme +
            theme(legend.position = "right")
          save_square_plot(p_shift, file.path(comp_dir, "figures"), paste0(name, "_treated_ecdf_rmse_by_initial_ploidy"), 6.6, 6.6)
        }

        model_rows <- list()
        local_all <- sh[is.finite(sh$ecdf_rmse), , drop = FALSE]
        if (nrow(local_all) >= 6 && length(unique(local_all$initial_ploidy)) > 1 && length(unique(local_all$dose_mg > 0)) > 1) {
          local_all$treatment_group <- ifelse(local_all$dose_mg == 0, "control", "treated")
          fit <- try(lm(ecdf_rmse ~ treatment_group * initial_ploidy, data = local_all), silent = TRUE)
          model_rows[[length(model_rows) + 1L]] <- lm_terms_table(fit, name, "shift_treatment_by_ploidy_interaction", "ecdf_rmse ~ treatment_group * initial_ploidy", nrow(local_all))
        }
        local_treated <- treated[is.finite(treated$ecdf_rmse), , drop = FALSE]
        if (nrow(local_treated) >= 6 && length(unique(local_treated$initial_ploidy)) > 1) {
          fit <- try(lm(ecdf_rmse ~ factor(dose_mg) + initial_ploidy, data = local_treated), silent = TRUE)
          model_rows[[length(model_rows) + 1L]] <- lm_terms_table(fit, name, "treated_shift_by_dose_and_ploidy", "ecdf_rmse ~ factor(dose_mg) + initial_ploidy", nrow(local_treated))
        }
        local_tgi <- treated[is.finite(treated$TGI_percent_auc), , drop = FALSE]
        if (nrow(local_tgi) >= 6 && length(unique(local_tgi$initial_ploidy)) > 1) {
          fit <- try(lm(TGI_percent_auc ~ factor(dose_mg) + initial_ploidy, data = local_tgi), silent = TRUE)
          model_rows[[length(model_rows) + 1L]] <- lm_terms_table(fit, name, "treated_TGI_by_dose_and_ploidy", "TGI_percent_auc ~ factor(dose_mg) + initial_ploidy", nrow(local_tgi))
          fit <- try(lm(TGI_percent_auc ~ scale(ecdf_rmse) * initial_ploidy + factor(dose_mg), data = local_tgi), silent = TRUE)
          model_rows[[length(model_rows) + 1L]] <- lm_terms_table(fit, name, "treated_TGI_shift_by_ploidy_interaction", "TGI_percent_auc ~ scale(ecdf_rmse) * initial_ploidy + factor(dose_mg)", nrow(local_tgi))
        }
        models <- if (length(model_rows) > 0) do.call(rbind, model_rows) else data.frame()
        if (nrow(models) > 0) {
          write_csv(models, file.path(comp_dir, "stats", paste0(name, "_ploidy_response_interaction_models.csv")))
      ...
      ensure_dir(file.path(tgi_dir, "figures"))
      treated_samples <- results$CellCycle$equal_shift[results$CellCycle$equal_shift$dose_mg > 0, , drop = FALSE]
      ploidies <- initial_ploidy_levels[initial_ploidy_levels %in% as.character(treated_samples$initial_ploidy)]
      tgi_cols <- unique(c("TGI_percent_auc", endpoint_tgi_cols(names(treated_samples))))
      tgi_rows <- list()
      if (length(ploidies) >= 2) {
        group_a <- ploidies[1]
        group_b <- ploidies[2]
        for (tgi_col in tgi_cols) {
          a_sum <- dose_value_summary(treated_samples[[tgi_col]], treated_samples$initial_ploidy, group_a)
          b_sum <- dose_value_summary(treated_samples[[tgi_col]], treated_samples$initial_ploidy, group_b)
          wt <- safe_wilcox(treated_samples[[tgi_col]], treated_samples$initial_ploidy)
          tt <- safe_ttest(treated_samples[[tgi_col]], treated_samples$initial_ploidy)
          perm <- permutation_mean_diff_p(treated_samples[[tgi_col]], treated_samples$initial_ploidy, group_a, group_b, n_perm, exact = TRUE)
          margin <- if (tgi_col == "TGI_percent_auc") 10 else NA_real_
          eq <- bootstrap_mean_diff_ci(treated_samples[[tgi_col]], treated_samples$initial_ploidy, group_a, group_b, n_boot, margin)
          tgi_rows[[length(tgi_rows) + 1L]] <- data.frame(
            comparison = paste0(group_a, "_vs_", group_b),
            group_a = group_a,
            group_b = group_b,
      ...
      if (nrow(endpoint_long) > 0) {
        write_csv(endpoint_long, file.path(tgi_dir, "stats", "treated_endpoint_TGI_by_ploidy_long.csv"))
        p_endpoint <- ggplot(endpoint_long, aes(x = day, y = TGI_percent_endpoint, color = initial_ploidy, group = sample_id)) +
          geom_hline(yintercept = 0, color = "grey75", linewidth = 0.35) +
          geom_line(alpha = 0.6, linewidth = 0.55) +
          geom_point(aes(shape = dose), size = 2.1) +
          labs(title = "Endpoint TGI over days by Initial ploidy", x = "day", y = "endpoint TGI (%)") +
          plot_theme
        save_plot(p_endpoint, file.path(tgi_dir, "figures"), "treated_endpoint_TGI_by_initial_ploidy_over_days", 7.4, 4.8)
      }
      if (nrow(treated_samples) > 0) {
        p_auc <- ggplot(treated_samples, aes(x = initial_ploidy, y = TGI_percent_auc, color = initial_ploidy)) +
          geom_hline(yintercept = 0, color = "grey75", linewidth = 0.35) +
          geom_boxplot(aes(group = initial_ploidy), outlier.shape = NA, color = "grey55", linewidth = 0.45) +
          geom_point(aes(shape = dose), position = position_jitter(width = 0.08, height = 0), size = 2.8) +
          geom_text(aes(label = sample_id), size = 2.35, nudge_y = 2.5, check_overlap = TRUE, show.legend = FALSE) +
          labs(title = "AUC TGI by Initial ploidy", x = "Initial ploidy", y = "AUC-based TGI (%)") +
          plot_theme
        save_square_plot(p_auc, file.path(tgi_dir, "figures"), "treated_TGI_AUC_by_initial_ploidy", 6.6, 6.6)
      }
      ...
      summary_lines <- c(
        "# Initial-Ploidy Response Summary",
        "",
        "This module treats Initial ploidy as a potential response modifier rather than only as a confounder.",
        "",
        "Difference tests ask whether 2N and 4N responses are detectably different. Equivalence-style rows use pre-specified descriptive margins: ECDF RMSE +/-0.01 and AUC TGI +/-10 percentage points. If the bootstrap CI is not contained inside the margin, equivalence is not established.",
        "",
        "Primary outputs:",
        "",
        "- `all_compartments_ploidy_stratified_treatment_ecdf_tests.csv`: 0 vs treated ECDF tests within each ploidy and compartment.",
        "- `all_compartments_treated_shift_by_ploidy_tests.csv`: treated-sample shift differences between 2N and 4N.",
        "- `TGI/stats/treated_TGI_auc_endpoint_by_ploidy_tests.csv`: treated-sample AUC and endpoint TGI differences between 2N and 4N.",
        "- `all_compartments_ploidy_response_interaction_models.csv`: ploidy interaction and adjusted sensitivity models."
      )
      write_text(summary_lines, file.path(dirs$PloidyResponse, "ploidy_response_summary.md"))
    ...
    compartment_comparison <- function(results) {
      cc <- results$CellCycle$equal_shift
      nc <- results$NonCellCycle$equal_shift
      keep <- c("sample_id", "initial_ploidy", "dose", "dose_mg", "TGI_percent_auc", "ecdf_rmse", "ecdf_ks", "ecdf_mean_abs", "signed_mean_shift", "cell_count")
      comp <- merge(cc[, keep], nc[, keep], by = "sample_id", suffixes = c("_CellCycle", "_NonCellCycle"))
      comp$dose <- comp$dose_CellCycle
      comp$dose_mg <- comp$dose_mg_CellCycle
      ...
      paired <- do.call(rbind, rows)
      write_csv(paired, file.path(dirs$Comparison, "stats", "paired_compartment_shift_tests_v5.csv"))
      treated <- comp[comp$dose_mg > 0, , drop = FALSE]
      obs_cc <- safe_cor_result(treated$ecdf_rmse_CellCycle, treated$TGI_percent_auc, "pearson")$estimate
      obs_nc <- safe_cor_result(treated$ecdf_rmse_NonCellCycle, treated$TGI_percent_auc, "pearson")$estimate
      obs_delta <- obs_cc - obs_nc
      ...
      write_csv(assoc_compare, file.path(dirs$Comparison, "stats", "CellCycle_vs_NonCellCycle_TGI_association_comparison.csv"))
      long <- rbind(
        data.frame(sample_id = comp$sample_id, dose = comp$dose, initial_ploidy = comp$initial_ploidy, compartment = "CellCycle", ecdf_rmse = comp$ecdf_rmse_CellCycle, signed_mean_shift = comp$signed_mean_shift_CellCycle, TGI_percent_auc = comp$TGI_percent_auc, stringsAsFactors = FALSE),
        data.frame(sample_id = comp$sample_id, dose = comp$dose, initial_ploidy = comp$initial_ploidy, compartment = "NonCellCycle", ecdf_rmse = comp$ecdf_rmse_NonCellCycle, signed_mean_shift = comp$signed_mean_shift_NonCellCycle, TGI_percent_auc = comp$TGI_percent_auc, stringsAsFactors = FALSE)
      )
      p1 <- ggplot(long, aes(x = compartment, y = ecdf_rmse, group = sample_id)) +
        geom_line(color = "grey65", linewidth = 0.4) +
        geom_point(aes(color = dose, shape = initial_ploidy), size = 2.5) +
        scale_color_manual(values = dose_cols_all[names(dose_cols_all) %in% unique(long$dose)], name = "dose") +
        labs(title = "Paired CellCycle vs NonCellCycle ECDF RMSE", x = NULL, y = "ECDF RMSE") +
        plot_theme
      save_square_plot(p1, file.path(dirs$Comparison, "figures"), "CellCycle_vs_NonCellCycle_paired_ecdf_rmse_v5", 6.4, 6.4)
      p2 <- ggplot(long, aes(x = compartment, y = signed_mean_shift, group = sample_id)) +
        geom_hline(yintercept = 0, color = "grey75", linewidth = 0.35) +
        geom_line(color = "grey65", linewidth = 0.4) +
        geom_point(aes(color = dose, shape = initial_ploidy), size = 2.5) +
        ...
        labs(title = "Paired CellCycle vs NonCellCycle signed shift", x = NULL, y = "signed mean shift") +
        plot_theme
      save_plot(p2, file.path(dirs$Comparison, "figures"), "CellCycle_vs_NonCellCycle_paired_signed_shift_v5", 6.3, 4.8)
      p3 <- ggplot(data.frame(delta_r = delta_perm), aes(x = delta_r)) +
        geom_histogram(bins = 35, fill = "#999999", color = "white") +
        geom_vline(xintercept = obs_delta, color = "#e41a1c", linewidth = 0.8) +
        labs(title = "Permutation null for CellCycle minus NonCellCycle TGI correlation", x = "delta r", y = "permutations") +
    ...
    composition_analysis <- function(results, n_perm = 10000L) {
      ...
      comp <- merge(cc_counts, nc_counts, by = "sample_id", all = TRUE)
      comp[is.na(comp)] <- 0
      meta <- results$CellCycle$sample_meta[, c("sample_id", "initial_ploidy", "dose", "dose_mg", "TGI_percent_auc"), drop = FALSE]
      comp <- merge(meta, comp, by = "sample_id", all.x = TRUE)
      comp$total_tumor_cells <- comp$CellCycle_cells + comp$NonCellCycle_cells
      comp$CellCycle_fraction <- comp$CellCycle_cells / comp$total_tumor_cells
      comp$NonCellCycle_fraction <- comp$NonCellCycle_cells / comp$total_tumor_cells
      ...
      for (value in c("CellCycle_fraction", "NonCellCycle_fraction")) {
        ...
        if (all(is.finite(comp$CellCycle_cells)) && length(unique(group)) == 2) {
          tmp <- comp
          tmp$treatment_group <- factor(group)
          fit <- try(suppressWarnings(glm(cbind(CellCycle_cells, NonCellCycle_cells) ~ treatment_group + initial_ploidy, family = quasibinomial(), data = tmp)), silent = TRUE)
          if (!inherits(fit, "try-error") && "treatment_grouptreated" %in% rownames(summary(fit)$coefficients)) {
            glm_p <- summary(fit)$coefficients["treatment_grouptreated", "Pr(>|t|)"]
          }
        }
      ...
      write_csv(fraction_tests, file.path(dirs$Composition, "cellcycle_fraction_treatment_tests.csv"))
      cl_tests <- lapply(split(cluster_counts, cluster_counts$cluster), function(tab) {
        tab <- merge(meta[, c("sample_id", "dose_mg", "initial_ploidy")], tab, by = "sample_id", all.x = TRUE)
        tab$cells[is.na(tab$cells)] <- 0
        tab$fraction_all_tumor[is.na(tab$fraction_all_tumor)] <- 0
        gp <- ifelse(tab$dose_mg == 0, "control", "treated")
        wt <- safe_wilcox(tab$fraction_all_tumor, gp)
      ...
      save_plot(p2, dirs$Composition, "CellCycle_cluster_fractions_by_dose", 9, 7)
      if (nrow(cluster_shift) > 0) {
        csp <- cluster_shift[cluster_shift$comparison == "0_vs_30plus120" & !cluster_shift$stratified_by_initial_ploidy, , drop = FALSE]
        p3 <- ggplot(csp, aes(x = as.character(cluster), y = observed_ecdf_rmse)) +
          geom_col(fill = "#377eb8", alpha = 0.85) +
          labs(title = "CellCycle cluster-specific treated-vs-control pseudotime shift", x = "cluster", y = "ECDF RMSE") +
          plot_theme
    ...
    make_growth_curve_long <- function(sample_meta) {
      ...
      rows <- lapply(seq_len(nrow(sample_meta)), function(i) {
        ...
        do.call(rbind, lapply(days, function(day) {
          ...
          data.frame(
            sample_id = sr$sample_id,
            initial_ploidy = sr$initial_ploidy,
            dose = sr$dose,
            dose_mg = sr$dose_mg,
            day = day,
            day_label = dl,
    ...
    growth_model_analysis <- function(results) {
      ...
      growth$log_volume <- log1p(growth$tumor_volume)
      growth$dose <- factor(growth$dose, levels = unique(meta$dose[order(meta$dose_mg)]))
      growth$initial_ploidy <- factor(growth$initial_ploidy, levels = initial_ploidy_levels)
      fit1 <- NULL
      fit2 <- NULL
      mode1 <- "lm"
      mode2 <- "lm"
      if (requireNamespace("nlme", quietly = TRUE) && requireNamespace("splines", quietly = TRUE)) {
        fit1 <- try(nlme::lme(log_volume ~ splines::ns(day, df = 3) * dose + initial_ploidy, random = ~ 1 | sample_id, data = growth, na.action = na.omit, control = nlme::lmeControl(msMaxIter = 100)), silent = TRUE)
        fit2 <- try(nlme::lme(log_volume ~ splines::ns(day, df = 2) * scale(ecdf_rmse) + dose + initial_ploidy, random = ~ 1 | sample_id, data = growth[growth$dose_mg > 0, ], na.action = na.omit, control = nlme::lmeControl(msMaxIter = 100)), silent = TRUE)
        if (inherits(fit1, "try-error")) {
          fit1 <- NULL
        } else {
          mode1 <- "nlme_lme_random_intercept"
      ...
      if (is.null(fit1) && requireNamespace("splines", quietly = TRUE)) fit1 <- lm(log_volume ~ splines::ns(day, df = 3) * dose + initial_ploidy, data = growth)
      if (is.null(fit2) && requireNamespace("splines", quietly = TRUE)) fit2 <- lm(log_volume ~ splines::ns(day, df = 2) * scale(ecdf_rmse) + dose + initial_ploidy, data = growth[growth$dose_mg > 0, ])
      coefs <- rbind(extract_model_coefs(fit1, paste0("dose_growth_model_", mode1)), extract_model_coefs(fit2, paste0("treated_shift_growth_model_", mode2)))
      write_csv(coefs, file.path(dirs$GrowthModel, "growth_model_coefficients.csv"))
      auc_table <- unique(meta[, c("sample_id", "initial_ploidy", "dose", "dose_mg", "TGI_percent_auc", "tumor_volume_auc", "tumor_volume_auc_delta", "matched_control_mean_auc_delta", "auc_start_day", "auc_end_day", "auc_n_days"), drop = FALSE])
      auc_table <- merge(auc_table, results$CellCycle$equal_shift[, c("sample_id", "ecdf_rmse", "signed_mean_shift")], by = "sample_id", all.x = TRUE)
      write_csv(auc_table, file.path(dirs$GrowthModel, "model_based_TGI_or_AUC.csv"))
      dose_levels <- unique(meta$dose[order(meta$dose_mg)])
      p1 <- ggplot(growth, aes(x = day, y = tumor_volume, color = dose, group = sample_id)) +
        ...
        scale_color_manual(values = dose_cols_all[names(dose_cols_all) %in% dose_levels], name = "dose") +
        labs(title = "Tumor growth curves by dose and Initial ploidy", x = "day", y = "tumor volume") +
        plot_theme
      save_plot(p1, dirs$GrowthModel, "growth_curves_by_dose", 8, 5.6)
      treated <- growth[growth$dose_mg > 0, , drop = FALSE]
      cut <- median(unique(auc_table$ecdf_rmse[auc_table$dose_mg > 0]), na.rm = TRUE)
      ...
      save_plot(p2, dirs$GrowthModel, "growth_curves_by_shift_high_low", 7.2, 5)
      if (!is.null(fit2)) {
        pred_grid <- expand.grid(day = seq(min(treated$day, na.rm = TRUE), max(treated$day, na.rm = TRUE), length.out = 60), ecdf_rmse = quantile(treated$ecdf_rmse, c(0.25, 0.75), na.rm = TRUE), dose = unique(treated$dose)[1], initial_ploidy = unique(treated$initial_ploidy)[1])
        pred_grid$shift_group <- rep(c("low shift", "high shift"), each = 60)
        pred <- try(predict(fit2, newdata = pred_grid, level = 0), silent = TRUE)
        if (!inherits(pred, "try-error")) {
          pred_grid$predicted_volume <- expm1(pred)
    ...
    emm_prediction_and_contrasts <- function(fit, growth, observed_days, control_label) {
      ...
      vc <- vc[common, common, drop = FALSE]
      dose_levels <- levels(growth$dose)
      ploidy_levels <- levels(growth$initial_ploidy)
      dose_map <- unique(growth[, c("dose", "dose_mg"), drop = FALSE])
      ...
      row_id <- function(day_value, dose_value, ploidy_value) {
        hit <- which(
          abs(safe_num(as.character(grid$day)) - day_value) < 1e-8 &
            as.character(grid$dose) == as.character(dose_value) &
            as.character(grid$initial_ploidy) == as.character(ploidy_value)
        )
        if (length(hit) == 0) NA_integer_ else hit[1]
      }
          ...
          if (is.finite(control_a) && is.finite(control_b)) {
            for (dose_value in treated_doses) {
              ...
              rows[[length(rows) + 1L]] <- data.frame(
                ...
                dose = dose_value,
                dose_mg = dose_map$dose_mg[match(as.character(dose_value), as.character(dose_map$dose))],
                initial_ploidy = paste0(ploidy_b, "_minus_", ploidy_a),
                day = day_value,
                estimate_log_relative_volume_diff = est$estimate,
                std_error = est$std_error,
                statistic = est$statistic,
    ...
    run_growth_curve_mixed_model <- function(results, spline_df = 3L) {
      ...
      growth <- growth[is.finite(growth$day) & is.finite(growth$tumor_volume) & growth$tumor_volume > 0 &
        is.finite(growth$log_relative_volume), , drop = FALSE]
      dose_levels <- unique(meta$dose[order(meta$dose_mg)])
      ploidy_levels <- unique(meta$initial_ploidy[order(meta$initial_ploidy_numeric)])
      growth$dose <- factor(growth$dose, levels = dose_levels)
      growth$initial_ploidy <- factor(growth$initial_ploidy, levels = ploidy_levels)
      growth$sample_id <- factor(growth$sample_id)
      write_csv(growth, file.path(dirs$GrowthCurveMixedModel, "tables", "tumor_growth_long_for_mixed_model.csv"))

      sample_design <- unique(growth[, c("sample_id", "dose", "dose_mg", "initial_ploidy")])
      observed_design <- aggregate(
        sample_id ~ dose + dose_mg + initial_ploidy,
        sample_design,
        function(x) length(unique(x))
      ...
      design_coverage$empty_combination <- design_coverage$n_samples == 0L
      design_coverage$version <- analysis_version
      write_csv(design_coverage, file.path(dirs$GrowthCurveMixedModel, "stats", "growth_curve_dose_initial_ploidy_design_coverage.csv"))
      ...
      control_label <- as.character(dose_levels[which(safe_num(gsub("[^0-9.]+", "", dose_levels)) == 0)[1]])
      if (!nzchar(control_label) || is.na(control_label)) control_label <- as.character(dose_levels[1])
      full_formula_label <- paste0("log_relative_volume ~ ns(day, df = ", spline_df, ") * dose * initial_ploidy + (1 | sample_id)")
      reduced_formula_label <- paste0("log_relative_volume ~ ns(day, df = ", spline_df, ") * dose + ns(day, df = ", spline_df, ") * initial_ploidy + dose * initial_ploidy + (1 | sample_id)")
      full_formula <- as.formula(paste0("log_relative_volume ~ splines::ns(day, df = ", spline_df, ") * dose * initial_ploidy + (1 | sample_id)"))
      reduced_formula <- as.formula(paste0("log_relative_volume ~ splines::ns(day, df = ", spline_df, ") * dose + splines::ns(day, df = ", spline_df, ") * initial_ploidy + dose * initial_ploidy + (1 | sample_id)"))
      linear_full_formula <- log_relative_volume ~ day * dose * initial_ploidy + (1 | sample_id)
      linear_reduced_formula <- log_relative_volume ~ day * dose + day * initial_ploidy + dose * initial_ploidy + (1 | sample_id)
      spline_fixed_full <- as.formula(paste0("~ splines::ns(day, df = ", spline_df, ") * dose * initial_ploidy"))
      spline_fixed_reduced <- as.formula(paste0("~ splines::ns(day, df = ", spline_df, ") * dose + splines::ns(day, df = ", spline_df, ") * initial_ploidy + dose * initial_ploidy"))
      linear_fixed_full <- ~ day * dose * initial_ploidy
      linear_fixed_reduced <- ~ day * dose + day * initial_ploidy + dose * initial_ploidy
      matrix_audit <- function(formula, analysis) {
        mm <- model.matrix(formula, data = growth)
        rank <- qr(mm)$rank
        data.frame(
          ...
          rank = rank,
          full_rank = rank == ncol(mm),
          empty_dose_initial_ploidy_combinations = sum(design_coverage$empty_combination),
          stringsAsFactors = FALSE
        )
      }
      design_audit <- rbind(
      ...
      fixed_effects <- rbind(
        fixed_effect_table(fit_full, "spline_full_time_dose_ploidy", full_formula_label, "natural_spline_df3", n_rows, n_samples),
        fixed_effect_table(fit_reduced, "spline_reduced_without_time_dose_ploidy", reduced_formula_label, "natural_spline_df3", n_rows, n_samples),
        fixed_effect_table(fit_linear_full, "linear_full_time_dose_ploidy", "log_relative_volume ~ day * dose * initial_ploidy + (1 | sample_id)", "linear_day", n_rows, n_samples),
        fixed_effect_table(fit_linear_reduced, "linear_reduced_without_time_dose_ploidy", "log_relative_volume ~ day * dose + day * initial_ploidy + dose * initial_ploidy + (1 | sample_id)", "linear_day", n_rows, n_samples)
      )
      if (nrow(fixed_effects) > 0) write_csv(fixed_effects, file.path(dirs$GrowthCurveMixedModel, "stats", "growth_curve_mixed_model_fixed_effects.csv"))

      lrt <- rbind(
        lrt_row(fit_reduced, fit_full, "primary_test_time_dose_ploidy_interaction", reduced_formula_label, full_formula_label, "natural_spline_df3", n_rows, n_samples),
        lrt_row(fit_linear_reduced, fit_linear_full, "sensitivity_linear_time_dose_ploidy_interaction", "log_relative_volume ~ day * dose + day * initial_ploidy + dose * initial_ploidy + (1 | sample_id)", "log_relative_volume ~ day * dose * initial_ploidy + (1 | sample_id)", "linear_day", n_rows, n_samples)
      )
      lrt$design_full_rank <- c(
        design_audit$full_rank[design_audit$analysis == "spline_full"] && design_audit$full_rank[design_audit$analysis == "spline_reduced"],
        design_audit$full_rank[design_audit$analysis == "linear_full"] && design_audit$full_rank[design_audit$analysis == "linear_reduced"]
      ...
      lrt$estimability_note <- ifelse(
        lrt$estimable,
        "complete dose-by-initial-ploidy design and full-rank fixed-effect matrices",
        "not estimable as the complete prespecified interaction because at least one dose-by-initial-ploidy combination is empty or the fixed-effect matrix is rank deficient"
      )
      lrt$p_value[!lrt$estimable] <- NA_real_
      write_csv(lrt, file.path(dirs$GrowthCurveMixedModel, "stats", "growth_curve_mixed_model_likelihood_ratio_tests.csv"))
        ...
        if (nrow(did) > 0) {
          p_did <- ggplot(did, aes(x = day, y = estimate_log_relative_volume_diff, color = dose, fill = dose)) +
            ...
            labs(
              title = "Ploidy difference in treatment effect over time",
              x = "day",
              y = "log growth effect difference: 4N minus 2N"
            ) +
            plot_theme
          save_plot(p_did, file.path(dirs$GrowthCurveMixedModel, "figures"), "ploidy_difference_in_treatment_effect_over_time", 7.4, 5)
        }
      ...
      summary_lines <- c(
        "# Growth-Curve Mixed Model Summary",
        "",
        "This module addresses Question 2: whether longitudinal tumor-growth response to gemcitabine differs by Initial ploidy.",
        "",
        "Response variable:",
        "",
        "- `log_relative_volume = log(tumor_volume / tumor_volume_Day_0)`",
        ...
        "Primary likelihood-ratio test:",
        "",
        paste0("- Full spline model vs reduced model without `ns(day):dose:initial_ploidy`; p = ", format_p(primary_lrt_p), "."),
        paste0("- Complete prespecified interaction estimable: ", bool_text(lrt$estimable[lrt$analysis == "primary_test_time_dose_ploidy_interaction"][1]), "."),
        paste0("- Empty dose-by-initial-ploidy combinations: ", sum(design_coverage$empty_combination), "."),
        "",
        "Timepoint contrasts:",
        "",
        "- `treated_minus_control_within_ploidy`: model-estimated treated-vs-control log relative-volume difference within 2N or 4N.",
        "- `ploidy_difference_in_treatment_effect`: `(4N treated - 4N control) - (2N treated - 2N control)` on the log-relative-volume scale.",
        "",
        "Interpretation note:",
        "",
        "- Initial ploidy is the pre-treatment sample grouping variable used in V2; these models test baseline-ploidy effect modification.",
        "",
        "Interpretation rule: a nonsignificant difference test means no clear detected ploidy difference in longitudinal response; it does not prove equality unless an equivalence margin is separately specified."
      )
      if (nrow(final_did) > 0) {
    ...
    parse_group_ids <- function(ids) {
      parts <- strsplit(ids, "|", fixed = TRUE)
      sample_ids <- vapply(parts, `[`, character(1), 1)
      parsed_initial_ploidy <- vapply(parts, `[`, character(1), 2)
      assignment_index <- match(sample_ids, initial_ploidy_assignments$sample_id)
      if (anyNA(assignment_index)) {
        stop("Pseudobulk group IDs contain samples without initial-ploidy assignments: ", paste(unique(sample_ids[is.na(assignment_index)]), collapse = ", "), call. = FALSE)
      }
      expected_initial_ploidy <- as.character(initial_ploidy_assignments$initial_ploidy[assignment_index])
      if (any(parsed_initial_ploidy != expected_initial_ploidy)) {
        stop("Pseudobulk group IDs disagree with original initial-ploidy metadata", call. = FALSE)
      }
      data.frame(
        group_id = ids,
        sample_id = sample_ids,
        initial_ploidy = parsed_initial_ploidy,
        dose = vapply(parts, `[`, character(1), 3),
        group_feature = vapply(parts, function(x) paste(x[-seq_len(3)], collapse = "|"), character(1)),
        stringsAsFactors = FALSE
      )
    ...
    aggregate_pseudobulk_by_sample <- function(pb) {
      ...
      meta <- pb$meta
      sample_mat <- rowsum(mat, group = meta$sample_id, reorder = FALSE)
      sample_meta <- meta[!duplicated(meta$sample_id), c("sample_id", "initial_ploidy", "dose"), drop = FALSE]
      sample_meta$dose_mg <- safe_num(sub("mg/kg", "", sample_meta$dose))
      sample_meta$treatment_group <- ifelse(sample_meta$dose_mg == 0, "control", "treated")
      list(mat = sample_mat, meta = sample_meta)
    }
    ...
    neighborhood_fallback <- function(results) {
      combined <- rbind(
        results$CellCycle$df[, c("cell_id", "sample_id", "cluster", "initial_ploidy", "dose", "dose_mg", "pseudotime", "compartment")],
        results$NonCellCycle$df[, c("cell_id", "sample_id", "cluster", "initial_ploidy", "dose", "dose_mg", "pseudotime", "compartment")]
      )
      breaks <- unique(quantile(combined$pseudotime, probs = seq(0, 1, length.out = 11), na.rm = TRUE))
      combined$neighborhood <- cut(combined$pseudotime, breaks = breaks, include.lowest = TRUE, labels = paste0("pt_bin_", seq_len(length(breaks) - 1)))
      counts <- aggregate(cell_id ~ sample_id + initial_ploidy + dose + dose_mg + compartment + neighborhood, combined, length)
      names(counts)[names(counts) == "cell_id"] <- "cells"
      totals <- aggregate(cells ~ sample_id + compartment, counts, sum)
      names(totals)[3] <- "compartment_cells"
      counts <- merge(counts, totals, by = c("sample_id", "compartment"))
    ...
    write_manuscript_outputs <- function(results, robustness, comparison, composition, growth, dose_specific = NULL, pseudotime_ploidy = NULL, auc_tgi = NULL, ploidy_response = NULL, growth_curve_mixed = NULL) {
      ...
      nc_dose <- results$NonCellCycle$dose_tests
      get_dose_p <- function(tab, comparison, strat = FALSE) {
        hit <- tab$comparison == comparison & tab$stratified_by_initial_ploidy == strat
        if (any(hit)) tab$p_ecdf_rmse[which(hit)[1]] else NA_real_
      }
      first_value <- function(tab, col) {
        if (is.null(tab) || nrow(tab) == 0 || !(col %in% names(tab))) return(NA_real_)
      ...
      growth_lrt <- if (!is.null(growth_curve_mixed)) growth_curve_mixed$lrt else data.frame()
      growth_contrasts <- if (!is.null(growth_curve_mixed)) growth_curve_mixed$contrasts else data.frame()
      cc_ecdf_2n <- if (nrow(ploidy_ecdf) > 0) ploidy_ecdf[ploidy_ecdf$compartment == "CellCycle" & ploidy_ecdf$initial_ploidy == "2N", , drop = FALSE] else data.frame()
      cc_ecdf_4n <- if (nrow(ploidy_ecdf) > 0) ploidy_ecdf[ploidy_ecdf$compartment == "CellCycle" & ploidy_ecdf$initial_ploidy == "4N", , drop = FALSE] else data.frame()
      nc_ecdf_2n <- if (nrow(ploidy_ecdf) > 0) ploidy_ecdf[ploidy_ecdf$compartment == "NonCellCycle" & ploidy_ecdf$initial_ploidy == "2N", , drop = FALSE] else data.frame()
      nc_ecdf_4n <- if (nrow(ploidy_ecdf) > 0) ploidy_ecdf[ploidy_ecdf$compartment == "NonCellCycle" & ploidy_ecdf$initial_ploidy == "4N", , drop = FALSE] else data.frame()
      cc_shift_ploidy <- if (nrow(ploidy_shift) > 0) ploidy_shift[ploidy_shift$compartment == "CellCycle" & ploidy_shift$response_metric == "ecdf_rmse", , drop = FALSE] else data.frame()
      nc_shift_ploidy <- if (nrow(ploidy_shift) > 0) ploidy_shift[ploidy_shift$compartment == "NonCellCycle" & ploidy_shift$response_metric == "ecdf_rmse", , drop = FALSE] else data.frame()
      tgi_auc_ploidy <- if (nrow(ploidy_tgi) > 0) ploidy_tgi[ploidy_tgi$tgi_measure == "TGI_percent_auc", , drop = FALSE] else data.frame()
      cc_shift_interaction <- if (nrow(ploidy_models) > 0) ploidy_models[ploidy_models$compartment == "CellCycle" & ploidy_models$analysis == "shift_treatment_by_ploidy_interaction" & grepl(":", ploidy_models$term), , drop = FALSE] else data.frame()
      cc_tgi_ploidy <- if (nrow(ploidy_models) > 0) ploidy_models[ploidy_models$compartment == "CellCycle" & ploidy_models$analysis == "treated_TGI_by_dose_and_ploidy" & grepl("initial_ploidy", ploidy_models$term), , drop = FALSE] else data.frame()
      cc_shift_tgi_ploidy <- if (nrow(ploidy_models) > 0) ploidy_models[ploidy_models$compartment == "CellCycle" & ploidy_models$analysis == "treated_TGI_shift_by_ploidy_interaction" & grepl(":", ploidy_models$term), , drop = FALSE] else data.frame()
      cc_auc_adjusted_shift <- if (nrow(auc_models) > 0) auc_models[auc_models$compartment == "CellCycle" & auc_models$analysis == "shift_ploidy_dose_adjusted" & auc_models$term == "scale(ecdf_rmse)", , drop = FALSE] else data.frame()
      cc_auc_adjusted_ploidy <- if (nrow(auc_models) > 0) auc_models[auc_models$compartment == "CellCycle" & auc_models$analysis == "shift_ploidy_dose_adjusted" & grepl("initial_ploidy", auc_models$term), , drop = FALSE] else data.frame()
      cc_auc_shift_ploidy_interaction <- if (nrow(auc_models) > 0) auc_models[auc_models$compartment == "CellCycle" & auc_models$analysis == "shift_by_ploidy_dose_adjusted" & grepl(":", auc_models$term), , drop = FALSE] else data.frame()
      nc_auc_adjusted_shift <- if (nrow(auc_models) > 0) auc_models[auc_models$compartment == "NonCellCycle" & auc_models$analysis == "shift_ploidy_dose_adjusted" & auc_models$term == "scale(ecdf_rmse)", , drop = FALSE] else data.frame()
      nc_auc_shift_ploidy_interaction <- if (nrow(auc_models) > 0) auc_models[auc_models$compartment == "NonCellCycle" & auc_models$analysis == "shift_by_ploidy_dose_adjusted" & grepl(":", auc_models$term), , drop = FALSE] else data.frame()
      growth_primary_lrt <- if (nrow(growth_lrt) > 0) growth_lrt[growth_lrt$analysis == "primary_test_time_dose_ploidy_interaction", , drop = FALSE] else data.frame()
      ...
      tex <- c(
        ...
        paste0(
          "Using sample-level ECDFs and an equal-sample ploidy-matched untreated reference, gemcitabine-treated tumors showed a redistribution of pseudotime states among cell-cycle-associated tumor cells relative to untreated controls ",
          "(0 vs treated ECDF RMSE permutation $p=", format_p(get_dose_p(cc_dose, "0_vs_30plus120", FALSE)), "$). ",
          "This result remained similar under Initial-ploidy-stratified sample-label permutation ($p=", format_p(get_dose_p(cc_dose, "0_vs_30plus120", TRUE)), "$). ",
          "The 30 mg/kg and 120 mg/kg treated groups were not clearly separated from each other ($p=", format_p(get_dose_p(cc_dose, "30_vs_120", FALSE)), "$), so the effect is best described as treatment-associated rather than dose-dependent."
        ),
        "",
        paste0(
        ...
        "",
        paste0(
          "Initial-ploidy effect modification was then tested on the pseudotime treatment effect itself, using the difference-in-differences curve ",
          "$[\\Delta ECDF_{4N}(t)-\\Delta ECDF_{2N}(t)]$. ",
          "For CellCycle, the 4N-minus-2N DID ECDF RMSE was ", format_num(first_value(cc_pseudotime_ploidy_did, "did_ecdf_rmse")), " ",
          "(ploidy-label permutation within treatment groups $p=", format_p(first_value(cc_pseudotime_ploidy_did, "p_did_ecdf_rmse")), "$; bootstrap 95\\% CI [", format_num(first_value(cc_pseudotime_ploidy_did, "did_ecdf_rmse_ci025")), ", ", format_num(first_value(cc_pseudotime_ploidy_did, "did_ecdf_rmse_ci975")), "]). ",
          "For NonCellCycle, the analogous DID ECDF RMSE was ", format_num(first_value(nc_pseudotime_ploidy_did, "did_ecdf_rmse")), " ",
          "($p=", format_p(first_value(nc_pseudotime_ploidy_did, "p_did_ecdf_rmse")), "$). ",
          "This analysis belongs to the pseudotime branch: it asks whether gemcitabine-induced pseudotime redistribution differs between 2N and 4N sample groups, not whether TGI differs by ploidy."
        ),
        "",
        paste0(
          "Among treated tumors, the magnitude of the CellCycle pseudotime redistribution showed a positive association with AUC-based TGI ",
        ...
        paste0(
          "To address overall tumor-growth inhibition directly, AUC-based TGI was modeled as a sample-level endpoint in treated tumors. ",
          "In CellCycle, the dose-adjusted model `TGI_percent_auc ~ ECDF shift + Initial ploidy + dose' gave an ECDF-shift term $p=", format_p(first_value(cc_auc_adjusted_shift, "p_value")), "$ and an Initial-ploidy term $p=", format_p(first_value(cc_auc_adjusted_ploidy, "p_value")), "$. ",
          "The shift-by-ploidy interaction sensitivity model gave $p=", format_p(first_value(cc_auc_shift_ploidy_interaction, "p_value")), "$. ",
          "In NonCellCycle, the analogous ECDF-shift term gave $p=", format_p(first_value(nc_auc_adjusted_shift, "p_value")), "$ and the shift-by-ploidy interaction gave $p=", format_p(first_value(nc_auc_shift_ploidy_interaction, "p_value")), "$. ",
          "Thus AUC-TGI is used as the primary summary of overall inhibition, while ploidy-specific equality is not inferred from these small treated-only regressions."
        ),
        ...
        "",
        paste0(
          "Initial-ploidy response analyses did not detect a clear 2N versus 4N response difference. ",
          "Within CellCycle, 0 versus treated ECDF tests were computed separately in 2N and 4N samples ",
          "(2N $p=", format_p(first_value(cc_ecdf_2n, "p_ecdf_rmse")), "$; 4N $p=", format_p(first_value(cc_ecdf_4n, "p_ecdf_rmse")), "$). ",
          "Among treated tumors, the 4N-minus-2N CellCycle ECDF RMSE difference was ", format_num(first_value(cc_shift_ploidy, "mean_diff_b_minus_a")), " ",
          "(permutation $p=", format_p(first_value(cc_shift_ploidy, "permutation_p_two_sided")), "$; bootstrap 95\\% CI [", format_num(first_value(cc_shift_ploidy, "bootstrap_ci025")), ", ", format_num(first_value(cc_shift_ploidy, "bootstrap_ci975")), "]). ",
          "The corresponding 4N-minus-2N AUC TGI difference was ", format_num(first_value(tgi_auc_ploidy, "mean_diff_b_minus_a")), " percentage points ",
          "(permutation $p=", format_p(first_value(tgi_auc_ploidy, "permutation_p_two_sided")), "$; bootstrap 95\\% CI [", format_num(first_value(tgi_auc_ploidy, "bootstrap_ci025")), ", ", format_num(first_value(tgi_auc_ploidy, "bootstrap_ci975")), "]). ",
          "The pre-specified equivalence margins were $\\pm 0.01$ ECDF RMSE and $\\pm 10$ TGI percentage points; the CIs should be used to state whether equivalence is formally established rather than interpreting nonsignificant difference tests as proof of equality."
        ),
        "",
        paste0(
          "For the longitudinal growth-response question, raw tumor-volume measurements were modeled as `log(tumor volume / Day 0 tumor volume)' with a natural-spline mixed model containing sample-level random intercepts. ",
          "The primary likelihood-ratio test compared the full model with `ns(day) \\times dose \\times Initial ploidy' against a reduced model without that three-way interaction; $p=", format_p(first_value(growth_primary_lrt, "p_value")), "$. ",
          "A linear-time sensitivity model gave $p=", format_p(first_value(growth_linear_lrt, "p_value")), "$. ",
          "At Day ", format_num(growth_final_day, digits = 4), ", the model-estimated ploidy difference-in-differences was ", format_num(first_value(did30, "estimate_log_relative_volume_diff")), " for 30 mg/kg ($p=", format_p(first_value(did30, "p_value")), "$) and ", format_num(first_value(did120, "estimate_log_relative_volume_diff")), " for 120 mg/kg ($p=", format_p(first_value(did120, "p_value")), "$). ",
          "This mixed model describes whether longitudinal tumor-growth response differs by the original baseline initial-ploidy group."
        ),
        "",
        paste0(
          "The formal CellCycle-minus-NonCellCycle TGI-correlation difference was $\\Delta r=", format_num(delta$observed_delta_r[1]), "$ ",
      ...
      change_log <- c(
        ...
        "- Added leave-one-out, bootstrap, cell-count downsampling, ploidy/dose residualization, paired compartment tests, composition, neighborhood fallback, pseudobulk/module, and growth-model sensitivity outputs.",
        "- Added dose-specific TGI exploration: 30 vs 120 AUC/endpoint TGI comparisons, within-dose shift-TGI correlations, slope-interaction sensitivity, and dose-specific figures.",
        "- Added explicit AUC-TGI association models for the overall inhibition question: AUC-TGI by pseudotime shift, Initial ploidy, treated dose, and shift-by-ploidy interaction.",
        "- Added spline mixed-effects growth-curve models for the longitudinal response question: log-relative tumor volume by time, dose, Initial ploidy, and time-by-dose-by-ploidy interaction.",
        "- Wording was revised to avoid unsupported dose-dependence, global ploidy-independence, predictive-biomarker, or definitive CellCycle-specific TGI-association claims."
      )
      write_text(change_log, file.path(dirs$Manuscript, "dose_pseudotime_conclusion_change_log.md"))
      lit <- c(
      ...
      report <- c(
        ...
        paste0("CellCycle 0 vs treated sample-aware ECDF p = ", format_p(get_dose_p(cc_dose, "0_vs_30plus120", FALSE)), "; ploidy-stratified p = ", format_p(get_dose_p(cc_dose, "0_vs_30plus120", TRUE)), "."),
        paste0("NonCellCycle 0 vs treated sample-aware ECDF p = ", format_p(get_dose_p(nc_dose, "0_vs_30plus120", FALSE)), "."),
        paste0("Pseudotime A2 CellCycle 4N-minus-2N treatment-effect DID ECDF RMSE = ", format_num(first_value(cc_pseudotime_ploidy_did, "did_ecdf_rmse")), ", permutation p = ", format_p(first_value(cc_pseudotime_ploidy_did, "p_did_ecdf_rmse")), "."),
        paste0("CellCycle treated-only AUC TGI association: Pearson r = ", format_num(cc_primary$pearson_r[1]), ", permutation p = ", format_p(cc_primary$pearson_p_permutation_two_sided[1]), "; Spearman rho = ", format_num(cc_primary$spearman_rho[1]), ", permutation p = ", format_p(cc_primary$spearman_p_permutation_two_sided[1]), "."),
        paste0("NonCellCycle treated-only AUC TGI association: Pearson r = ", format_num(nc_primary$pearson_r[1]), ", permutation p = ", format_p(nc_primary$pearson_p_permutation_two_sided[1]), "; Spearman rho = ", format_num(nc_primary$spearman_rho[1]), ", permutation p = ", format_p(nc_primary$spearman_p_permutation_two_sided[1]), "."),
        paste0("Question 1 AUC-TGI model: CellCycle dose-adjusted ECDF-shift term p = ", format_p(first_value(cc_auc_adjusted_shift, "p_value")), "; Initial-ploidy term p = ", format_p(first_value(cc_auc_adjusted_ploidy, "p_value")), "; shift-by-ploidy interaction p = ", format_p(first_value(cc_auc_shift_ploidy_interaction, "p_value")), "."),
        paste0("Dose-specific CellCycle AUC TGI comparison, 120 minus 30 mg/kg mean difference = ", format_num(first_value(cc_auc_dose, "mean_diff_b_minus_a")), ", permutation p = ", format_p(first_value(cc_auc_dose, "permutation_p_two_sided")), "."),
        paste0("Dose-specific NonCellCycle within-dose shift-TGI correlations do not mirror the CellCycle positive pattern: 30 mg/kg Pearson r = ", format_num(first_value(nc_30_cor, "pearson_r")), ", permutation p = ", format_p(first_value(nc_30_cor, "pearson_p_permutation_two_sided")), "; 120 mg/kg Pearson r = ", format_num(first_value(nc_120_cor, "pearson_r")), ", permutation p = ", format_p(first_value(nc_120_cor, "pearson_p_permutation_two_sided")), "."),
        paste0("Initial-ploidy response analysis: CellCycle treated ECDF RMSE 4N minus 2N mean difference = ", format_num(first_value(cc_shift_ploidy, "mean_diff_b_minus_a")), ", permutation p = ", format_p(first_value(cc_shift_ploidy, "permutation_p_two_sided")), "; AUC TGI 4N minus 2N mean difference = ", format_num(first_value(tgi_auc_ploidy, "mean_diff_b_minus_a")), ", permutation p = ", format_p(first_value(tgi_auc_ploidy, "permutation_p_two_sided")), "."),
        paste0("Question 2 spline mixed model: time-by-dose-by-ploidy LRT p = ", format_p(first_value(growth_primary_lrt, "p_value")), "; Day ", format_num(growth_final_day, digits = 4), " ploidy difference-in-differences p values are 30 mg/kg p = ", format_p(first_value(did30, "p_value")), " and 120 mg/kg p = ", format_p(first_value(did120, "p_value")), "."),
        "",
        "## What changed relative to original analysis",
        "",
        ...
        "## Analysis structure",
        "",
        "A. Pseudotime analysis: A1 tests whether gemcitabine changes CellCycle/NonCellCycle pseudotime distributions; A2 tests whether the pseudotime treatment effect differs between 2N and 4N sample groups.",
        "",
        "B. TGI / tumor-growth response analysis: B1 tests whether pseudotime shift is associated with AUC-based TGI; B2 tests whether tumor-growth response differs between 2N and 4N, using AUC-TGI as the overall inhibition summary and a spline mixed model for longitudinal growth curves.",
        "",
        "## A1: CellCycle pseudotime treatment effect",
        "",
        paste0("The CellCycle treated-vs-untreated ECDF test gives p = ", format_p(get_dose_p(cc_dose, "0_vs_30plus120", FALSE)), "; the Initial-ploidy-stratified version gives p = ", format_p(get_dose_p(cc_dose, "0_vs_30plus120", TRUE)), "."),
        "",
        "## A1: NonCellCycle pseudotime comparison",
        "",
        paste0("The NonCellCycle treated-vs-untreated ECDF test gives p = ", format_p(get_dose_p(nc_dose, "0_vs_30plus120", FALSE)), ". This is the comparison compartment for specificity. Its treated-only AUC TGI association is not positive like CellCycle: Pearson r = ", format_num(nc_primary$pearson_r[1]), ", permutation p = ", format_p(nc_primary$pearson_p_permutation_two_sided[1]), "."),
        ...
        "## A2: pseudotime ploidy effect modification",
        "",
        paste0("A2 compares the treatment-effect curves themselves: Delta ECDF_2N(t) = treated 2N minus control 2N, Delta ECDF_4N(t) = treated 4N minus control 4N, and DID(t) = Delta ECDF_4N(t) minus Delta ECDF_2N(t). For CellCycle, DID ECDF RMSE = ", format_num(first_value(cc_pseudotime_ploidy_did, "did_ecdf_rmse")), ", initial-ploidy-label permutation p = ", format_p(first_value(cc_pseudotime_ploidy_did, "p_did_ecdf_rmse")), ", bootstrap CI = [", format_num(first_value(cc_pseudotime_ploidy_did, "did_ecdf_rmse_ci025")), ", ", format_num(first_value(cc_pseudotime_ploidy_did, "did_ecdf_rmse_ci975")), "]. For NonCellCycle, DID ECDF RMSE = ", format_num(first_value(nc_pseudotime_ploidy_did, "did_ecdf_rmse")), ", permutation p = ", format_p(first_value(nc_pseudotime_ploidy_did, "p_did_ecdf_rmse")), "."),
        "This A2 test is a pseudotime analysis, not a TGI analysis. It asks whether gemcitabine-induced pseudotime redistribution differs between 2N and 4N sample groups.",
        "",
        "## B1: TGI correlation and robustness",
        "",
        paste0("The primary CellCycle TGI association is positive but should remain nominal/suggestive. Leave-one-out all positive: ", bool_text(robustness$summary$leave_one_out_all_positive[1]), "; bootstrap Pearson r 95% interval = [", format_num(robustness$summary$bootstrap_pearson_r_ci025[1]), ", ", format_num(robustness$summary$bootstrap_pearson_r_ci975[1]), "]."),
        ...
        "## B1: AUC-TGI correlation with pseudotime shift and ploidy",
        "",
        paste0("AUC-based TGI is the primary summary endpoint for overall tumor-growth inhibition. In the treated-only CellCycle model adjusted for dose and Initial ploidy, the ECDF-shift term has p = ", format_p(first_value(cc_auc_adjusted_shift, "p_value")), "; the Initial-ploidy term has p = ", format_p(first_value(cc_auc_adjusted_ploidy, "p_value")), "; and the shift-by-ploidy interaction has p = ", format_p(first_value(cc_auc_shift_ploidy_interaction, "p_value")), ". The analogous NonCellCycle adjusted ECDF-shift term has p = ", format_p(first_value(nc_auc_adjusted_shift, "p_value")), ", and the NonCellCycle shift-by-ploidy interaction has p = ", format_p(first_value(nc_auc_shift_ploidy_interaction, "p_value")), "."),
        "",
        "## B2a support: dose-specific and endpoint TGI",
        "",
        paste0("The new dose-specific module asks two questions: whether TGI differs between 30 and 120 mg/kg, and whether the shift-TGI association has the same direction within each dose stratum. For CellCycle AUC-based TGI, 120 minus 30 mg/kg mean difference = ", format_num(first_value(cc_auc_dose, "mean_diff_b_minus_a")), " percentage points; exact/permutation p = ", format_p(first_value(cc_auc_dose, "permutation_p_two_sided")), ". Across endpoint TGI columns, the smallest exploratory endpoint permutation p = ", format_p(endpoint_min_p), " and the smallest BH q = ", format_p(endpoint_min_q), "."),
        ...
        paste0("For NonCellCycle, the sample-level AUC TGI 120 minus 30 mg/kg mean difference is the same response contrast, ", format_num(first_value(nc_auc_dose, "mean_diff_b_minus_a")), " percentage points, with exact/permutation p = ", format_p(first_value(nc_auc_dose, "permutation_p_two_sided")), ". Across endpoint TGI columns, the smallest exploratory endpoint permutation p = ", format_p(nc_endpoint_min_p), " and the smallest BH q = ", format_p(nc_endpoint_min_q), ". Within-dose NonCellCycle AUC shift-TGI correlations are negative/weak: 30 mg/kg Pearson r = ", format_num(first_value(nc_30_cor, "pearson_r")), ", permutation p = ", format_p(first_value(nc_30_cor, "pearson_p_permutation_two_sided")), "; 120 mg/kg Pearson r = ", format_num(first_value(nc_120_cor, "pearson_r")), ", permutation p = ", format_p(first_value(nc_120_cor, "pearson_p_permutation_two_sided")), ". The NonCellCycle interaction-model slope-difference p = ", format_p(first_value(nc_int_hit, "p_value")), "."),
        "",
        "## B2a: AUC-TGI response by Initial ploidy and supporting shift summaries",
        "",
        paste0("This module treats Initial ploidy as an effect modifier. Within-group CellCycle 0 vs treated ECDF tests gave 2N p = ", format_p(first_value(cc_ecdf_2n, "p_ecdf_rmse")), " and 4N p = ", format_p(first_value(cc_ecdf_4n, "p_ecdf_rmse")), ". Within-group NonCellCycle tests gave 2N p = ", format_p(first_value(nc_ecdf_2n, "p_ecdf_rmse")), " and 4N p = ", format_p(first_value(nc_ecdf_4n, "p_ecdf_rmse")), "."),
        paste0("Among treated tumors, CellCycle ECDF RMSE 4N minus 2N mean difference = ", format_num(first_value(cc_shift_ploidy, "mean_diff_b_minus_a")), "; permutation p = ", format_p(first_value(cc_shift_ploidy, "permutation_p_two_sided")), "; bootstrap 95% CI = [", format_num(first_value(cc_shift_ploidy, "bootstrap_ci025")), ", ", format_num(first_value(cc_shift_ploidy, "bootstrap_ci975")), "]. The pre-specified ECDF RMSE equivalence margin was +/-0.01; CI within margin = ", bool_text(first_value(cc_shift_ploidy, "ci_within_equivalence_margin")), "."),
        paste0("TGI AUC 4N minus 2N mean difference = ", format_num(first_value(tgi_auc_ploidy, "mean_diff_b_minus_a")), " percentage points; permutation p = ", format_p(first_value(tgi_auc_ploidy, "permutation_p_two_sided")), "; bootstrap 95% CI = [", format_num(first_value(tgi_auc_ploidy, "bootstrap_ci025")), ", ", format_num(first_value(tgi_auc_ploidy, "bootstrap_ci975")), "]. The pre-specified AUC TGI equivalence margin was +/-10 percentage points; CI within margin = ", bool_text(first_value(tgi_auc_ploidy, "ci_within_equivalence_margin")), "."),
        paste0("Interaction sensitivities: CellCycle treatment-by-ploidy ECDF RMSE interaction p = ", format_p(first_value(cc_shift_interaction, "p_value")), "; treated TGI ploidy term p = ", format_p(first_value(cc_tgi_ploidy, "p_value")), "; treated shift-by-ploidy TGI interaction p = ", format_p(first_value(cc_shift_tgi_ploidy, "p_value")), ". These small-n models should be interpreted as screening sensitivity rather than definitive proof of equality or difference."),
        "",
        "## B2b: longitudinal growth response by Initial ploidy",
        "",
        paste0("The primary spline mixed-effects model uses `log(tumor volume / Day 0 tumor volume)` and a sample-level random intercept. The full-vs-reduced likelihood-ratio test for the time-by-dose-by-Initial-ploidy interaction gives p = ", format_p(first_value(growth_primary_lrt, "p_value")), ". The linear-time sensitivity version gives p = ", format_p(first_value(growth_linear_lrt, "p_value")), "."),
        paste0("At Day ", format_num(growth_final_day, digits = 4), ", the 4N-minus-2N treatment-effect difference on the log-relative-volume scale is ", format_num(first_value(did30, "estimate_log_relative_volume_diff")), " for 30 mg/kg (p = ", format_p(first_value(did30, "p_value")), ") and ", format_num(first_value(did120, "estimate_log_relative_volume_diff")), " for 120 mg/kg (p = ", format_p(first_value(did120, "p_value")), "). These contrasts compare `(4N treated - 4N control) - (2N treated - 2N control)` and directly address whether longitudinal treatment response differs by Initial ploidy."),
        "Initial ploidy is defined before treatment; these growth analyses therefore evaluate baseline-ploidy effect modification.",
        "",
        "## Ploidy, dose, and cell-count sensitivity",
        "",
        "Ploidy/dose confounding tables and downsampling summaries are in `C_SupportingAnalyses/C1_Robustness/`. These should be used to say whether the shift is not detectably explained by ploidy or dose; they should not be used to claim strong independence.",
        ...
        "## Gene program, pseudobulk, neighborhood, and growth-model results",
        "",
        "When available, targeted pseudobulk counts were used for module/pseudobulk summaries. MiloR was unavailable, so neighborhood DA used pseudotime bins as a fallback. The legacy GrowthModel module remains a supportive sensitivity analysis, while the GrowthCurveMixedModel module is the formal longitudinal growth-response analysis for Initial-ploidy effect modification.",
        "",
        "## Final answer to the scientific question",
        "",
        "The analysis has two main branches. In the pseudotime branch, A1 tests gemcitabine-associated pseudotime redistribution, and A2 tests whether that effect differs between original 2N and 4N samples. In the TGI branch, B1 uses AUC-based TGI for shift-TGI association, while B2 separates overall AUC-TGI response from longitudinal mixed-model growth response. Initial ploidy is the baseline grouping variable throughout.",
        "",
        "## Supported claims",
        "",
        "- Gemcitabine treatment is associated with a CellCycle pseudotime redistribution.",
        "- The CellCycle treatment effect can be tested with sample-level and ploidy-stratified permutations.",
        "- Pseudotime Initial-ploidy effect modification is tested directly with Delta ECDF 4N-minus-2N difference-in-differences curves.",
        "- The treated-only CellCycle TGI association is positive in direction.",
        "- AUC-based TGI is the primary endpoint for the overall tumor-growth inhibition question.",
        "- The spline mixed-effects model is the primary analysis for whether longitudinal tumor-growth response differs by Initial ploidy.",
        "- Dose-specific TGI checks can be reported as exploratory negative/descriptive evidence.",
        "- Initial-ploidy response checks compare 2N with 4N, with equivalence judged by the pre-specified CI margins.",
        "",
        "## Suggestive or unsupported claims",
        "",
        "- Predictive biomarker claims are unsupported.",
        ...
        "- NonCellCycle sample-equal ECDFs by dose/ploidy.",
        "- CellCycle AUC TGI vs ECDF RMSE.",
        "- Pseudotime 2N vs 4N Delta ECDF effect-modification curves.",
        "- AUC-TGI association panels by Initial ploidy and dose.",
        "- Dose-specific AUC/endpoint TGI panels and within-dose shift-TGI panel.",
        "- Ploidy-response shift and TGI panels.",
        "- Spline mixed-model predicted growth curves and ploidy difference-in-differences over time.",
        "- Leave-one-out/bootstrap robustness panels.",
    ...
    message("Input root: ", input_root)
    message("Results root: ", results_root)
    message("Grouping variable: original sample initial_ploidy (2N/4N)")
    ...
    message("Running continuous endpoint-ploidy exploration by initial-ploidy group")
    for (name in names(compartments)) {
      plot_endpoint_continuous_exploration(name, results[[name]])
      plot_sample_level_initial_ploidy_violin(name, results[[name]], compartments[[name]], n_perm)
    }
    ...
    summary_values <- list(
      results_root = results_root,
      cellcycle_treated_vs_control_p = results$CellCycle$dose_tests$p_ecdf_rmse[results$CellCycle$dose_tests$comparison == "0_vs_30plus120" & !results$CellCycle$dose_tests$stratified_by_initial_ploidy][1],
      cellcycle_treated_vs_control_ploidy_stratified_p = results$CellCycle$dose_tests$p_ecdf_rmse[results$CellCycle$dose_tests$comparison == "0_vs_30plus120" & results$CellCycle$dose_tests$stratified_by_initial_ploidy][1],
      noncellcycle_treated_vs_control_p = results$NonCellCycle$dose_tests$p_ecdf_rmse[results$NonCellCycle$dose_tests$comparison == "0_vs_30plus120" & !results$NonCellCycle$dose_tests$stratified_by_initial_ploidy][1],
      cellcycle_pseudotime_ploidy_did_ecdf_rmse = scalar_or_na(pseudotime_ploidy$tests$did_ecdf_rmse[pseudotime_ploidy$tests$compartment == "CellCycle"]),
      cellcycle_pseudotime_ploidy_did_ecdf_rmse_perm_p = scalar_or_na(pseudotime_ploidy$tests$p_did_ecdf_rmse[pseudotime_ploidy$tests$compartment == "CellCycle"]),
      cellcycle_pseudotime_ploidy_did_ecdf_rmse_ci025 = scalar_or_na(pseudotime_ploidy$tests$did_ecdf_rmse_ci025[pseudotime_ploidy$tests$compartment == "CellCycle"]),
      cellcycle_pseudotime_ploidy_did_ecdf_rmse_ci975 = scalar_or_na(pseudotime_ploidy$tests$did_ecdf_rmse_ci975[pseudotime_ploidy$tests$compartment == "CellCycle"]),
      ...
      cellcycle_TGI_AUC_pearson_perm_p = manuscript$cc_primary$pearson_p_permutation_two_sided[1],
      cellcycle_AUC_TGI_adjusted_shift_p = scalar_or_na(auc_tgi$models$p_value[auc_tgi$models$compartment == "CellCycle" & auc_tgi$models$analysis == "shift_ploidy_dose_adjusted" & auc_tgi$models$term == "scale(ecdf_rmse)"]),
      cellcycle_AUC_TGI_adjusted_initial_ploidy_p = scalar_or_na(auc_tgi$models$p_value[auc_tgi$models$compartment == "CellCycle" & auc_tgi$models$analysis == "shift_ploidy_dose_adjusted" & grepl("initial_ploidy", auc_tgi$models$term)]),
      cellcycle_AUC_TGI_shift_by_ploidy_interaction_p = scalar_or_na(auc_tgi$models$p_value[auc_tgi$models$compartment == "CellCycle" & auc_tgi$models$analysis == "shift_by_ploidy_dose_adjusted" & grepl(":", auc_tgi$models$term)]),
      cellcycle_AUC_TGI_mean_ETP_unadjusted_slope = scalar_or_na(auc_tgi$mean_etp_models$estimate[auc_tgi$mean_etp_models$compartment == "CellCycle" & auc_tgi$mean_etp_models$analysis == "mean_etp_only" & auc_tgi$mean_etp_models$term == "scale(sample_mean_endpoint_ploidy)"]),
      cellcycle_AUC_TGI_mean_ETP_unadjusted_p = scalar_or_na(auc_tgi$mean_etp_models$p_value[auc_tgi$mean_etp_models$compartment == "CellCycle" & auc_tgi$mean_etp_models$analysis == "mean_etp_only" & auc_tgi$mean_etp_models$term == "scale(sample_mean_endpoint_ploidy)"]),
      cellcycle_AUC_TGI_mean_ETP_adjusted_p = scalar_or_na(auc_tgi$mean_etp_models$p_value[auc_tgi$mean_etp_models$compartment == "CellCycle" & auc_tgi$mean_etp_models$analysis == "mean_etp_ploidy_dose_adjusted" & auc_tgi$mean_etp_models$term == "scale(sample_mean_endpoint_ploidy)"]),
      ...
      noncellcycle_TGI_AUC_shift_120mg_perm_p = scalar_or_na(dose_specific$correlations$pearson_p_permutation_two_sided[dose_specific$correlations$compartment == "NonCellCycle" & dose_specific$correlations$dose_mg == 120 & dose_specific$correlations$shift_metric == "ecdf_rmse" & dose_specific$correlations$tgi_measure == "TGI_percent_auc"]),
      noncellcycle_shift_TGI_AUC_dose_interaction_p = scalar_or_na(dose_specific$interactions$p_value[dose_specific$interactions$compartment == "NonCellCycle" & grepl("scale\\(ecdf_rmse\\).*:factor\\(dose_mg\\)|factor\\(dose_mg\\).*:scale\\(ecdf_rmse\\)", dose_specific$interactions$term)]),
      cellcycle_2N_treated_vs_control_ecdf_p = scalar_or_na(ploidy_response$ecdf$p_ecdf_rmse[ploidy_response$ecdf$compartment == "CellCycle" & ploidy_response$ecdf$initial_ploidy == "2N"]),
      cellcycle_4N_treated_vs_control_ecdf_p = scalar_or_na(ploidy_response$ecdf$p_ecdf_rmse[ploidy_response$ecdf$compartment == "CellCycle" & ploidy_response$ecdf$initial_ploidy == "4N"]),
      noncellcycle_2N_treated_vs_control_ecdf_p = scalar_or_na(ploidy_response$ecdf$p_ecdf_rmse[ploidy_response$ecdf$compartment == "NonCellCycle" & ploidy_response$ecdf$initial_ploidy == "2N"]),
      noncellcycle_4N_treated_vs_control_ecdf_p = scalar_or_na(ploidy_response$ecdf$p_ecdf_rmse[ploidy_response$ecdf$compartment == "NonCellCycle" & ploidy_response$ecdf$initial_ploidy == "4N"]),
      cellcycle_ecdf_rmse_4N_minus_2N_mean_diff = scalar_or_na(ploidy_response$shift$mean_diff_b_minus_a[ploidy_response$shift$compartment == "CellCycle" & ploidy_response$shift$response_metric == "ecdf_rmse"]),
      cellcycle_ecdf_rmse_4N_minus_2N_perm_p = scalar_or_na(ploidy_response$shift$permutation_p_two_sided[ploidy_response$shift$compartment == "CellCycle" & ploidy_response$shift$response_metric == "ecdf_rmse"]),
      cellcycle_ecdf_rmse_4N_minus_2N_ci025 = scalar_or_na(ploidy_response$shift$bootstrap_ci025[ploidy_response$shift$compartment == "CellCycle" & ploidy_response$shift$response_metric == "ecdf_rmse"]),
      cellcycle_ecdf_rmse_4N_minus_2N_ci975 = scalar_or_na(ploidy_response$shift$bootstrap_ci975[ploidy_response$shift$compartment == "CellCycle" & ploidy_response$shift$response_metric == "ecdf_rmse"]),
      cellcycle_ecdf_rmse_equivalence_margin_met = scalar_or_na(ploidy_response$shift$ci_within_equivalence_margin[ploidy_response$shift$compartment == "CellCycle" & ploidy_response$shift$response_metric == "ecdf_rmse"]),
      tgi_auc_4N_minus_2N_mean_diff = scalar_or_na(ploidy_response$tgi$mean_diff_b_minus_a[ploidy_response$tgi$tgi_measure == "TGI_percent_auc"]),
      tgi_auc_4N_minus_2N_perm_p = scalar_or_na(ploidy_response$tgi$permutation_p_two_sided[ploidy_response$tgi$tgi_measure == "TGI_percent_auc"]),
      tgi_auc_4N_minus_2N_ci025 = scalar_or_na(ploidy_response$tgi$bootstrap_ci025[ploidy_response$tgi$tgi_measure == "TGI_percent_auc"]),
      tgi_auc_4N_minus_2N_ci975 = scalar_or_na(ploidy_response$tgi$bootstrap_ci975[ploidy_response$tgi$tgi_measure == "TGI_percent_auc"]),
      tgi_auc_equivalence_margin_met = scalar_or_na(ploidy_response$tgi$ci_within_equivalence_margin[ploidy_response$tgi$tgi_measure == "TGI_percent_auc"]),
      cellcycle_treatment_by_ploidy_ecdf_interaction_p = scalar_or_na(ploidy_response$models$p_value[ploidy_response$models$compartment == "CellCycle" & ploidy_response$models$analysis == "shift_treatment_by_ploidy_interaction" & grepl(":", ploidy_response$models$term)]),
      cellcycle_TGI_ploidy_adjusted_term_p = scalar_or_na(ploidy_response$models$p_value[ploidy_response$models$compartment == "CellCycle" & ploidy_response$models$analysis == "treated_TGI_by_dose_and_ploidy" & grepl("initial_ploidy", ploidy_response$models$term)]),
      cellcycle_shift_TGI_ploidy_interaction_p = scalar_or_na(ploidy_response$models$p_value[ploidy_response$models$compartment == "CellCycle" & ploidy_response$models$analysis == "treated_TGI_shift_by_ploidy_interaction" & grepl(":", ploidy_response$models$term)]),
      growth_curve_spline_time_dose_ploidy_lrt_p = scalar_or_na(growth_curve_mixed$lrt$p_value[growth_curve_mixed$lrt$analysis == "primary_test_time_dose_ploidy_interaction"]),
      growth_curve_linear_time_dose_ploidy_lrt_p = scalar_or_na(growth_curve_mixed$lrt$p_value[growth_curve_mixed$lrt$analysis == "sensitivity_linear_time_dose_ploidy_interaction"]),
      growth_curve_final_day = summary_growth_final_day,
    ...
    write_text(c(
      ...
      paste0("- CellCycle ploidy-stratified 0 vs treated ECDF p: ", format_p(summary_values$cellcycle_treated_vs_control_ploidy_stratified_p)),
      paste0("- NonCellCycle 0 vs treated ECDF p: ", format_p(summary_values$noncellcycle_treated_vs_control_p)),
      paste0("- A2 CellCycle pseudotime treatment-effect 4N-minus-2N DID ECDF RMSE: ", format_num(summary_values$cellcycle_pseudotime_ploidy_did_ecdf_rmse), ", permutation p: ", format_p(summary_values$cellcycle_pseudotime_ploidy_did_ecdf_rmse_perm_p), ", bootstrap CI: [", format_num(summary_values$cellcycle_pseudotime_ploidy_did_ecdf_rmse_ci025), ", ", format_num(summary_values$cellcycle_pseudotime_ploidy_did_ecdf_rmse_ci975), "]"),
      paste0("- A2 NonCellCycle pseudotime treatment-effect 4N-minus-2N DID ECDF RMSE: ", format_num(summary_values$noncellcycle_pseudotime_ploidy_did_ecdf_rmse), ", permutation p: ", format_p(summary_values$noncellcycle_pseudotime_ploidy_did_ecdf_rmse_perm_p), ", bootstrap CI: [", format_num(summary_values$noncellcycle_pseudotime_ploidy_did_ecdf_rmse_ci025), ", ", format_num(summary_values$noncellcycle_pseudotime_ploidy_did_ecdf_rmse_ci975), "]"),
      paste0("- CellCycle TGI AUC Pearson r: ", format_num(summary_values$cellcycle_TGI_AUC_pearson_r), ", permutation p: ", format_p(summary_values$cellcycle_TGI_AUC_pearson_perm_p)),
      paste0("- Question 1 CellCycle AUC-TGI adjusted ECDF-shift p: ", format_p(summary_values$cellcycle_AUC_TGI_adjusted_shift_p), ", Initial-ploidy p: ", format_p(summary_values$cellcycle_AUC_TGI_adjusted_initial_ploidy_p), ", shift-by-ploidy interaction p: ", format_p(summary_values$cellcycle_AUC_TGI_shift_by_ploidy_interaction_p)),
      paste0("- CellCycle AUC-TGI vs sample mean ETP: standardized unadjusted slope ", format_num(summary_values$cellcycle_AUC_TGI_mean_ETP_unadjusted_slope), ", p: ", format_p(summary_values$cellcycle_AUC_TGI_mean_ETP_unadjusted_p), "; dose/ploidy-adjusted p: ", format_p(summary_values$cellcycle_AUC_TGI_mean_ETP_adjusted_p), "; mean-ETP-by-ploidy interaction p: ", format_p(summary_values$cellcycle_AUC_TGI_mean_ETP_by_ploidy_interaction_p)),
      paste0("- CellCycle minus NonCellCycle TGI association delta r: ", format_num(summary_values$compartment_delta_r), ", permutation p: ", format_p(summary_values$compartment_delta_r_perm_p)),
      paste0("- Dose-specific CellCycle AUC TGI 120 minus 30 mg/kg mean difference: ", format_num(summary_values$cellcycle_TGI_AUC_120_minus_30_mean_diff), ", permutation p: ", format_p(summary_values$cellcycle_TGI_AUC_30_vs_120_perm_p)),
      paste0("- Within-dose CellCycle AUC shift-TGI Pearson r, 30 mg/kg: ", format_num(summary_values$cellcycle_TGI_AUC_shift_30mg_pearson_r), ", permutation p: ", format_p(summary_values$cellcycle_TGI_AUC_shift_30mg_perm_p)),
      ...
      paste0("- Within-dose NonCellCycle AUC shift-TGI Pearson r, 120 mg/kg: ", format_num(summary_values$noncellcycle_TGI_AUC_shift_120mg_pearson_r), ", permutation p: ", format_p(summary_values$noncellcycle_TGI_AUC_shift_120mg_perm_p)),
      paste0("- NonCellCycle shift-TGI dose-interaction sensitivity p: ", format_p(summary_values$noncellcycle_shift_TGI_AUC_dose_interaction_p)),
      paste0("- Initial-ploidy response CellCycle 2N 0 vs treated ECDF p: ", format_p(summary_values$cellcycle_2N_treated_vs_control_ecdf_p)),
      paste0("- Initial-ploidy response CellCycle 4N 0 vs treated ECDF p: ", format_p(summary_values$cellcycle_4N_treated_vs_control_ecdf_p)),
      paste0("- Initial-ploidy response NonCellCycle 2N 0 vs treated ECDF p: ", format_p(summary_values$noncellcycle_2N_treated_vs_control_ecdf_p)),
      paste0("- Initial-ploidy response NonCellCycle 4N 0 vs treated ECDF p: ", format_p(summary_values$noncellcycle_4N_treated_vs_control_ecdf_p)),
      paste0("- Treated CellCycle ECDF RMSE 4N minus 2N mean difference: ", format_num(summary_values$cellcycle_ecdf_rmse_4N_minus_2N_mean_diff), ", permutation p: ", format_p(summary_values$cellcycle_ecdf_rmse_4N_minus_2N_perm_p), ", bootstrap CI: [", format_num(summary_values$cellcycle_ecdf_rmse_4N_minus_2N_ci025), ", ", format_num(summary_values$cellcycle_ecdf_rmse_4N_minus_2N_ci975), "], equivalence margin met: ", bool_text(summary_values$cellcycle_ecdf_rmse_equivalence_margin_met)),
      paste0("- Treated AUC TGI 4N minus 2N mean difference: ", format_num(summary_values$tgi_auc_4N_minus_2N_mean_diff), ", permutation p: ", format_p(summary_values$tgi_auc_4N_minus_2N_perm_p), ", bootstrap CI: [", format_num(summary_values$tgi_auc_4N_minus_2N_ci025), ", ", format_num(summary_values$tgi_auc_4N_minus_2N_ci975), "], equivalence margin met: ", bool_text(summary_values$tgi_auc_equivalence_margin_met)),
      paste0("- CellCycle treatment-by-ploidy ECDF interaction p: ", format_p(summary_values$cellcycle_treatment_by_ploidy_ecdf_interaction_p)),
      paste0("- CellCycle TGI ploidy adjusted term p: ", format_p(summary_values$cellcycle_TGI_ploidy_adjusted_term_p)),
      paste0("- CellCycle shift-TGI ploidy interaction p: ", format_p(summary_values$cellcycle_shift_TGI_ploidy_interaction_p)),
      paste0("- Question 2 spline mixed-model time-by-dose-by-ploidy LRT p: ", format_p(summary_values$growth_curve_spline_time_dose_ploidy_lrt_p), "; linear-time sensitivity LRT p: ", format_p(summary_values$growth_curve_linear_time_dose_ploidy_lrt_p)),
    ...
    message("Completed 04h pseudotime/TGI ", analysis_version)
    message("Key outputs:")
    message("  ", file.path(dirs$CellCycle, "stats", "dose_group_ecdf_tests_v5.csv"))
    message("  ", file.path(dirs$CellCycle, "stats", "CellCycle_TGI_associations_v5.csv"))
    message("  ", file.path(dirs$Manuscript, "final_analysis_report.md"))
  })
  quit(save = 'no', status = 0L)
}

if (identical(.ptgi_internal_engine, 'comparison')) {
  local({
    ...
    comprehensive_report <- function(version) {
      ...
      higher_note <- if (is_v42) {
        "120 mg/kg  ETP-higher   dose-by-group  higher-vs-lower   NE  "
      } else if (is_v43) {
        paste0(
          "ETP_reference_balanced_threshold_2_24  2.24  untreated ETP reference  initial 2N  initial 4N ",
          untreated_initial_composition(version),
          "  Dose  initial  ETP   reference  initial   "
        )
      } else {
        " ETP-lower  ETP-higher  30 mg/kg lower  120 mg/kg higher  1   "
      }
      a2_note <- if (is_v42) {
        sprintf(
          paste0(
            "CellCycle DID RMSE=%s p=%s   NonCellCycle  DID RMSE=%s p=%s ",
            " bootstrap CI  ETP-higher  control  treated  1   64  ",
            " 120 mg/kg  /  CellCycle  ETP effect modification "
          ),
        ...
        sprintf(
          paste0(
            ...
            " endpoint-defined group  pseudotime redistribution  "
          ),
          fmt(value(version, "cellcycle_pseudotime_ploidy_did_ecdf_rmse"), 4),
          fmt_p(value(version, "cellcycle_pseudotime_ploidy_did_ecdf_rmse_perm_p")),
          fmt_ci(
      ...
      b3_note <- if (is_v42) {
        "ETP-higher  treated-vs-control treated higher-vs-lower shift/TGI equivalence  spline mixed model  NE NE  “” "
      } else {
        sprintf(
          paste0(
            "treated ETP-higher minus ETP-lower CellCycle shift =%s p=%s CI=%s ",
            "AUC-TGI =%s p=%s CI=%s    "
          ),
      ...
      growth_note <- if (is_v42) {
        sprintf(
          "spline  linear time-by-dose-by-ETP-group LRT  NE Day 38  30 mg/kg DID=%s p=%s  120 mg/kg  endpoint-high  NE ",
      ...
      c(
        sprintf("# %s  EndTimePoint ploidy  pseudotime–TGI ", version),
        "",
        "## 1.  ",
        "",
        sprintf(
          paste0(
            "%s  16  ploidy   %.3f **** ",
            "sample mean ploidy > threshold  ETP-higher  ETP-lower  initial 2N/4N  "
          ), version, threshold
        ),
        "",
        sprintf(" %s ", coverage_sentence(version)),
        higher_note,
        "",
        paste0(
          " (1) gemcitabine  CellCycle pseudotime  ",
          "(2) endpoint-defined ploidy group  TGI  ",
          "(3) pseudotime shift  AUC-TGI  (4) compartment  "
        ),
        "",
        "** estimand  ** ETP  ETP  initial ploidy  ETP group ",
        " ETP group  untreated ECDF reference  ECDF shift  TGI  ",
        " ETP    mediator/collider/selection  ",
        " ETP  endpoint-defined descriptive association / sensitivity analysis  baseline predictive biomarker   ",
        "",
        "## 2. ",
        "",
        sprintf(
          "- CellCycle 0 vs treated p=%s  ETP group  p=%s  CellCycle pseudotime redistribution  ",
          fmt_p(value(version, "cellcycle_treated_vs_control_p")),
          fmt_p(value(version, "cellcycle_treated_vs_control_ploidy_stratified_p"))
        ),
        sprintf(
          "- NonCellCycle 0 vs treated p=%s  redistribution ",
          fmt_p(value(version, "noncellcycle_treated_vs_control_p"))
        ),
        sprintf(
          "- CellCycle shift vs AUC-TGI Pearson r=%s permutation p=%s   n=8 treated  ",
          fmt(value(version, "cellcycle_TGI_AUC_pearson_r"), 3),
          fmt_p(value(version, "cellcycle_TGI_AUC_pearson_perm_p"))
        ),
        sprintf(
          "- 120 minus 30 mg/kg AUC-TGI=%s percentage points p=%s  TGI  ",
        ...
        sprintf(
          paste0(
            "CellCycle sample-aware ECDF test  0-vs-treated p=%s ETP-group-stratified p=%s ",
            "NonCellCycle p=%s  permutation   ",
            " gemcitabine  CellCycle   compartment  pseudotime  "
          ),
          fmt_p(value(version, "cellcycle_treated_vs_control_p")),
          fmt_p(value(version, "cellcycle_treated_vs_control_ploidy_stratified_p")),
          fmt_p(value(version, "noncellcycle_treated_vs_control_p"))
        ...
        "",
        "## 4. A2 ETP group  pseudotime treatment effect ",
        "",
        a2_note,
        " DID  ETP-higher  ETP-lower  treated-minus-control ECDF   endpoint-defined strata  effect modifier ",
        "",
        "## 5. B1/B2 pseudotime shift ",
        "",
        sprintf(
          paste0(
            "treated-only CellCycle ECDF RMSE  AUC-TGI  Pearson r=%s p=%s  dose  ETP group  ",
            "shift term p=%s ETP group term p=%s shift-by-group interaction p=%s ",
            "NonCellCycle  r=%s p=%s compartment delta-r=%s label-swap p=%s "
          ),
          fmt(value(version, "cellcycle_TGI_AUC_pearson_r"), 3),
          fmt_p(value(version, "cellcycle_TGI_AUC_pearson_perm_p")),
        ...
        paste0(
          "“CellCycle state redistribution  response alignment” ",
          " delta-r   CellCycle-specific TGI association ",
          " ETP group   "
        ),
        "",
        sprintf(
          paste0(
            " 30 mg/kg r=%s p=%s 120 mg/kg r=%s p=%s slope-difference p=%s ",
            " treated dose  4   within-dose  "
          ),
        ...
        "",
        "## 6. B3 endpoint group  shift/TGI response ",
        "",
        b3_note,
        "",
        "## 7. B4 ",
        "",
        growth_note,
        " ETP group /   baseline response heterogeneity  ",
        "",
        "## 8.  compartment ",
        "",
        sprintf(
          paste0(
            "CellCycle shift–TGI  leave-one-out  r=%s bootstrap Pearson 95%% interval=%s ",
            " %s  "
          ),
          ...
          if (robust_value(version, "bootstrap_pearson_r_ci025") <= 0) " 0" else " 0 "
        ),
        sprintf(
          "CellCycle fraction  treatment permutation p=%s  compartment abundance  ",
          fmt_p(fraction_p(version))
        ),
        sprintf(
          paste0(
            "Cluster 10  0-vs-treated ECDF RMSE p=%s  shift–TGI r=%s p=%s ",
            " cluster 10   TGI  "
          ), fmt_p(c10_p), fmt(c10_r, 3), fmt_p(c10_tgi_p)
        ),
        "",
        "## 9. C7  endpoint ploidy ",
        "",
        sprintf(
          paste0(
            "ETP_fixed_threshold_2_25 ETP_boundary_stress_threshold_2_375  ETP_reference_balanced_threshold_2_24  C7   CellCycle endpoint mean/median/max ploidy vs AUC-TGI ",
        ...
        paste0(
          " scatter-plot exploration   pooled single-cell r  ",
          "  p  r    treatment timing  "
        ),
        "",
        ...
        sprintf(
          paste0(
            "C4  module-score  pseudotime  control G2M rho=%s E2F/S-phase rho=%s ",
            " pseudotime  module trend  raw single-cell gene dynamics "
          ), fmt(g2m, 3), fmt(e2f, 3)
        ),
        sprintf(
          paste0(
            "C5  targeted 75-gene pseudobulk  genome-wide   DE contrast  padj<0.05  %s/%s/%s ",
            "module enrichment  E2F/S-phase/DNA-replication  q=%s p53 module q=%s  0.05  "
          ),
        ...
        sprintf(
          paste0(
            "C6  MiloR   pseudotime-bin fallback treatment DA  q<0.05  bin =%s ",
            "TGI DA  CellCycle bin 4  n=5 rho=-1  q /  "
        ...
        "### ",
        "",
        "- Gemcitabine  CellCycle tumor-cell pseudotime distribution redistribution  NonCellCycle  shift ",
        "- CellCycle shift–AUC-TGI   ",
        "-  30  120 mg/kg AUC-TGI  ",
        "- ETP group  endpoint-defined  ",
        "",
        "### ",
        "",
        "-  ETP-higher/lower  baseline predictive biomarker ",
        "-  ETP  ploidy-independent causal effect ",
        "-   equivalence CI  ",
        "-  pooled single-cell  ",
        if (is_v42) {
          "-  ETP_boundary_stress_threshold_2_375  NE “” 120 mg/kg endpoint-high  "
        } else if (is_v43) {
          "- ETP_reference_balanced_threshold_2_24  untreated reference  initial-source   30 mg/kg ETP-lower  120 mg/kg ETP-higher   initial 4N "
        } else {
          "- ETP_fixed_threshold_2_25   strata  "
        },
        "",
        "## 12. ",
        "",
        paste0(
          " gemcitabine  cell-cycle-associated tumor states  ",
          " tumor-growth inhibition  Endpoint ploidy   ",
          if (is_v42) {
            "ETP_boundary_stress_threshold_2_375  endpoint-high  2  120 mg/kg   "
          } else if (is_v43) {
            "ETP_reference_balanced_threshold_2_24  untreated ETP reference  initial 2N  initial 4N  endpoint-defined reference-sensitivity   Dose  "
          } else {
            "ETP_fixed_threshold_2_25   endpoint-defined   "
          },
          "  endpoint-ploidy   dose×ETP   "
        ),
        "",
        "## ",
        "",
    ...
    slide_records <- function(version) {
      ...
      records <- list(
        list(
          ...
          zh = sprintf(" pseudotime redistribution tumor response  supporting evidence  16  ploidy  %.3f  ", threshold),
        ...
        list(
          ...
          zh = sprintf("CellCycle 0-vs-treated ECDF p=%s ETP-stratified p=%s violin  endpoint group  dose  ploidy  ", fmt_p(value(version, "cellcycle_treated_vs_control_p")), fmt_p(value(version, "cellcycle_treated_vs_control_ploidy_stratified_p"))),
          ...
          caveat_zh = if (is_v42) "ETP-higher  2  120 mg/kg    " else " dose   dose-by-group  1   "
        ),
        list(
          ...
          zh = sprintf("16  0/30/120 mg/kg  %s ", coverage_sentence(version)),
          ...
          caveat_zh = if (is_v42) "120 mg/kg  ETP-higher  " else "  30-lower  120-higher  1  "
        ),
        list(
          ...
          zh = "endpoint  AUC-based TGI  tumor-growth  ETP  ETP group  TGI  pseudotime  ETP  untreated ECDF reference ",
          ...
          caveat_zh = "TGI  ECDF-shift matching rule   "
        ),
        list(
          ...
          zh = " ECDF  ETP group  untreated reference  untreated  leave-one-control-out reference ",
          ...
          caveat_zh = " reference ECDF  shift metric  TGI  "
        ),
        list(
          ...
          zh = sprintf("CellCycle 0-vs-treated ECDF p=%s  ETP group  p=%s ", fmt_p(value(version, "cellcycle_treated_vs_control_p")), fmt_p(value(version, "cellcycle_treated_vs_control_ploidy_stratified_p"))),
        ...
        list(
          ...
          zh = sprintf("NonCellCycle 0-vs-treated p=%s  redistribution  ", fmt_p(value(version, "noncellcycle_treated_vs_control_p"))),
          ...
          caveat_zh = "comparison compartment   compartment  "
        ),
        list(
          ...
          caveat_zh = if (is_v42) " compartment  CI  endpoint-high   CellCycle  " else "  strata  "
        ),
        list(
        ...
        list(
          ...
          zh = sprintf("AUC-TGI 120-minus-30=%s p=%s  dose  r 30 mg/kg=%s 120 mg/kg=%s ", fmt(value(version, "cellcycle_TGI_AUC_120_minus_30_mean_diff"), 2), fmt_p(value(version, "cellcycle_TGI_AUC_30_vs_120_perm_p")), fmt(value(version, "cellcycle_TGI_AUC_shift_30mg_pearson_r"), 3), fmt(value(version, "cellcycle_TGI_AUC_shift_120mg_pearson_r"), 3)),
          caveat_en = "Each treated-dose stratum contains four samples.", caveat_zh = " treated-dose stratum  4  "
        ),
        list(
          ...
          zh = sprintf("NonCellCycle  shift-TGI r=%s p=%s  dose  r=%s  %s ", fmt(value(version, "noncellcycle_TGI_AUC_pearson_r"), 3), fmt_p(value(version, "noncellcycle_TGI_AUC_pearson_perm_p")), fmt(value(version, "noncellcycle_TGI_AUC_shift_30mg_pearson_r"), 3), fmt(value(version, "noncellcycle_TGI_AUC_shift_120mg_pearson_r"), 3)),
          ...
          caveat_zh = sprintf(" compartment delta-r  p=%s  ", fmt_p(value(version, "compartment_delta_r_perm_p")))
        ),
        list(
          ...
          zh = if (is_v42) {
            sprintf("endpoint-high vs endpoint-low  AUC-TGI  endpoint-TGI trajectory  30  120 mg/kg  TGI Day-38  120-minus-30=%s dose-label permutation p=%s  ", fmt(endpoint_tgi_day38_value(version, "mean_diff_b_minus_a"), 2), fmt_p(endpoint_tgi_day38_value(version, "permutation_p_two_sided")))
          } else {
            sprintf("endpoint-high minus endpoint-low  AUC-TGI=%s p=%s  endpoint-TGI trajectory  30  120 mg/kg  TGI Day-38  120-minus-30=%s dose-label permutation p=%s  ", fmt(value(version, "tgi_auc_ETP_higher_minus_ETP_lower_mean_diff"), 2), fmt_p(value(version, "tgi_auc_ETP_higher_minus_ETP_lower_perm_p")), fmt(endpoint_tgi_day38_value(version, "mean_diff_b_minus_a"), 2), fmt_p(endpoint_tgi_day38_value(version, "permutation_p_two_sided")))
          },
          ...
          caveat_zh = if (is_v42) "treated endpoint-high  1   120 mg/kg  NE  " else " endpoint-TGI trajectory  AUC group comparison   dose  endpoint-group  "
        ),
        list(
          ...
          caveat_zh = " growth curves  ETP_boundary_stress_threshold_2_375  120 mg/kg  "
        ),
        list(
          ...
          zh = sprintf("leave-one-out  r=%s bootstrap 95%% interval=%s ", fmt(robust_value(version, "leave_one_out_min_pearson_r"), 3), fmt_ci(robust_value(version, "bootstrap_pearson_r_ci025"), robust_value(version, "bootstrap_pearson_r_ci975"), 3)),
          caveat_en = "Wide intervals and one influential flag show small-n fragility.", caveat_zh = " influential flag  "
        ),
        list(
          ...
          zh = sprintf(
            "treated CellCycle shift  ploidy  p=%s 30-vs-120 ECDF-shift t-test p=%s  within-dose-centered shift–TGI  r=%s p=%s  ",
            fmt_p(endpoint_ploidy_shift_p(version)), fmt_p(dose_shift_p(version)),
            fmt(within_dose_centered_value(version, "estimate"), 3), fmt_p(within_dose_centered_value(version, "p_value"))
          ),
          caveat_en = "Only eight treated tumors contribute. The figure is an exploratory within-dose-centering sensitivity analysis.",
          caveat_zh = " 8  treated   within-dose centering  "
        ),
        list(
        ...
        list(
          ...
          caveat_zh = "CellCycle  treatment distribution shift   TGI correlation difference  "
        ),
        list(
          ...
          zh = sprintf(" control trend G2M rho=%s E2F/S-phase rho=%s targeted pseudobulk  E2F module q=0.0198 ", fmt(module_estimate(version, "G2M_checkpoint", "score_vs_pseudotime_control"), 3), fmt(module_estimate(version, "E2F_S_phase_DNA_replication", "score_vs_pseudotime_control"), 3)),
          caveat_en = "The 75-gene panel and pseudotime aggregation do not establish raw single-cell gene dynamics.",
          caveat_zh = "75-gene panel  pseudotime aggregation  raw single-cell gene dynamics "
        )
      )
    ...
    slide_interpretation_bilingual <- function(version) {
      recs <- slide_records(version)
      lines <- c(
        sprintf("# %s slide-by-slide interpretation / ", version),
        "",
        "This file pairs concise English narration with a Chinese scientific interpretation.",
        " ",
        "",
        "Global guardrail / : endpoint ploidy is post-treatment; ETP is endpoint-defined descriptive/sensitivity analysis, not baseline predictive-biomarker evidence. /  ploidy  ETP /  ",
        ""
      )
      for (i in seq_along(recs)) {
        r <- recs[[i]]
    ...
    readme <- c(
      ...
      "",
      "`Code/in-vivo/04h_pseudotime_TGI.R`",
      "",
      "## Files",
      "",
...
# Standalone 04h workflow. The complete initial-ploidy, ETP-threshold,
# mean-ETP, robustness, growth-response, and method-comparison engines live in
# this file. Only method-neutral helpers are loaded from Utils.R.
...
if (!file.exists(utils_path)) stop("Missing shared utilities: ", utils_path, call. = FALSE)
sys.source(utils_path, envir = .GlobalEnv)

cmd_args <- ptgi_parse_cli_args(commandArgs(trailingOnly = TRUE))
repo_root <- normalizePath(file.path(script_dir, "..", ".."), mustWork = FALSE)

arg_or <- function(name, default) {
  ptgi_arg(cmd_args, name, default)
}

normalize_choice <- ptgi_normalize_choice
...
ptgi_ensure_dir(results_root)
workflow_log_dir <- file.path(results_root, "workflow_logs")
ptgi_ensure_dir(workflow_log_dir)
  ...
  status_rows <- if (method_workers > 1L && length(selected_methods) > 1L) {
    psock_lapply(
      ...
      exports = c(
        ...
        "initial_method", "common_args", "results_root", "analysis_script"
      )
    )
  } else {
...
if (run_comparison) {
  comparison_status <- run_child(
    "method_comparison",
    analysis_script,
    c("--internal_engine=comparison", paste0("--results_root=", results_root))
  )

...
plot_sample_histogram_16x1 <- function(hist_df, title, file_stub, height = NULL) {
  ...
  p <- ggplot(hist_df, aes(x = .data$time_bin_mid, y = .data$cell_percent, fill = .data$Ploidy, color = .data$Ploidy)) +
    ...
    labs(
      title = title,
      subtitle = "Tumor cells from scVelo all_cells; clusters 4c, 6, and 10 only. Y axis is percent of selected cells within each sample.",
      x = "scVelo pseudotime",
      y = "Cell percent",
      fill = "Initial Ploidy",
...
plot_dose_histogram_panels <- function(hist_df, title, file_stub, sample_info, sample_order) {
  ...
  p <- patchwork::wrap_plots(dose_plots, nrow = 1, guides = "collect") +
    patchwork::plot_annotation(
      title = title,
      subtitle = "Tumor cells from scVelo all_cells; clusters 4c, 6, and 10 only."
    )
...
plot_density_curves <- function(density_df, title, file_stub, facet_by_dose = FALSE) {
  ...
  p <- ggplot(density_df, aes(x = .data$pseudotime, y = .data$density, group = .data$sampleID, color = .data$mean_cell_ploidy)) +
    ...
    labs(
      title = title,
      subtitle = "One density curve per sample; Tumor cells from scVelo all_cells; clusters 4c, 6, and 10 only.",
      x = "scVelo pseudotime",
      y = "Density"
    ) +
    theme_04f(10) +
...
plot_initial_ploidy_cell_response <- function(df, file_stub) {
  ...
  p <- ggplot(plot_df, aes(x = .data$dose_x, y = .data$pseudotime, fill = .data$Ploidy, group = interaction(.data$DosePanel, .data$Ploidy))) +
    ...
    scale_fill_manual(values = c("2N" = "#3B73B9", "4N" = "#D95F02"), drop = FALSE) +
    labs(
      title = "scVelo all_cells-derived Tumor clusters 4c/6/10 pseudotime by Initial Ploidy and Dose",
      x = "Dose (mg/kg)",
      y = "scVelo pseudotime",
      fill = "Initial Ploidy"
    ) +
...
plot_sample_level_response <- function(sample_df, file_stub) {
  ...
  p <- ggplot(plot_df, aes(x = .data$dose_x, y = .data$value, color = .data$Ploidy, group = interaction(.data$DosePanel, .data$Ploidy))) +
    ...
    scale_color_manual(values = c("2N" = "#1D4E89", "4N" = "#A84600"), drop = FALSE) +
    labs(
      title = "Sample-level Dose response in Tumor clusters 4c/6/10",
...
cell_cycle_results_root <- get_env_scalar(
  "CELL_CYCLE_RESULTS_SHARED_ROOT",
  file.path(results_root, "04f_cell_cycle_results")
)
...
output_base <- get_env_scalar(
  "CELL_CYCLE_RESULTS_ROOT",
  file.path(results_root, "04f_cell_cycle_results", scenario_id)
)
target_clusters <- strsplit(get_env_scalar("TARGET_CLUSTERS", "4c,6,10"), ",", fixed = TRUE)[[1]]
...
cell_cycle_results_root <- ensure_dir_04f(cell_cycle_results_root)
fig4_tgi_export_file <- file.path(
  cell_cycle_results_root,
  "fig4_pseudotime_distribution_per_sample_cell_level_with_ploidy_dose_tgi.csv"
)
tgi_process_file <- file.path(cell_cycle_results_root, "TGI_calculation_process.md")

message("04f cell cycle results")
...
message("  TGI calculation process: ", tgi_process_file)
message("  target clusters: ", paste(target_clusters, collapse = ", "))
...
if (nrow(target_df) == 0) {
  stop("No Tumor cells in target clusters were found after filtering all_cells scVelo metrics.", call. = FALSE)
}
...
analysis_manifest <- data.frame(
  ...
  tgi_process_file = tgi_process_file,
  source_object = "scVelo All_cells",
  extracted_tn = "Tumor",
  target_clusters = paste(target_clusters, collapse = ","),
...
filter_summary <- dplyr::bind_rows(
  ...
  target_df %>% dplyr::summarise(filter = "all_cells_scVelo_Tumor_clusters_4c_6_10", n_cells = dplyr::n(), n_samples = dplyr::n_distinct(.data$sampleID))
)
...
write_text_04f(
  make_in_vivo_tgi_process_markdown(
    ...
    analysis_label = "04f cell-cycle results",
    target_cell_description = "target Tumor cell from clusters 4c, 6, or 10",
    cluster_description = paste0("Tumor clusters ", paste(target_clusters, collapse = ", ")),
    code_location = "/Users/4482173/Documents/GitHub/Gemcitabine-model/Code/in-vivo/04f_cell_cycle_results.R",
...
plot_sample_histogram_16x1(
  hist_reference,
  "scVelo all_cells-derived Tumor clusters 4c/6/10 sample pseudotime histograms",
  file.path(out_hist, "all_samples_pseudotime_frequency_histograms_16x1")
)
plot_sample_histogram_16x1(
  hist_ploidy_order,
  "scVelo all_cells-derived Tumor clusters 4c/6/10 sample pseudotime histograms ordered by mean Cell Ploidy",
...
plot_density_curves(
  density_df,
  "scVelo all_cells-derived Tumor clusters 4c/6/10 sample pseudotime density",
...
p_fig1 <- ggplot(umap_base, aes(x = .data$umap1, y = .data$umap2, color = .data$pseudotime)) +
  ...
  labs(title = "Tumor cells only; clusters 4c, 6, and 10; scVelo all-cells-derived pseudotime", x = "UMAP 1", y = "UMAP 2") +
  coord_equal() +
  theme_04f(9)
...
p_fig2 <- ggplot(umap_base, aes(x = .data$umap1, y = .data$umap2, color = .data$cluster_target)) +
  ...
  facet_wrap(~initial_ploidy_group, nrow = 1) +
  scale_color_manual(values = c("4c" = "#7B3294", "6" = "#008837", "10" = "#E66101"), drop = FALSE) +
  labs(title = "Tumor cells only; clusters 4c, 6, and 10; cluster annotation", x = "UMAP 1", y = "UMAP 2", color = "Cluster") +
  coord_equal() +
  theme_04f(9)
...
p_fig3 <- ggplot(umap_base, aes(x = .data$umap1, y = .data$umap2, color = .data$cell_ploidy)) +
  ...
  scale_color_gradientn(colors = c("#313695", "#74ADD1", "#FEE08B", "#F46D43", "#A50026"), name = "Cell Ploidy") +
  labs(title = "Tumor cells only; clusters 4c, 6, and 10; Cell Ploidy by dose", x = "UMAP 1", y = "UMAP 2") +
  coord_equal() +
  theme_04f(8)
...
p_fig3_time <- ggplot(umap_base, aes(x = .data$umap1, y = .data$umap2, color = .data$pseudotime)) +
  ...
  facet_grid(initial_ploidy_group ~ dose) +
  scale_color_viridis_c(option = "magma", name = "Pseudotime") +
  labs(title = "Tumor cells only; clusters 4c, 6, and 10; scVelo pseudotime by dose", x = "UMAP 1", y = "UMAP 2") +
  coord_equal() +
  theme_04f(8)
...
p_fig4_counts <- ggplot(counts_by_sample_cluster, aes(x = .data$sample_id, y = .data$n_target_cells, fill = .data$cluster_target)) +
  ...
  scale_fill_manual(values = c("4c" = "#7B3294", "6" = "#008837", "10" = "#E66101"), drop = FALSE) +
...
sample_metric_long <- sample_pseudotime_metrics %>%
  dplyr::select("sample_id", "initial_ploidy_group", "dose", "n_cells", "mean_pseudotime", "median_pseudotime", "early_fraction", "middle_fraction", "late_fraction", "pseudotime_entropy", "fraction_cluster_4c", "fraction_cluster_6", "fraction_cluster_10") %>%
...
p_fig7_cluster_ploidy <- ggplot(target_meta_dose_plot, aes(x = .data$dose_x, y = .data$cell_ploidy, fill = .data$initial_ploidy_group, group = interaction(.data$dose, .data$initial_ploidy_group))) +
  ...
  facet_wrap(~cluster_target, nrow = 1) +
  dose_scale_04f() +
...
if (identical(h5res$status, "ok")) {
  ...
  p_sig_annot_ploidy <- ggplot(sig_annotation_ploidy, aes(x = .data$annotation, y = .data$row_id, fill = .data$initial_ploidy_group)) +
    ...
    scale_fill_manual(values = ploidy_annotation_colors, drop = FALSE, name = "Initial Ploidy", guide = guide_legend(ncol = 1, byrow = TRUE)) +
  ...
  p_sig_annot_dose <- ggplot(sig_annotation_dose, aes(x = .data$annotation, y = .data$row_id, fill = .data$dose)) +
    geom_tile(color = "white", linewidth = 0.15) +
    scale_fill_manual(values = dose_colors, drop = FALSE, name = "Dose", guide = guide_legend(ncol = 1, byrow = TRUE)) +
    scale_y_discrete(limits = rev(sig_row_order), expand = c(0, 0)) +
    scale_x_discrete(position = "top") +
    labs(x = NULL, y = NULL) +
    theme_04f(6) +
  ...
  p_sig_heat_main <- ggplot(sig_heat, aes(x = .data$pseudotime_bin_10, y = .data$row_id, fill = .data$mean_score)) +
    geom_tile(color = "white", linewidth = 0.15) +
    scale_fill_gradient2(low = "#313695", mid = "white", high = "#A50026", midpoint = 0, name = "Score", guide = guide_colorbar(direction = "vertical", barheight = grid::unit(22, "mm"), barwidth = grid::unit(4, "mm"))) +
  ...
  if (identical(gsea_input$status, "ok")) {
    ...
    if (nrow(gsea_nes) > 0) {
      ...
      p_gsea_heat_main <- ggplot(gsea_heat, aes(x = .data$pseudotime_bin_10, y = .data$row_id, fill = .data$NES)) +
        geom_tile(color = "white", linewidth = 0.15) +
        scale_fill_gradient2(low = "#313695", mid = "white", high = "#A50026", midpoint = 0, limits = c(-nes_limit, nes_limit), name = "NES", guide = guide_colorbar(direction = "vertical", barheight = grid::unit(22, "mm"), barwidth = grid::unit(4, "mm"))) +
        scale_y_discrete(limits = rev(gsea_row_order), expand = c(0, 0)) +
...
key_results <- data.frame(
  ...
  main_result = c(
    ...
    ifelse(identical(h5res$status, "ok"), "Signature scores were computed from matched genes in h5ad X; pseudobulk DESeq2 was skipped because X was not integer-like.", "Signature analysis was skipped; see logs/signature_analysis_status.tsv.")
  ),
  supporting_figure = c(
    ...
    "figures/pdf/fig11_signature_scores_heatmap.pdf"
  ),
  supporting_table = c(
    ...
    "stats/signature_score_models.tsv"
  ),
  model = c(
    ...
    ifelse(identical(h5res$status, "ok"), "WLS signature models", "Not run")
  ),
  ...
  interpretation = c(
    ...
    "Composition effect suggests cell-cycle state redistribution; within-state effect suggests ploidy change after pseudotime matching.",
...
report_lines <- c(
  "# Downstream scVelo Tumor Cell-Cycle Trajectory Analysis",
  "",
  ...
  "The selected table is the project-confirmed 04e merged scVelo all-cells output with Cell Ploidy annotations. Tumor cells were extracted from this all-cells table and then restricted to clusters 4c, 6, and 10.",
  "",
  ...
  ifelse(
    ...
    "Signature scores were computed from h5ad `X` for matched genes in Gemcitabine metabolism, S phase/DNA replication, replication stress, DNA damage response, G2/M checkpoint, apoptosis, and senescence/p53 response signatures. h5ad `X` did not appear integer-like, so DESeq2 pseudobulk DE was skipped and signature-score models are the primary expression-level analysis.",
    paste0("Signature analysis was skipped. Status: ", h5res$status, ". See `logs/signature_analysis_status.tsv`.")
  ),
  "",
  "## 9. Main Conclusions",
  "",
...
readme_lines <- c(
  "# 04f cell-cycle downstream results",
  "",
  "This directory contains the full prompt-driven downstream analysis for scVelo all-cells-derived Tumor cells in clusters 4c, 6, and 10.",
  "",
...
write_text_04f(
  c(
    "Rscript Code/in-vivo/04f_cell_cycle_results.R",
    "",
...
write_csv_04f(status, file.path(output_base, "04f_cell_cycle_results_status.csv"))

message("04f cell cycle results completed.")
message("Output root: ", output_base)
message("Target Tumor cells in clusters ", paste(target_clusters, collapse = ", "), ": ", nrow(target_df))

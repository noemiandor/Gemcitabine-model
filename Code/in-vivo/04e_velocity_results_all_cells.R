...
input_analysis_objects <- "All_cells"
...
analysis_object_display_label <- function(analysis_object) {
  out <- as.character(analysis_object)
  out[out == "All_cells"] <- "all_cells"
  out[out == "CellLine/ploidy_all"] <- "all_cells, CellLine"
  out[out == "Tumor/ploidy_all"] <- "all_cells, Tumor"
...
standardize_tn_label <- function(x) {
  out <- as_clean_chr(x)
  out <- trimws(out)
  lower <- tolower(out)
...
infer_tn_from_ids_column <- function(x, default_non_cell_culture_to_tumor = TRUE) {
  x <- as_clean_chr(x)
  out <- rep(NA_character_, length(x))
  has_value <- !is.na(x) & nzchar(x)
  out[has_value & grepl("Cell-Culture", x, fixed = TRUE)] <- "CellLine"
  if (isTRUE(default_non_cell_culture_to_tumor)) {
    out[has_value & is.na(out)] <- "Tumor"
  } else {
    out[has_value & grepl("Tumor", x, ignore.case = TRUE)] <- "Tumor"
    out[has_value & grepl("CellLine|Cell.Line|Cell_Culture|Cell-Culture", x, ignore.case = TRUE)] <- "CellLine"
  }
  out
}

derive_tn_split <- function(df) {
  tn <- rep(NA_character_, nrow(df))
  source <- rep(NA_character_, nrow(df))

  explicit_cols <- c("TN", "SampleOrigin", "trajectory_context", "trajectory_tn_scope", "sample_type")
  for (candidate in explicit_cols) {
    if (candidate %in% names(df)) {
      values <- standardize_tn_label(df[[candidate]])
  ...
  if ("IDs" %in% names(df)) {
    values <- infer_tn_from_ids_column(df$IDs, default_non_cell_culture_to_tumor = TRUE)
    fill <- (is.na(tn) | !nzchar(tn)) & !is.na(values) & nzchar(values)
    tn[fill] <- values[fill]
    source[fill] <- "IDs"
  }
  ...
  for (candidate in fallback_cols) {
    if (candidate %in% names(df)) {
      values <- infer_tn_from_ids_column(df[[candidate]], default_non_cell_culture_to_tumor = FALSE)
      fill <- (is.na(tn) | !nzchar(tn)) & !is.na(values) & nzchar(values)
      tn[fill] <- values[fill]
      source[fill] <- candidate
    }
...
load_method_metrics <- function(file, method_label, analysis_object, time_col) {
  ...
  tn_split <- derive_tn_split(df)
  out <- df %>%
    dplyr::mutate(
      method = method_label,
      analysis_object = analysis_object,
      analysis_object_safe = safe_file_component(analysis_object),
      analysis_object_display = analysis_object_display_label(analysis_object),
      input_analysis_object = analysis_object,
      ...
      TN = tn_split$TN,
      tn_split_source = tn_split$tn_split_source
    )
...
metric_manifest <- function(results_root) {
  data.frame(
    method = c("scVelo", "PAGA", "Monocle3"),
    analysis_object = rep(input_analysis_objects, times = 3L),
    time_col = c("velocity_pseudotime", "dpt_pseudotime", "pseudotime"),
    input_file = c(
      file.path(results_root, "04_trajectory", scenario_id, "scvelo", "01_velocity_groups", input_analysis_objects, "scvelo_cell_metrics.csv"),
      file.path(results_root, "04_trajectory", scenario_id, "PAGA", "04_paga_groups", input_analysis_objects, "paga_cell_metrics.csv"),
      file.path(results_root, "04a_psudo_rajectory", monocle3_root_id, "01_pseudotrajectory_groups", input_analysis_objects, "cells_pseudotime.csv")
    ),
    stringsAsFactors = FALSE
  ) %>%
    dplyr::mutate(
...
update_analysis_object_fields <- function(df, analysis_object) {
  object_parts <- strsplit(analysis_object, "/", fixed = TRUE)[[1]]
  df$analysis_object <- analysis_object
  df$analysis_object_safe <- safe_file_component(analysis_object)
  df$analysis_object_display <- analysis_object_display_label(analysis_object)
  df$analysis_tn_scope <- object_parts[1]
  df$analysis_ploidy_scope <- if (length(object_parts) > 1) object_parts[2] else "all"
  df
}

expand_all_cells_analysis_objects <- function(metrics) {
  if (nrow(metrics) == 0) return(metrics)
  if (!("TN" %in% names(metrics))) {
    stop("Cannot derive CellLine/Tumor analysis objects because TN is missing.", call. = FALSE)
  }
  resolved_tn <- metrics$TN %in% c("CellLine", "Tumor")
  if (!any(resolved_tn)) {
    stop("Cannot derive CellLine/Tumor analysis objects because no all_cells rows have TN = CellLine or Tumor.", call. = FALSE)
  }
  if (any(!resolved_tn)) {
    warning(sum(!resolved_tn), " all_cells metric rows have unresolved TN and will only remain in All_cells.", call. = FALSE)
  }

  rows <- list(update_analysis_object_fields(metrics, "All_cells"))
  for (tn_label in c("CellLine", "Tumor")) {
    ...
    rows[[length(rows) + 1L]] <- update_analysis_object_fields(sub, paste0(tn_label, "/ploidy_all"))
  }
  dplyr::bind_rows(rows)
}

summarise_all_cells_tn_split <- function(metrics) {
  metrics %>%
    dplyr::mutate(
      TN = dplyr::if_else(.data$TN %in% c("CellLine", "Tumor"), .data$TN, "Unresolved"),
      tn_split_source = dplyr::if_else(!is.na(.data$tn_split_source) & nzchar(.data$tn_split_source), .data$tn_split_source, "unresolved")
    ) %>%
    dplyr::count(.data$method, .data$input_analysis_object, .data$TN, .data$tn_split_source, name = "n_cells") %>%
    dplyr::arrange(.data$method, .data$TN, .data$tn_split_source)
}
...
plot_coverage90_core_umaps <- function(df, added_dir, method_label, object_display_label, filter_name) {
  dat <- df %>%
    dplyr::mutate(
      Ploidy = factor(as.character(.data$annotation_ploidy), levels = c("2N", "4N")),
      Dose = factor(as.character(.data$Dose), levels = sort_maybe_numeric(unique(as.character(.data$Dose)))),
      pseudotime = safe_num(.data$pseudotime),
      cell_ploidy = safe_num(.data$cell_ploidy),
      UMAP_1 = safe_num(.data$UMAP_1),
      UMAP_2 = safe_num(.data$UMAP_2),
    ...
    dplyr::filter(
      ...
      !is.na(.data$clusters),
      nzchar(.data$clusters)
    )
  if (nrow(dat) == 0) {
    return(data.frame(
      coverage_filter = filter_name,
      plot_key = NA_character_,
      output_pdf = NA_character_,
      status = "skipped_no_umap_cluster_data",
      n_points = 0L,
      n_cluster_tiles = 0L,
      n_cluster_labels = 0L,
      stringsAsFactors = FALSE
  ...
  plot_specs <- list(
    list(
      key = "pseudotime_by_dose",
      legacy_stub = paste0(filter_name, "_cell_ploidy_time_by_dose_scatter"),
      alias_stub = paste0(filter_name, "_cluster_pseudotime_umap_by_dose"),
      time_col = "pseudotime",
      split_cols = c("Dose"),
      label = paste(method_label, object_display_label, filter_name, "cluster pseudotime UMAP by Dose"),
      fill_label = paste0("Mean ", method_label, "\npseudotime")
    ),
    list(
      key = "pseudotime_by_ploidy_dose",
      legacy_stub = paste0(filter_name, "_cluster_pseudotime_umap_by_ploidy_dose"),
      alias_stub = NA_character_,
      time_col = "pseudotime",
      split_cols = c("Ploidy", "Dose"),
      label = paste(method_label, object_display_label, filter_name, "2N and 4N cluster pseudotime UMAP by Dose"),
      fill_label = paste0("Mean ", method_label, "\npseudotime")
    ),
    list(
      key = "cell_ploidy_by_dose",
      legacy_stub = paste0(filter_name, "_tumor_umap_dose_cell_ploidy"),
      alias_stub = paste0(filter_name, "_cluster_cell_ploidy_umap_by_dose"),
      time_col = "cell_ploidy",
      split_cols = c("Dose"),
      label = paste(method_label, object_display_label, filter_name, "cluster mean cell ploidy UMAP by Dose"),
      fill_label = "Mean cell\nploidy"
    ),
    list(
      key = "pseudotime_by_ploidy",
      legacy_stub = paste0(filter_name, "_cluster_pseudotime_umap_by_ploidy"),
      alias_stub = NA_character_,
      time_col = "pseudotime",
      split_cols = c("Ploidy"),
      label = paste(method_label, object_display_label, filter_name, "2N and 4N cluster pseudotime UMAP"),
      fill_label = paste0("Mean ", method_label, "\npseudotime")
    ),
    list(
      key = "cell_ploidy_by_ploidy",
      legacy_stub = paste0(filter_name, "_cluster_cell_ploidy_umap_by_ploidy"),
      alias_stub = NA_character_,
      time_col = "cell_ploidy",
      split_cols = c("Ploidy"),
      label = paste(method_label, object_display_label, filter_name, "2N and 4N cluster mean cell ploidy UMAP"),
      fill_label = "Mean cell\nploidy"
    ),
    list(
      key = "cell_ploidy_by_ploidy_dose",
      legacy_stub = paste0(filter_name, "_cell_ploidy_time_by_dose_ploidy_scatter"),
      alias_stub = paste0(filter_name, "_cluster_cell_ploidy_umap_by_ploidy_dose"),
      time_col = "cell_ploidy",
      split_cols = c("Ploidy", "Dose"),
      label = paste(method_label, object_display_label, filter_name, "2N and 4N cluster mean cell ploidy UMAP by Dose"),
      fill_label = "Mean cell\nploidy"
    )
  )
  ...
  for (i in seq_along(plot_specs)) {
    spec <- plot_specs[[i]]
    plot_df <- dat %>%
      dplyr::filter(is.finite(.data[[spec$time_col]])) %>%
      dplyr::filter(dplyr::if_all(dplyr::all_of(spec$split_cols), ~ !is.na(.x) & nzchar(as.character(.x))))
    legacy_stub <- file.path(added_dir, spec$legacy_stub)
    if (nrow(plot_df) == 0) {
      status_rows[[i]] <- data.frame(
        coverage_filter = filter_name,
        plot_key = spec$key,
        output_pdf = paste0(legacy_stub, ".pdf"),
    ...
    res <- plot_cluster_time_core_umap(
      plot_df,
      label = spec$label,
      output_stub = legacy_stub,
      time_col = spec$time_col,
      split_cols = spec$split_cols,
      fill_label = spec$fill_label
    )
    if (!is.na(spec$alias_stub)) {
      alias_stub <- file.path(added_dir, spec$alias_stub)
      file.copy(paste0(legacy_stub, ".pdf"), paste0(alias_stub, ".pdf"), overwrite = TRUE)
      file.copy(paste0(legacy_stub, ".png"), paste0(alias_stub, ".png"), overwrite = TRUE)
    }
    status_rows[[i]] <- data.frame(
      coverage_filter = filter_name,
      plot_key = spec$key,
      output_pdf = paste0(legacy_stub, ".pdf"),
      status = if (nrow(res) > 0) res$status[1] else "ok",
      n_points = if (nrow(res) > 0) res$n_points[1] else nrow(plot_df),
      n_cluster_tiles = if (nrow(res) > 0) res$n_cluster_tiles[1] else NA_integer_,
      n_cluster_labels = if (nrow(res) > 0) res$n_cluster_labels[1] else NA_integer_,
      stringsAsFactors = FALSE
    )
  }
...
run_tumor_cell_ploidy_time_added_outputs <- function(df, out_dir, method_label, object_display_label) {
  ...
  for (filter_name in names(filters)) {
    ...
    umap_pdf <- file.path(added_dir, paste0(filter_name, "_tumor_umap_dose_cell_ploidy.pdf"))
    if (identical(filter_name, "coverage90")) {
      core_umap_status <- plot_coverage90_core_umaps(
        dat,
        added_dir = added_dir,
        method_label = method_label,
        object_display_label = object_display_label,
        filter_name = filter_name
      )
      write_csv_safe(core_umap_status, file.path(added_dir, paste0(filter_name, "_cluster_core_umap_status.csv")))
      p1 <- NULL
      p2 <- NULL
      umap_status <- data.frame(
        output_pdf = umap_pdf,
        n_cells = nrow(dat),
        n_with_cell_ploidy = sum(is.finite(dat$cell_ploidy), na.rm = TRUE),
        n_with_dose = sum(!is.na(dat$Dose) & nzchar(as.character(dat$Dose)), na.rm = TRUE),
        status = paste0("core_umap_", paste(unique(core_umap_status$status), collapse = ";")),
        stringsAsFactors = FALSE
      )
    } else {
      p1 <- plot_cell_ploidy_time_by_dose(
        ...
        title = paste(method_label, object_display_label, "cell ploidy vs pseudotime by Dose:", filter_name)
      )
      p2 <- plot_cell_ploidy_time_by_dose(
        dat,
        split_by_ploidy = TRUE,
        title = paste(method_label, object_display_label, "cell ploidy vs pseudotime by Dose and 2N/4N:", filter_name)
      )
      ...
      umap_status <- plot_tumor_dose_ploidy_umap_pair(
        dat,
        title = paste(method_label, object_display_label, filter_name),
        output_pdf = umap_pdf
      )
    }
    status_rows[[filter_name]] <- data.frame(
      ...
      scatter_by_dose_pdf = if (identical(filter_name, "coverage90") || !is.null(p1)) scatter_by_dose else NA_character_,
      scatter_by_dose_ploidy_pdf = if (identical(filter_name, "coverage90") || !is.null(p2)) scatter_by_dose_ploidy else NA_character_,
...
run_trajectory_direction_consistency_outputs <- function(dat, out_dir, root_cluster = "6") {
  ...
  trend_df <- dat %>%
    ...
    dplyr::mutate(
      analysis_object_display = analysis_object_display_label(.data$analysis_object),
  ...
  root_summary <- trend_df %>%
    ...
    dplyr::select(analysis_object, analysis_object_display, method, clusters, n_cells, median_pseudotime, cluster_rank, n_clusters, rank_percentile)
  ...
  p_rank <- ggplot(plot_df, aes(x = .data$method, y = .data$rank_percentile, group = .data$clusters, color = .data$clusters)) +
    ...
    facet_wrap(~analysis_object_display, nrow = 1) +
    labs(
...
run_cross_method_comparison_outputs <- function(metrics, output_base) {
  ...
  for (obj in unique(dat$analysis_object)) {
    ...
    obj_display <- unique(obj_df$analysis_object_display)
    obj_display <- if (length(obj_display) == 1 && !is.na(obj_display) && nzchar(obj_display)) obj_display else analysis_object_display_label(obj)
    obj_dir <- .ensure_dir(file.path(out_dir, safe_file_component(obj)))

    cluster_plot <- plot_method_violin_box(
      obj_df,
      "clusters",
      paste(obj_display, "cluster pseudotime by method"),
      x_label = "Cluster"
    )
    ploidy_plot <- plot_method_violin_box(
      obj_df %>% dplyr::filter(.data$annotation_ploidy %in% c("2N", "4N")),
      "annotation_ploidy",
      paste(obj_display, "2N/4N pseudotime by method"),
      x_label = "Ploidy"
    )
    dose_plot <- plot_method_violin_box(
      obj_df %>% dplyr::filter(!is.na(.data$Dose), nzchar(as.character(.data$Dose))),
      "Dose",
      paste(obj_display, "Dose pseudotime by method"),
      x_label = "Dose"
    )
    dose_ploidy_plot <- plot_method_violin_box(
      ...
      "Dose",
      paste(obj_display, "Dose pseudotime by method within 2N/4N"),
      x_label = "Dose",
    ...
    object_status[[obj]] <- data.frame(
      analysis_object = obj,
      analysis_object_display = obj_display,
      n_rows = nrow(obj_df),
...
run_tumor_ploidy_dose_response_outputs <- function(metrics, output_base) {
  ...
  tumor_display <- analysis_object_display_label("Tumor/ploidy_all")
  dat <- metrics %>%
    dplyr::filter(
  ...
  cell_plot <- ggplot(dat, aes(x = .data$Dose, y = .data$pseudotime, fill = .data$method)) +
    ...
    scale_fill_manual(values = method_colors, drop = FALSE, name = "Method") +
    labs(title = paste(tumor_display, "pseudotime response to Dose within 2N and 4N"), x = "Dose", y = "Pseudotime") +
    theme_extra(base_size = 10)
  ...
  sample_plot <- ggplot(sample_summary, aes(x = .data$Dose, y = .data$sample_median_pseudotime, color = .data$method, group = .data$method)) +
    ...
    labs(title = paste(tumor_display, "sample-median pseudotime response to Dose within 2N and 4N"), x = "Dose", y = "Sample median pseudotime") +
    theme_extra(base_size = 10)
  ...
  if (nrow(continuous_dat) > 0) {
    continuous_scatter <- ggplot(continuous_dat, aes(x = .data$cell_ploidy, y = .data$pseudotime, color = .data$annotation_ploidy)) +
      ...
      labs(title = paste(tumor_display, "continuous cell ploidy vs pseudotime by method and Dose"), x = "Continuous cell ploidy", y = "Pseudotime") +
  ...
  status <- data.frame(
    ...
    analysis_object_display = tumor_display,
...
run_scvelo_deep_dive_outputs <- function(metrics, output_base, results_root) {
  ...
  tumor_display <- analysis_object_display_label("Tumor/ploidy_all")
  scvelo_tumor_title <- paste("scVelo", tumor_display)
  dat <- metrics %>%
    dplyr::filter(
      .data$method == "scVelo",
      .data$analysis_object == "Tumor/ploidy_all",
  ...
  p_delta <- ggplot(plot_shift, aes(x = .data$clusters, y = .data$delta_median_4N_minus_2N, color = .data$time_shift_direction)) +
    ...
    labs(
      title = paste0(scvelo_tumor_title, ": 4N versus 2N median pseudotime shift by cluster"),
  ...
  p_violin <- ggplot(plot_dat, aes(x = .data$clusters, y = .data$pseudotime, fill = .data$annotation_ploidy)) +
    ...
    labs(
      title = paste0(scvelo_tumor_title, ": 2N and 4N pseudotime distributions by cluster"),
      x = "Cluster ordered by scVelo median pseudotime",
      y = "Pseudotime"
    ) +
    theme_extra(base_size = 10) +
  ...
  if (nrow(gsea_ploidy) > 0) {
    ...
    if (nrow(heat_df) > 0) {
      p_gsea <- ggplot(heat_df, aes(x = .data$cluster, y = .data$hallmark_label, fill = .data$NES)) +
        ...
        labs(
          title = paste("Tumor-only 4N vs 2N Hallmark GSEA across", scvelo_tumor_title, "ordered clusters"),
          x = "Cluster ordered by scVelo median pseudotime",
          y = "Hallmark pathway"
        ) +
        theme_extra(base_size = 9) +
  ...
  p_annotation <- ggplot(annotation_plot_df, aes(x = .data$rank_percentile, y = .data$delta_median_4N_minus_2N, color = .data$time_shift_direction)) +
    ...
    scale_color_manual(values = c("4N_advanced" = "#2C7FB8", "4N_delayed" = "#D95F0E", "similar" = "grey50", "insufficient" = "grey70"), drop = FALSE) +
    labs(
      title = paste(scvelo_tumor_title, "4N time shift over cluster trajectory with 03b primary annotations"),
  ...
  if (nrow(continuous_dat) > 0) {
    p_ploidy_cluster_dose <- ggplot(continuous_dat, aes(x = .data$clusters, y = .data$cell_ploidy, fill = .data$Dose)) +
      ...
      labs(
        title = paste0(scvelo_tumor_title, ": continuous cell ploidy by cluster and Dose"),
        x = "Cluster ordered by scVelo median pseudotime",
  ...
  if (nrow(time_bin_dose_summary) > 0) {
    p_late_fraction <- ggplot(time_bin_dose_summary, aes(x = .data$Dose, y = .data$high_cell_ploidy_fraction, color = .data$time_bin, group = .data$time_bin)) +
      ...
      labs(
        title = paste(scvelo_tumor_title, "high continuous-ploidy fraction across early/middle/late clusters by Dose"),
        x = "Dose",
    ...
    p_mean_timebin <- ggplot(time_bin_dose_summary, aes(x = .data$Dose, y = .data$mean_cell_ploidy, color = .data$time_bin, group = .data$time_bin)) +
      ...
      scale_color_manual(values = c("early" = "#1B9E77", "middle" = "#7570B3", "late" = "#D95F02"), drop = FALSE) +
      labs(
        title = paste(scvelo_tumor_title, "mean continuous cell ploidy across early/middle/late clusters by Dose"),
        x = "Dose",
  ...
  if (nrow(high_distribution) > 0) {
    p_composition <- high_distribution %>%
      ...
      labs(
        title = paste(scvelo_tumor_title, "time-bin composition of high continuous-ploidy cells by Dose"),
        x = "Dose",
  ...
  if (nrow(cluster_high_fraction) > 0) {
    p_heat <- cluster_high_fraction %>%
      ...
      labs(
        title = paste(scvelo_tumor_title, "high continuous-ploidy fraction by Dose and ordered cluster"),
        x = "Dose",
        y = "Cluster"
      ) +
      theme_extra(base_size = 10) +
  ...
  if (nrow(annotation_continuous_dat) > 0) {
    p_annotation_dose <- ggplot(annotation_continuous_dat, aes(x = .data$Dose, y = .data$cell_ploidy, fill = .data$Dose)) +
      ...
      facet_wrap(~annotation_ploidy, nrow = 1) +
      labs(
        title = paste0(scvelo_tumor_title, ": continuous cell ploidy response to Dose within 2N and 4N"),
        x = "Dose",
        y = "Continuous cell ploidy",
        fill = "Dose"
      ) +
  ...
  if (nrow(annotation_time_bin_dose_summary) > 0) {
    p_annotation_timebin <- annotation_time_bin_dose_summary %>%
      ...
      scale_color_manual(values = c("early" = "#1B9E77", "middle" = "#7570B3", "late" = "#D95F02"), drop = FALSE) +
      labs(
        title = paste(scvelo_tumor_title, "mean continuous cell ploidy by Dose within annotation ploidy and time bin"),
        x = "Dose",
        y = "Mean continuous cell ploidy",
        color = "Cluster time bin"
      ) +
  ...
  if (nrow(annotation_high_within_time_bin_summary) > 0) {
    p_annotation_high <- annotation_high_within_time_bin_summary %>%
      ...
      scale_color_manual(values = c("early" = "#1B9E77", "middle" = "#7570B3", "late" = "#D95F02"), drop = FALSE) +
      labs(
        title = paste(scvelo_tumor_title, "within-annotation high continuous-ploidy fraction by Dose and time bin"),
        x = "Dose",
  ...
  if (nrow(annotation_cluster_dose_summary) > 0) {
    p_annotation_cluster_mean <- annotation_cluster_dose_summary %>%
      ...
      labs(
        title = paste(scvelo_tumor_title, "mean continuous cell ploidy by Dose and ordered cluster within 2N and 4N"),
        x = "Dose",
        y = "Cluster"
      ) +
      theme_extra(base_size = 10) +
  ...
  if (nrow(annotation_high_within_cluster_summary) > 0) {
    p_annotation_cluster_high <- annotation_high_within_cluster_summary %>%
      ...
      labs(
        title = paste(scvelo_tumor_title, "within-annotation top-quartile continuous-ploidy fraction by Dose and cluster"),
        x = "Dose",
        y = "Cluster"
      ) +
      theme_extra(base_size = 10) +
...
run_ploidy_pseudotime_outputs <- function(df, method_dir, method_label, object_label, object_display_label, object_safe) {
  ...
  p <- plot_frequency_histogram(
    hist_df,
    "annotation_ploidy",
    paste(method_label, object_display_label, "2N vs 4N pseudotime frequency"),
    "Pseudotime"
  )
  if (!is.null(p)) {
  ...
  sample_status <- save_sample_frequency_pages(
    ...
    paste(method_label, object_display_label, "sample-level 2N vs 4N pseudotime frequency"),
    "Pseudotime"
  )
...
run_dose_pseudotime_outputs <- function(df, method_dir, method_label, object_label, object_display_label, object_safe) {
  ...
  p <- plot_frequency_histogram(
    hist_df,
    "Dose",
    paste(method_label, object_display_label, "Dose pseudotime frequency"),
    "Pseudotime"
  )
  if (!is.null(p)) {
  ...
  sample_status <- save_sample_frequency_pages(
    ...
    paste(method_label, object_display_label, "sample-level Dose pseudotime frequency"),
    "Pseudotime"
  )
  write_csv_safe(sample_status, file.path(out_dir, "sample_pages", "sample_histogram_status.csv"))
...
run_cluster_correlation_outputs <- function(df, method_dir, method_label, object_label, object_display_label, object_safe) {
  ...
  p <- plot_cluster_ploidy_correlation(
    df,
    paste(method_label, object_display_label, "cell ploidy and pseudotime by cluster")
  )
  if (!is.null(p)) {
...
run_tumor_ploidy_dose_outputs <- function(df, method_dir, method_label, object_display_label, object_safe) {
  ...
  added_outputs <- run_tumor_cell_ploidy_time_added_outputs(dat, out_dir, method_label, object_display_label)
...
run_cluster_ploidy_distribution_outputs <- function(df, method_dir, method_label, object_label, object_display_label, object_safe) {
  ...
  for (subset_label in c("all", "2N", "4N")) {
    res <- plot_cluster_cell_ploidy_distribution(
      df,
      subset_label,
      paste(method_label, object_display_label, subset_label, "cell ploidy distribution by cluster")
    )
...
run_method_object_analysis <- function(df, output_base) {
  ...
  object_display_label <- unique(df$analysis_object_display)
  ...
  if (length(object_display_label) != 1 || is.na(object_display_label) || !nzchar(object_display_label)) {
    object_display_label <- analysis_object_display_label(object_label)
  }
  ...
  ploidy_res <- run_ploidy_pseudotime_outputs(df, method_dir, method_label, object_label, object_display_label, object_safe)
  dose_res <- NULL
  if (identical(object_label, "Tumor/ploidy_all")) {
    dose_res <- run_dose_pseudotime_outputs(df, method_dir, method_label, object_label, object_display_label, object_safe)
  }
  corr_df <- run_cluster_correlation_outputs(df, method_dir, method_label, object_label, object_display_label, object_safe)
  tumor_res <- NULL
  if (identical(object_label, "Tumor/ploidy_all")) {
    tumor_res <- run_tumor_ploidy_dose_outputs(df, method_dir, method_label, object_display_label, object_safe)
  }
  dist_status <- run_cluster_ploidy_distribution_outputs(df, method_dir, method_label, object_label, object_display_label, object_safe)

  data.frame(
    method = method_label,
    analysis_object = object_label,
    analysis_object_display = object_display_label,
...
output_base <- get_env_scalar("EXTRA_04E_OUTPUT_ROOT", file.path(results_root, "04e_velocity_results_all_cells", scenario_id))
...
dry_run <- is_truthy_env("VELOCITY_RESULTS_ALL_CELLS_DRY_RUN", default = is_truthy_env("VELOCITY_RESULTS_DRY_RUN", default = FALSE))

message("04e velocity results all-cells analysis")
...
raw_metrics <- load_all_metrics(manifest) %>%
  dplyr::filter(is.finite(.data$pseudotime))
if (nrow(raw_metrics) == 0) {
  stop("No finite all_cells pseudotime rows were loaded for ", scenario_id, ".", call. = FALSE)
}
raw_metrics <- fill_umap_from_reference(raw_metrics)
tn_split_summary <- summarise_all_cells_tn_split(raw_metrics)
write_csv_safe(tn_split_summary, file.path(out_inputs, "all_cells_tn_split_summary.csv"))
metrics <- expand_all_cells_analysis_objects(raw_metrics)
...
write_csv_safe(
  metrics %>%
    ...
    dplyr::select(dplyr::any_of(c("method", "input_analysis_object", "analysis_object", "TN", "tn_split_source", "cell", "sample", "harvest", "barcode_raw", "annotation_ploidy", "Dose", "clusters"))) %>%
    utils::head(5000),
  file.path(out_inputs, "unmapped_cell_ploidy_examples.csv")
)

if (isTRUE(dry_run)) {
  message("VELOCITY_RESULTS_ALL_CELLS_DRY_RUN or VELOCITY_RESULTS_DRY_RUN is true; stopping after input discovery and mapping summaries.")
...
write_csv_safe(summary_df, file.path(output_base, "04e_velocity_results_all_cells_summary.csv"))
...
run_summary_lines <- c(
  "04e velocity results all-cells analysis completed.",
  ...
  paste0("Input all-cells method tables: ", nrow(manifest)),
  paste0("Loaded finite all-cells pseudotime rows: ", nrow(raw_metrics)),
  paste0("Derived finite method/object rows after all-cells TN split: ", nrow(metrics)),
  paste0("Rows with mapped Cell Ploidy: ", sum(is.finite(metrics$cell_ploidy), na.rm = TRUE)),
  "",
  "Main outputs:",
  "  00_inputs/input_metric_manifest.csv",
  "  00_inputs/all_cells_tn_split_summary.csv",

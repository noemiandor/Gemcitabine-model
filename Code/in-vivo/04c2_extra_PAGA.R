...
run_paga_scenario <- function(spec, results_root, output_base, deg_round2_root, top_n, max_cascade_genes, max_neighbor_edges) {
  ...
  paga_extra <- run_paga_extra_analysis(
    ...
    analysis_label = paste("04c2 PAGA", spec$scenario_id),
    top_n = top_n,
  ...
  run_summary_lines <- c(
    "04c2 extra PAGA analysis completed.",
    ...
    paste0("PAGA groups processed: ", nrow(status)),
    paste0("PAGA groups completed: ", if (nrow(status) > 0) sum(status$status == "completed", na.rm = TRUE) else 0L),
    paste0("PAGA groups partial: ", if (nrow(status) > 0) sum(status$status == "partial", na.rm = TRUE) else 0L),
    paste0("PAGA groups failed: ", if (nrow(status) > 0) sum(status$status == "failed", na.rm = TRUE) else 0L),
    "",
    "Main inputs:",
    "  04_paga_groups/**/paga_result.h5ad",
    "  04_paga_groups/**/paga_edges.csv",
    "  04_paga_groups/**/paga_cell_metrics.csv",
...
output_base <- get_env_scalar("EXTRA_PAGA_OUTPUT_ROOT", file.path(results_root, "04c2_extra_PAGA"))
...
scenario_specs <- data.frame(
  scenario_id = "root14_NOend",
  trajectory_branch = "root14_no_end",
  trajectory_subdir = "0401_trajectory",
  stringsAsFactors = FALSE
)
...
write_table_csv(summary_df, file.path(output_base, "04c2_PAGA_scenario_summary.csv"))
message("04c2 extra PAGA scenarios completed: ", paste(summary_df$scenario_id, collapse = ", "))

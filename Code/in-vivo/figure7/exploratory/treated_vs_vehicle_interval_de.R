...
prepare_origin_interaction_pathways <- function(
  ...
  selected_rows <- lapply(c("2N", "4N"), function(origin) {
    ...
    local$selected_direction <- if (identical(origin, "2N")) {
      "negative"
    } else {
      "positive"
    }
...
plot_origin_interaction_pathways <- function(
  ...
  selected <- comparison$selected_interactions
  if (any(!selected$collection %in% names(collection_labels))) {
  ...
  selected$panel_label <- factor(
    unname(panel_labels[selected$panel_id]),
    levels = unname(panel_labels)
  )
  selected$collection_label <- factor(
    as.character(selected$collection),
    levels = names(collection_labels),
    labels = unname(collection_labels)
  )
  selected$minus_log10_fdr <- -log10(
    pmax(as.numeric(selected$padj), .Machine$double.xmin)
  )
  selected$pathway_plot_key <- paste(
    selected$panel_id,
    selected$collection,
    selected$pathway,
    sep = "\r"
  ...
  ordered_pathway_keys <- unlist(lapply(names(panel_labels), function(panel_id) {
    local <- selected[selected$panel_id == panel_id, , drop = FALSE]
    local <- local[
  ...
  selected$pathway_plot_key <- factor(
    selected$pathway_plot_key,
    levels = ordered_pathway_keys
  )
  pathway_label_lookup <- setNames(
    as.character(selected$pathway_label),
    as.character(selected$pathway_plot_key)
  )
  pathway_labels <- setNames(
    vapply(
  ...
  plot_data <- do.call(rbind, lapply(c("2N", "4N"), function(origin) {
    data.frame(
      panel_label = as.character(selected$panel_label),
      collection_label = as.character(selected$collection_label),
      pathway_plot_key = as.character(selected$pathway_plot_key),
      origin = origin,
      origin_NES = selected[[paste0("NES_", origin)]],
      minus_log10_fdr = selected$minus_log10_fdr,
      stringsAsFactors = FALSE
    )
  }))
  ...
  symmetric_limit <- max(
    abs(c(selected$NES_2N, selected$NES_4N)),
    na.rm = TRUE
  ) * 1.08
  n_significant <- sum(selected$interaction_fdr_significant)
  ...
  plot <- ggplot2::ggplot() +
    ...
    ggplot2::labs(
      ...
      size = expression(-log[10]~"FDR"),
      caption = paste0(
        "Pathways were selected by the top 10 negative and top 10 positive formal interaction NES across Hallmark, Reactome, and GO BP.\n",
        ...
        "Point size is based on the formal-interaction FDR; ",
        sprintf(

...
figure7_read_cell_table <- function(path, compartment) {
  required <- c(
    ...
    "tumor_volume_delta_Day_17", "TGI_percent_Day_17"
  )
  ...
  numeric_columns <- unique(c(
    "gemcitabine_dose_mg_per_kg", "pseudotime", "cell_ploidy",
    "tumor_volume_baseline", "tumor_volume_delta_Day_17", "TGI_percent_Day_17",
...
figure7_sample_table <- function(cellcycle, noncellcycle, config) {
  ...
  rows <- lapply(sort(unique(union$sample_id)), function(id) {
    ...
    data.frame(
      ...
      tumor_volume_delta_Day_17 = figure7_unique_sample_value(cc, "tumor_volume_delta_Day_17", id, TRUE),
      embedded_TGI_percent_Day_17 = figure7_unique_sample_value(cc, "TGI_percent_Day_17", id, TRUE),
  ...
  for (ploidy in c("2N", "4N")) {
    ...
    samples$matched_control_reference_delta[hit] <- mean(controls$tumor_volume_delta_Day_17)
    samples$matched_control_n[hit] <- nrow(controls)
    samples$matched_control_sample_ids[hit] <- paste(sort(controls$sample_id), collapse = ";")
  }
  samples$TGI_percent_Day_17 <- 100 * (1 - samples$tumor_volume_delta_Day_17 /
                                          samples$matched_control_reference_delta)
  treated <- samples$dose_mg %in% as.numeric(unlist(config$tgi$treated_doses_mg_per_kg))
  difference <- abs(samples$TGI_percent_Day_17 - samples$embedded_TGI_percent_Day_17)
  tolerance <- as.numeric(config$tgi$numerical_tolerance)
  if (any(!is.finite(samples$embedded_TGI_percent_Day_17[treated]))) {
    figure7_stop("All treated mice require a finite embedded Day-17 TGI for verification")
  }
  if (any(difference[treated] > tolerance)) {
    figure7_stop("Recomputed Day-17 TGI disagrees with embedded values for: ",
  ...
  samples$tgi_outcome <- "day"; samples$tgi_day <- 17L
  samples$tgi_measure <- "TGI_percent_Day_17"
  samples$matched_control_summary <- "mean"; samples$matched_control_group <- "initial_ploidy"
  ...
  if (any(!is.finite(samples$TGI_percent_Day_17[treated_rows]))) {
    figure7_stop("All eight treated mice require finite recomputed Day-17 TGI")
  }
  rownames(samples) <- NULL
  samples
}

figure7_prepare_cellcycle <- function(cellcycle, samples) {
  ...
  data$TGI_percent_Day_17 <- samples$TGI_percent_Day_17[index]
  data
}

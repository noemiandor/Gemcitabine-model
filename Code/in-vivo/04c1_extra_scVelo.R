...
run_scvelo_scenario <- function(spec, results_root, output_base, deg_root, gsea_root) {
  ...
  if (nrow(velocity_cells) == 0) stop("No scVelo metric rows found for ", spec$scenario_id, call. = FALSE)
  velocity_cells$extra_scenario <- spec$scenario_id
  velocity_cells$extra_trajectory_branch <- spec$trajectory_branch
  ...
  method_answer_summary <- make_method_answer_summary(scvelo_separate, method_scvelo)
  ...
  run_summary_lines <- c(
    "04c1 extra scVelo analysis completed.",
...
write_table_csv(summary_df, file.path(output_base, "04c1_scVelo_scenario_summary.csv"))
message("04c1 extra scVelo scenarios completed: ", paste(summary_df$scenario_id, collapse = ", "))

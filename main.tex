...
Hypothesis 2: Cells cease dividing under treatment. Resistance is a function of how effectively tumor cells are able to enter and exit the polyploid state in response to drug concentration.\newline

When exposed to a drug, dividing cells ($D$) become polyploid ($P$) at a rate $k_p$. Dividing and polyploid cells die at different rates ($d_d$ and $d_p$ respectively)

\begin{equation}
\left\{\begin{array}{c}
\frac{\mathrm{d} D}{\mathrm{~d} t}=f(D)-k_{\mathrm{p}} \cdot \text { Exposure } \cdot D - d_d \cdot D \\
\frac{d P}{\mathrm{~d} t}=k_{\mathrm{p}} \cdot \text { Exposure } \cdot D - d_p \cdot P \\
T=D+P
\end{array}\right.
\end{equation}
